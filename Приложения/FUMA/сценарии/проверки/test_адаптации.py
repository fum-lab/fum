...
ПРИЛОЖЕНИЕ = Path(__file__).resolve().parents[2]


class ПроверкиАдаптации(unittest.TestCase):
    def test_манифест_сохраняет_39_исходных_и_все_новые_файлы(self):
        манифест = json.loads((ПРИЛОЖЕНИЕ / 'macOS/манифест-переноса.json').read_text())
        ...
        корень = ПРИЛОЖЕНИЕ.parents[1]
        перенос = json.loads((корень / 'Журнал/2026-09-15_22-05-55_MSK_подключить-платформенные-компоненты/материалы/перенос.json').read_text())
        новые_пути = {пара['исходник']: пара['назначение'] for пара in перенос['перемещения']}
        назначения = set()
        for запись in манифест['файлы'] + манифест['новые_файлы']:
            ...
            путь = корень / новые_пути.get(исторический_путь, исторический_путь)
            self.assertTrue(путь.is_file(), str(путь))
    ...
    def test_исходники_не_зависят_от_машинного_checkout(self):
        нарушения = []
        корень = ПРИЛОЖЕНИЕ.parents[1]
        имена = subprocess.check_output(['git', 'ls-files', '-z', '--', 'Приложения/FUMA'], cwd=корень).decode().split('\0')
    ...
    def test_шаблоны_не_предполагают_системных_каталогов(self):
        for путь in sorted((ПРИЛОЖЕНИЕ / 'сценарии').glob('*.sh')):
    ...
    def test_преобразование_публичного_шаблона_сохраняет_shebang(self):
        описание = importlib.util.spec_from_file_location('адаптация', ПРИЛОЖЕНИЕ / 'сценарии/адаптировать-прежние-пути.py')
    ...
    def test_launchd_хранится_как_неприменённый_шаблон(self):
        for путь in sorted((ПРИЛОЖЕНИЕ / 'macOS/launchd').glob('*.plist')):
    ...
    def test_установщики_закрыты_до_внешнего_действия(self):
        for путь in sorted((ПРИЛОЖЕНИЕ / 'сценарии').glob('install_*.sh')):
            with self.subTest(путь=путь.name):
...
''')
            ...
            сборка_существовала = (ПРИЛОЖЕНИЕ / '.build').exists()
            результат = subprocess.run(['bash', str(ПРИЛОЖЕНИЕ / 'сценарии/fum_mcp.sh')],
            ...
            self.assertEqual(сборка_существовала, (ПРИЛОЖЕНИЕ / '.build').exists())
    ...
    def test_миграция_формирует_отчёт_при_относительном_корне(self):
        with tempfile.TemporaryDirectory(prefix='fum-migration-') as каталог:
            ...
            результат = subprocess.run([sys.executable, str(ПРИЛОЖЕНИЕ / 'сценарии/адаптировать-прежние-пути.py'),
