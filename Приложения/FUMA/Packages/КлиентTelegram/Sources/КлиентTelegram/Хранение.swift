...
final class ЛокальныйЖурнал {
    ...
    func восстановить(применяя обработка: (ЛокальнаяЗапись) throws -> Void) throws {
        guard !отказ else { throw ОшибкаКлиента.требуетсяРазбор }
        for квитанция in контейнер.записи {
            ...
            try обработка(JSONDecoder().decode(ЛокальнаяЗапись.self, from: открыто))
        }
    }
