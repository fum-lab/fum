...
public struct ПараметрыБиблиотеки: Sendable {
    ...
    private let идентичностьБазы: ИдентичностьКаталога
    private let идентичностьФайлов: ИдентичностьКаталога
    public init(приложение: ДанныеПриложения, каталогБазы: URL, каталогФайлов: URL, ключБазы: Data) throws {
        ...
        идентичностьБазы = try идентичностьПриватногоКаталога(каталогБазы)
        идентичностьФайлов = try идентичностьПриватногоКаталога(каталогФайлов)
        guard идентичностьБазы != идентичностьФайлов else { throw ОшибкаКлиента.доступЗапрещён("Каталоги должны физически различаться") }
        self.приложение = приложение; self.каталогБазы = каталогБазы
        self.каталогФайлов = каталогФайлов; self.ключБазы = ключБазы
    }
    func проверитьКаталоги() throws {
        guard try идентичностьПриватногоКаталога(каталогБазы) == идентичностьБазы,
              try идентичностьПриватногоКаталога(каталогФайлов) == идентичностьФайлов else {
            throw ОшибкаКлиента.доступЗапрещён("Каталог заменён после подготовки параметров")
        }
    }
}
