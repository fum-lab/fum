...
private func проверитьЗаписиХвоста(корень: URL, ключ: Data) throws {
    ...
    try журнал.восстановить { запись in
        switch (записей, запись) {
        case (0, .разрыв): break
        case (1, .неприменённые(let хвост)) where хвост == [повреждённыйКадр()]: break
        default: throw ОшибкаКлиента.требуетсяРазбор
        }
        записей += 1
    }
    guard записей == 2 else { throw ОшибкаКлиента.требуетсяРазбор }
}
...
package func проверитьСквознойХвост(каталог: URL, подготовка: Bool) async throws -> Data {
    ...
    do { try чужойЖурнал.восстановить { _ in } } catch { чужойОтклонён = true }
    чужойЖурнал.закрыть()
