...
Подключить действующий запуск следующего шага как первый адаптер универсального диспетчера и мигрировать на месте уже существующую прикреплённую задачу и её пятиминутный heartbeat. Карточочный навык, схема рабочего набора и claim `branch_ref + step_id` должны остаться специализированным модулем, вызываемым общим слоем, а не быть переписаны в универсальный реестр.
...
- Общий диспетчер вызывает существующие `show`, `claim` и release-восстановление и сохраняет `branch_ref`, `step_id`, `card_id`, содержательный хэш и двойную проверку занятости Codex.
...
<!-- content-sha256: sha256:4daf39ce0833efe6395ac5385c100da82a47f9770a5a2dd6cdaa39a8bf8b5224 -->
<!-- FUM-MD-RECENCY:END -->
