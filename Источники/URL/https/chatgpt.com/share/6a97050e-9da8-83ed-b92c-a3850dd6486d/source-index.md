# Источник: Модель строительства сооружений

Исходный URL: <https://chatgpt.com/share/6a97050e-9da8-83ed-b92c-a3850dd6486d>

Тип источника: расшаренный диалог ChatGPT.

## Основные файлы

- [Оформленный диалог](модель-строительства-сооружений.md)
- [Отчёт об извлечении](extraction-report.md)
- [Структурный слой сообщений](chatgpt-share.messages.json)
- [Сохранённый HTML](chatgpt-share.html)

## Все сохранённые файлы

- `chatgpt-share.decoded-data.json`
- `chatgpt-share.headers.txt`
- `chatgpt-share.html`
- `chatgpt-share.initial-state.json`
- `chatgpt-share.messages.json`
- `chatgpt-share.react-router-stream.txt`
- `chatgpt-share.script-02.txt`
- `chatgpt-share.script-07.txt`
- `chatgpt-share.script-09.txt`
- `chatgpt-share.visible-text.txt`
- `extraction-report.md`
- `snapshot-manifest.json`
- `source-index.md`
- `source-url.txt`
- `модель-строительства-сооружений.md`

<!-- FUM-MD-RECENCY:BEGIN -->
<!-- last-content-edit: 2026-09-07 18:43:58 MSK -->
<!-- content-sha256: sha256:efe3e08cee03e75edb0805b88842027b1a2b156cc6932f05f47deb5ea9406dc7 -->
<!-- FUM-MD-RECENCY:END -->
