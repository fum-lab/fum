...
def основная():
    ...
    исходники = {путь.name: hashlib.sha256(путь.read_bytes()).hexdigest()
                 for путь in sorted(основа.SCRIPTS_DIR.glob("*.py"))}
    данные = {
        "схема": "fum.профиль-пакетного-анализа-политики.1",
        "python": platform.python_version(),
        "система": platform.system(),
        "архитектура": platform.machine(),
        "реализация": исходники,
        "профилировщик_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
