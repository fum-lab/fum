...
def дерево(root, ref):
    ...
    for name in directories:
        if not any(p.startswith(name + "/") for p in result):
            raise Отказ("Пустое поддерево не представимо точным индексом")
    return result


def проверить_копию(root, ref):
    expected = дерево(root, ref)
    actual = {}
    ...
    if actual != expected or git(root, "write-tree").decode().strip() != oid(root, ref + "^{tree}"):
...
def проверить_границу(root, old, new):
    ...
    before, after = дерево(root, old), дерево(root, new)
...
def перейти(root, old, leading, new, intent, transaction_root, *, наблюдатель=lambda phase: None):
    ...
    state = {"M": old, "L": leading, "C": new, "T": oid(root, new + "^{tree}"),
    ...
    if current == new:
        ...
        проверить_копию(root, new)
        return {"состояние": "уже_достигнуто", "коммит": new}
    if current != old:
        raise Отказ("Master изменился")
    проверить_границу(root, old, new)
    проверить_копию(root, old)
    ...
    try:
        ...
        head_guard = subprocess.Popen(команда_git() + ["update-ref", "--no-deref", "--stdin"],
                                      cwd=root, env=среда(), stdin=subprocess.PIPE,
        ...
        process = subprocess.Popen(команда_git() + ["update-ref", "--no-deref", "--stdin"],
                                   cwd=transaction_root, env=среда(), stdin=subprocess.PIPE,
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=0)
