...
class ПроверкаПереводаОбъявленийКода(unittest.TestCase):
    ...
    def test_инвентарь_исключает_производную_проекцию(сам) -> None:
        with tempfile.TemporaryDirectory() as временный_каталог:
            корень = Path(временный_каталог)
            сам.записать(
                корень,
                "Proyekcii/Bratislavskaya-pamyatj/fajlyi/Instrumentyi/kopiya.py",
                "def old_generated_copy():\n    pass\n",
            )
            сам.записать(
                корень,
                "Proyekcii-copy/канонический.py",
                "def old_prefix_near_miss():\n    pass\n",
            )
            сам.записать(
                корень,
                "Документация/Proyekcii/канонический.py",
                "def old_nested_near_miss():\n    pass\n",
            )
            ...
            сам.assertEqual(результат.returncode, 0, результат.stderr)
            объявления = json.loads(результат.stdout)["объявления"]
            сам.assertEqual(
                {запись["имя"] for запись in объявления},
                {
                    "old_prefix_near_miss",
                    "old_nested_near_miss",
                },
            )
        ...
        with tempfile.TemporaryDirectory() as временный_каталог:
            ...
            сам.записать(
                корень,
                "proyekcii/канонический.py",
                "def old_lowercase_near_miss():\n    pass\n",
            )
            ...
            сам.assertEqual(
                {запись["имя"] for запись in объявления},
                {"old_lowercase_near_miss"},
            )

    def test_инвентарь_исключает_только_корневые_подузлы(сам) -> None:
        with tempfile.TemporaryDirectory() as временный_каталог:
            корень = Path(временный_каталог)
            сам.записать(
                корень,
                "Подузлы/слот-0001/копия.py",
                "def old_worktree_copy():\n    pass\n",
            )
            сам.записать(
                корень,
                "Документация/Подузлы/канонический.py",
    ...
    def test_карта_разрешает_обязательный_префикс_обнаружения_теста(сам) -> None:
        with tempfile.TemporaryDirectory() as временный_каталог:
            корень = Path(временный_каталог)
            файл = сам.записать(
                корень,
                    ...
                    сам.карта(
                        [
                            (
                                "код.py",
                                сам.хэш(файл),
                                {"test_old_case": "test_русский_случай"},
                            )
                        ]
                    ),
                    ensure_ascii=False,
