...
def главный():
    ...
    манифест = КОРЕНЬ / "Планирование/финансирование-и-ресурсы/исходные-данные/очистка-снимков.json"
    профиль = Профиль()
    входы = {}
    набор = []
    отдельные_документы = []
    for запись in json.loads(манифест.read_text())["снимки"]:
        ...
        if (каталог / "body.pdf").is_file():
            отдельные_документы.append(запись["URL"])
            continue
    ...
    for повтор in range(7):
        with профиль.стадия("Очистка сохранённого набора"):
            for тело, заголовки in набор:
                очищено = архив.очистить_служебную_разметку(тело)
