...
async function прочитать_кратко(среда, параметры) {
    ...
    try {
        const {корень, задача, режим, снимок, ключ_кэша, максимум_байтов = 16000, профиль_представления = "прежний"} = параметры;
        if (![корень, задача, снимок, ключ_кэша].every(значение => typeof значение === "string" && значение.length > 0 && !/[\x00-\x1f]/.test(значение)) ||
            !["новый", "сохранённый"].includes(режим) || !["прежний", "порождённый"].includes(профиль_представления) || !Number.isInteger(максимум_байтов) ||
        ...
        const результат = await команда(["python3", "-B", [корень, "Инструменты/fum-svyaznostj-rabochej-sessii/scripts/показать-ответ-задачи.py"].join("/"),
            ...
            "--максимум-байтов", максимум_байтов, "--путь-в-результате", "--профиль", профиль_представления]);
