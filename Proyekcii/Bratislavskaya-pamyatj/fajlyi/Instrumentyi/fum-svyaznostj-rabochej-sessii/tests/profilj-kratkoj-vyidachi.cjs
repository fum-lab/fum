...
async function выполнить() {
    ...
    try {
        ...
        const хэш = крипто.createHash("sha256").update(данные).digest("hex");
        const запись = {состояние: "принят", корень, задача, снимок, sha256: хэш};
        let команды = 0;
        ...
        for (let пара = 0; пара < 3; пара++) {
            for (const формат of пара % 2 ? ["краткий", "подробный"] : ["подробный", "краткий"]) {
                ...
                замеры.push({пара, формат, длительность_наносекунды: длительность, байтов: Buffer.byteLength(выход), хэш: крипто.createHash("sha256").update(выход).digest("hex")});
            }
        }
        утверждать.equal(фс.readFileSync(снимок, "utf8"), данные);
        for (let пара = 0; пара < 3; пара++) утверждать.ok(замеры.find(замер => замер.пара === пара && замер.формат === "краткий").байтов < замеры.find(замер => замер.пара === пара && замер.формат === "подробный").байтов);
        const результат = {схема: "fum.профиль-краткой-выдачи.1", версия_среды: process.version, хэш_входа: хэш, вход_байтов: Buffer.byteLength(данные), команды, вызовы_интерфейса: 0, вход_неизменен: true, замеры,
        ...
        console.log(JSON.stringify({команды, подробный_байтов: замеры.find(замер=>замер.формат==="подробный").байтов, краткий_байтов: замеры.find(замер=>замер.формат==="краткий").байтов}));
