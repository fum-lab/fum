...
СКРИПТ_БРАТИСЛАВСКОЙ_ПРОЕКЦИИ = Path(
    "Инструменты/fum-bratislavskaya-proyekciya-pamyati/scripts/"
    "братиславская_проекция_памяти.py"
)
КОНТРАКТ_БРАТИСЛАВСКОЙ_ПРОЕКЦИИ = Path(
    "Инструменты/fum-bratislavskaya-proyekciya-pamyati/"
    "контракт-v2.json"
)
МАНИФЕСТ_БРАТИСЛАВСКОЙ_ПРОЕКЦИИ = Path(
...
СХЕМА_СНИМКА_ЗАПУСКОВ_С_РАУНДАМИ = "fum.test-run-report.v3"
...
ДОКУМЕНТАЦИОННЫЕ_НАБОРЫ_ТЕСТОВ = (
    Path("Инструменты/fum-bratislavskaya-proyekciya-pamyati/tests"),
...
def прочитать_наблюдения_закрытого_снимка(
    ...
    if схема_снимка == СХЕМА_СНИМКА_ЗАПУСКОВ:
    ...
    elif схема_снимка in {СХЕМА_СНИМКА_ЗАПУСКОВ_С_ВЕРДИКТОМ, СХЕМА_СНИМКА_ЗАПУСКОВ_С_РАУНДАМИ}:
        ...
        if схема_снимка == СХЕМА_СНИМКА_ЗАПУСКОВ_С_РАУНДАМИ:
            ожидаемые_поля_снимка.add("отпечаток_содержимого_закрытия")
            содержимое_закрытия = снимок.get("отпечаток_содержимого_закрытия")
            if not isinstance(содержимое_закрытия, str) or SHA256_RE.fullmatch(содержимое_закрытия) is None:
                raise ValueError(f"invalid closed content fingerprint: {путь_снимка}")
    else:
...
def build_steps(
    ...
    *,
    ...
    steps.append(
        SmokeStep(
            name="Проверка братиславской проекции памяти",
            command=(
                python_cmd,
                скрипт_проекции,
                "проверить-манифест",
                ...
                контракт_проекции,
                "--манифест",
                МАНИФЕСТ_БРАТИСЛАВСКОЙ_ПРОЕКЦИИ.as_posix(),
            ),
            ранняя_проверка=True,
        )
    )
