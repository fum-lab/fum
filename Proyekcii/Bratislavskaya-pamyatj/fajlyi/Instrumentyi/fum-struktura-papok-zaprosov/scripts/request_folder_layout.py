...
КОММЕНТАРИЙ_ПРЯМЫХ_ПРОВЕРОК = (
    "<!-- Управляемый блок между marker-строками формирует "
    "fum-otchyotyi-o-zapuskakh-proverok; строки таблицы и итог вручную "
    "не редактировать. -->"
)
НАЧАЛЬНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК = (
    "<!-- FUM-CHECK-RUNS:BEGIN состояние=открыт; "
    "каталог=материалы/запуски-проверок -->"
)
КОНЕЧНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК = "<!-- FUM-CHECK-RUNS:END -->"
...
def _проверить_формат_шаблона_отчёта(текст: str) -> None:
    ...
    каркас_запусков = re.compile(
        ...
        rf"{re.escape(НАЧАЛЬНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК)}\n\n"
        rf"{re.escape(МАРКЕР_НЕЗАПОЛНЕННОГО_ШАБЛОНА)}\n\n"
    ...
    if (
        текст.count(НАЧАЛЬНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК) != 1
        or текст.count(КОНЕЧНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК) != 1
        or текст.index(НАЧАЛЬНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК)
        >= текст.index(КОНЕЧНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК)
    ):
        raise LayoutError("шаблон отчёта имеет неверную marker-пару прямых проверок")
    _проверить_активность_каркаса(
        текст,
        (
            МАРКЕР_НЕЗАПОЛНЕННОГО_ШАБЛОНА,
            КОММЕНТАРИЙ_ПРЯМЫХ_ПРОВЕРОК,
            НАЧАЛЬНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК,
            КОНЕЧНЫЙ_МАРКЕР_ПРЯМЫХ_ПРОВЕРОК,
        ),
    )
...
def _project_relative_files(repo_root: Path) -> tuple[PurePosixPath, ...]:
    ...
    for raw in result.stdout.split(b"\0"):
        ...
        if relative.parts and relative.parts[0] == "Proyekcii":
            continue
