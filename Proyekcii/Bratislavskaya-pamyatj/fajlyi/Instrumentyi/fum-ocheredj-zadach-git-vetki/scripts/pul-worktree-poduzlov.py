...
ИМЯ_НАМЕРЕНИЯ_ПЕРЕКЛЮЧЕНИЯ = ".fum-намерение-переключения.json"
...
@dataclass(frozen=True)
class ОписаниеПодмодуля:
    имя: str
    путь: str
    ревизия: str
    адрес_форка: str
    адрес_исходного_репозитория: str | None
    путь_источника: Path
    каталог_источника_системы_версий: Path
...
def рабочее_дерево_вне_подмодулей_чисто(путь: Path) -> bool:
    return текст_команды_версий(
        путь,
        [
            "status",
...
def декодировать_нулевые_значения(
    сырые_байты: bytes,
    *,
    состояние: str,
    назначение: str,
    ...
    if not сырые_байты:
        return []
    части = сырые_байты.split(b"\0")
    if части[-1] != b"":
        raise ОшибкаПула(65, состояние, f"{назначение} вернула некорректный NUL-формат.")
    try:
        return [часть.decode("utf-8", errors="strict") for часть in части[:-1]]
    except UnicodeDecodeError as ошибка:
        raise ОшибкаПула(65, состояние, f"{назначение} не является UTF-8.") from ошибка


def прочитать_значения_конфигурации(
    корень: Path,
    аргументы: Sequence[str],
    *,
    состояние: str,
    ...
    результат = выполнить_команду_версий(
        корень,
        ["config", "--null", *аргументы],
        проверять=False,
    )
    if результат.код == 1:
        return []
    if результат.код != 0:
        raise ОшибкаПула(65, состояние, f"Не удалось прочитать {назначение}.")
    return декодировать_нулевые_значения(
        результат.вывод,
        состояние=состояние,
...
def проверить_имя_или_путь_подмодуля(значение: str, назначение: str) -> tuple[str, ...]:
    разобранный = PurePosixPath(значение)
    служебный_компонент = any(
        часть == ".fum-устаревшее-рабочее-дерево"
        or часть.startswith(".fum-переходный-подмодуль-")
        or часть.startswith(".fum-указатель-подмодуля-")
        for часть in разобранный.parts
    )
    if (
        not значение
        or значение != значение.strip()
        or значение.startswith("-")
        or значение != unicodedata.normalize("NFC", значение)
        or разобранный.is_absolute()
        or разобранный.as_posix() != значение
        or "\\" in значение
        or any(ord(знак) < 32 or ord(знак) == 127 for знак in значение)
        or any(часть in {"", ".", "..", ".git"} for часть in разобранный.parts)
        or служебный_компонент
    ):
        raise ОшибкаПула(
            65,
            "некорректная_регистрация_подмодуля",
            f"{назначение} подмодуля не является безопасным относительным Git-путём.",
        )
    return разобранный.parts


def прочитать_единственное_значение_подмодуля(
    контекст: КонтекстПула,
    объект_конфигурации: str,
    ключ: str,
    *,
    обязательное: bool,
) -> str | None:
    значения = прочитать_значения_конфигурации(
        контекст.основной_корень,
        ["--blob", объект_конфигурации, "--get-all", ключ],
        состояние="некорректная_регистрация_подмодуля",
        назначение=ключ,
    )
    допустимая_кратность = len(значения) == 1 if обязательное else len(значения) <= 1
    if not допустимая_кратность or any(
        not значение or any(ord(знак) < 32 or ord(знак) == 127 for знак in значение)
        for значение in значения
    ):
        raise ОшибкаПула(
            65,
            "некорректная_регистрация_подмодуля",
            f"{ключ} должна иметь {'ровно одно' if обязательное else 'не более одного'} непустое значение.",
        )
    return значения[0] if значения else None


def прочитать_ревизию_подмодуля(
    контекст: КонтекстПула,
    вершина: str,
    путь: str,
) -> str:
    ...
    записи = результат.вывод.split(b"\0")
    if len(записи) != 2 or записи[-1] != b"" or not записи[0]:
        raise ОшибкаПула(
            65,
            "некорректная_регистрация_подмодуля",
            f"{путь}: в вершине отсутствует единственный gitlink.",
        )
    метаданные, разделитель, сырое_имя = записи[0].partition(b"\t")
    try:
        фактический_путь = сырое_имя.decode("utf-8", errors="strict")
        поля = метаданные.decode("ascii", errors="strict").split()
    except UnicodeError as ошибка:
        raise ОшибкаПула(
            65,
            "некорректная_регистрация_подмодуля",
            f"{путь}: gitlink имеет некорректную кодировку.",
        ) from ошибка
    if (
        not разделитель
        ...
        raise ОшибкаПула(
            65,
            "некорректная_регистрация_подмодуля",
            f"{путь}: запись дерева не является exact gitlink.",
        )
    return поля[2]


def потребовать_обычный_путь_внутри(корень: Path, части: Sequence[str], назначение: str) -> Path:
    текущий = корень
    for часть in части:
        ...
        if текущий.is_symlink():
            raise ОшибкаПула(
                73,
                "некорректный_источник_подмодуля",
                f"{назначение} не должен проходить через символическую ссылку.",
            )
    разрешённый = текущий.resolve()
    try:
        разрешённый.relative_to(корень.resolve())
    except ValueError as ошибка:
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля",
            f"{назначение} вышел за разрешённый корень.",
        ) from ошибка
    return разрешённый


def прочитать_локальные_значения(корень: Path, ключ: str) -> list[str]:
    return прочитать_значения_конфигурации(
        корень,
        ["--local", "--get-all", ключ],
        состояние="некорректный_источник_подмодуля",
        назначение=ключ,
    )


def проверить_удалённые_источники_подмодуля(
    корень: Path,
    описание: ОписаниеПодмодуля,
    *,
    источник: bool,
) -> None:
    ожидаемые = {"origin"}
    if описание.адрес_исходного_репозитория is not None:
        ожидаемые.add("upstream")
    результат = выполнить_команду_версий(корень, ["remote"], проверять=False)
    if результат.код != 0:
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
            f"{описание.путь}: список remote недоступен.",
        )
    try:
        фактические = set(результат.вывод.decode("utf-8", errors="strict").splitlines())
    except UnicodeDecodeError as ошибка:
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
            f"{описание.путь}: имя remote не является UTF-8.",
        ) from ошибка
    if фактические != ожидаемые:
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
            f"{описание.путь}: ожидаются только канонические remote.",
        )
    адреса = {
        "origin": описание.адрес_форка,
        "upstream": описание.адрес_исходного_репозитория,
    }
    for имя in sorted(ожидаемые):
        if прочитать_локальные_значения(корень, f"remote.{имя}.url") != [адреса[имя]]:
            raise ОшибкаПула(
                73,
                "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
                f"{описание.путь}: URL remote {имя} не совпал с регистрацией.",
            )
        if прочитать_локальные_значения(корень, f"remote.{имя}.pushurl"):
            raise ОшибкаПула(
                73,
                "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
                f"{описание.путь}: отдельный pushurl remote {имя} не допускается.",
            )
        if прочитать_локальные_значения(корень, f"remote.{имя}.fetch") != [
            f"+refs/heads/*:refs/remotes/{имя}/*"
        ]:
            raise ОшибкаПула(
                73,
                "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
                f"{описание.путь}: refspec remote {имя} неканоничен.",
            )
        ссылки = текст_команды_версий(
            корень,
            ["for-each-ref", "--format=%(refname)", f"refs/remotes/{имя}"],
        ).splitlines()
        if not ссылки:
            raise ОшибкаПула(
                73,
                "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
                f"{описание.путь}: remote {имя} не имеет локального tracking-ref.",
            )
    достижима = any(
        выполнить_команду_версий(
            корень,
            ["merge-base", "--is-ancestor", описание.ревизия, ссылка],
            проверять=False,
        ).код
        == 0
        for ссылка in текст_команды_версий(
            корень,
    ...
    if not достижима:
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля" if источник else "несовпадение_материализации_подмодуля",
            f"{описание.путь}: gitlink не достижим из tracking-ref origin.",
        )


def проверить_автономное_хранилище_объектов(
    каталог_системы_версий: Path,
    *,
    состояние: str,
    описание: str,
) -> None:
    объекты = каталог_системы_версий / "objects"
    сведения = объекты / "info"
    альтернативы = сведения / "alternates"
    if объекты.is_symlink() or not объекты.is_dir():
        raise ОшибкаПула(73, состояние, f"{описание}: каталог objects неканоничен.")
    if сведения.is_symlink() or (сведения.exists() and not сведения.is_dir()):
        raise ОшибкаПула(73, состояние, f"{описание}: каталог objects/info неканоничен.")
    if os.path.lexists(альтернативы):
        raise ОшибкаПула(
            73,
            состояние,
            f"{описание}: запрещено внешнее хранилище объектов.",
        )
    каталог_пакетов = объекты / "pack"
    if каталог_пакетов.is_symlink() or (
        каталог_пакетов.exists()
        and (
            not каталог_пакетов.is_dir()
            or any(каталог_пакетов.glob("*.promisor"))
        )
    ):
        raise ОшибкаПула(
            73,
            состояние,
            f"{описание}: частичное хранилище объектов не допускается.",
        )


def проверить_отсутствие_частичного_хранилища(
    корень: Path,
    *,
    состояние: str,
    описание: str,
) -> None:
    частичное_расширение = прочитать_значения_конфигурации(
        корень,
        ["--local", "--get-all", "extensions.partialClone"],
        состояние=состояние,
        назначение="extensions.partialClone",
    )
    обещания_удалённых_источников = прочитать_значения_конфигурации(
        корень,
        ["--local", "--get-regexp", r"^remote\..*\.promisor$"],
        состояние=состояние,
        назначение="remote.*.promisor",
    )
    if частичное_расширение or обещания_удалённых_источников:
        raise ОшибкаПула(
            73,
            состояние,
            f"{описание}: partial clone и promisor remote не допускаются.",
        )


def проверить_источник_подмодуля(
    контекст: КонтекстПула,
    описание: ОписаниеПодмодуля,
) -> None:
    путь = описание.путь_источника
    метка = путь / ".git"
    if not путь.is_dir() or not метка.is_file() or метка.is_symlink():
        raise ОшибкаПула(
            73,
            "источник_подмодуля_не_инициализирован",
            f"{описание.путь}: основной checkout не содержит материализованный submodule.",
        )
    проверить_автономное_хранилище_объектов(
        описание.каталог_источника_системы_версий,
        состояние="альтернативное_хранилище_источника_подмодуля",
        описание=описание.путь,
    )
    фактический_корень = Path(
        текст_команды_версий(путь, ["rev-parse", "--show-toplevel"])
    ).resolve()
    фактический_каталог = Path(
        текст_команды_версий(путь, ["rev-parse", "--absolute-git-dir"])
    ).resolve()
    фактический_общий = Path(
        текст_команды_версий(
            путь,
            ["rev-parse", "--path-format=absolute", "--git-common-dir"],
        )
    ).resolve()
    корень_надрепозитория = текст_команды_версий(
        путь,
        ["rev-parse", "--show-superproject-working-tree"],
    )
    if (
        фактический_корень != путь
        or фактический_каталог != описание.каталог_источника_системы_версий
        or фактический_общий != описание.каталог_источника_системы_версий
        or not корень_надрепозитория
        or Path(корень_надрепозитория).resolve() != контекст.основной_корень
    ):
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля",
            f"{описание.путь}: источник имеет неканоническую Git-топологию.",
        )
    if текст_команды_версий(
        путь,
        ["symbolic-ref", "--quiet", "HEAD"],
        проверять=False,
    ):
        raise ОшибкаПула(
            73,
            "присоединённый_источник_подмодуля",
            f"{описание.путь}: источник submodule обязан иметь detached HEAD.",
        )
    проверить_отсутствие_частичного_хранилища(
        путь,
        состояние="частичный_источник_подмодуля",
        описание=описание.путь,
    )
    if not рабочее_дерево_чисто(путь):
        raise ОшибкаПула(
            73,
            "изменённый_источник_подмодуля",
            f"{описание.путь}: локальный источник submodule изменён.",
        )
    if текст_команды_версий(путь, ["rev-parse", "HEAD"]) != описание.ревизия:
        raise ОшибкаПула(
            73,
            "несовпадение_вершины_источника_подмодуля",
            f"{описание.путь}: HEAD локального источника не совпал с gitlink.",
        )
    if текст_команды_версий(путь, ["rev-parse", "--is-shallow-repository"]) != "false":
        raise ОшибкаПула(
            73,
            "некорректный_источник_подмодуля",
            f"{описание.путь}: shallow-источник submodule не допускается.",
        )
    проверить_автономное_хранилище_объектов(
        описание.каталог_источника_системы_версий,
        состояние="альтернативное_хранилище_источника_подмодуля",
    ...
    проверить_удалённые_источники_подмодуля(путь, описание, источник=True)


def описания_подмодулей_вершины(
    контекст: КонтекстПула,
    вершина: str,
    *,
    проверять_источники: bool = True,
) -> list[ОписаниеПодмодуля]:
    запись = выполнить_команду_версий(
        контекст.основной_корень,
        ["ls-tree", "-z", вершина, "--", ".gitmodules"],
    ).вывод
    if not запись:
        return []
    части_записи = запись.split(b"\0")
    if len(части_записи) != 2 or части_записи[-1] != b"":
        raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", ".gitmodules имеет неверную запись дерева.")
    метаданные, разделитель, сырое_имя = части_записи[0].partition(b"\t")
    поля = метаданные.decode("ascii", errors="strict").split()
    if not разделитель or сырое_имя != b".gitmodules" or len(поля) != 3 or поля[:2] != ["100644", "blob"]:
        raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", ".gitmodules должен быть обычным blob режима 100644.")
    объект_конфигурации = поля[2]
    результат = выполнить_команду_версий(
        контекст.основной_корень,
        [
            "config",
    ...
    if результат.код not in {0, 1}:
        raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", "Не удалось разобрать .gitmodules.")
    записи = декодировать_нулевые_значения(
        результат.вывод,
        состояние="некорректная_регистрация_подмодуля",
        назначение="перечень путей .gitmodules",
    )
    if not записи:
        raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", ".gitmodules не содержит зарегистрированных путей.")
    описания: list[ОписаниеПодмодуля] = []
    встреченные_имена: set[str] = set()
    встреченные_пути: set[str] = set()
    встреченные_части_имён: list[tuple[str, ...]] = []
    встреченные_части_путей: list[tuple[str, ...]] = []
    for запись_пути in записи:
        ключ, разделитель, путь = запись_пути.partition("\n")
        if not разделитель or not ключ.startswith("submodule.") or not ключ.endswith(".path"):
            raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", "Запись пути .gitmodules повреждена.")
        имя = ключ.removeprefix("submodule.").removesuffix(".path")
        части_имени = проверить_имя_или_путь_подмодуля(имя, "Имя")
        части_пути = проверить_имя_или_путь_подмодуля(путь, "Путь")
        пересекающиеся_имена = any(
            части_имени[: len(прежние)] == прежние
            or прежние[: len(части_имени)] == части_имени
            for прежние in встреченные_части_имён
        )
        пересекающиеся_пути = any(
            части_пути[: len(прежние)] == прежние
            or прежние[: len(части_пути)] == части_пути
            for прежние in встреченные_части_путей
        )
        if (
            имя in встреченные_имена
            or путь in встреченные_пути
            or пересекающиеся_имена
            or пересекающиеся_пути
        ):
            raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", "Имена или пути submodule повторены либо пересекаются.")
        встреченные_имена.add(имя)
        встреченные_пути.add(путь)
        встреченные_части_имён.append(части_имени)
        встреченные_части_путей.append(части_пути)
        адрес_форка = прочитать_единственное_значение_подмодуля(
            контекст,
            объект_конфигурации,
            f"submodule.{имя}.url",
            обязательное=True,
        )
        адрес_исходного_репозитория = прочитать_единственное_значение_подмодуля(
            контекст,
            объект_конфигурации,
            f"submodule.{имя}.fumUpstream",
            обязательное=False,
        )
        if адрес_форка is None:
            raise ОшибкаПула(65, "некорректная_регистрация_подмодуля", f"{путь}: URL форка отсутствует.")
        ревизия = прочитать_ревизию_подмодуля(контекст, вершина, путь)
        if проверять_источники:
            путь_источника = потребовать_обычный_путь_внутри(
                контекст.основной_корень,
                части_пути,
                f"Источник {путь}",
            )
            каталог_источника = потребовать_обычный_путь_внутри(
                контекст.общий_каталог_системы_версий,
                ("modules", *части_имени),
                f"Git-каталог источника {имя}",
            )
        else:
            путь_источника = контекст.основной_корень.joinpath(*части_пути)
            каталог_источника = контекст.общий_каталог_системы_версий.joinpath(
                "modules",
                *части_имени,
            )
        описание = ОписаниеПодмодуля(
            ...
            путь=путь,
            ревизия=ревизия,
            адрес_форка=адрес_форка,
            адрес_исходного_репозитория=адрес_исходного_репозитория,
            путь_источника=путь_источника,
            каталог_источника_системы_версий=каталог_источника,
        )
        if проверять_источники:
            проверить_источник_подмодуля(контекст, описание)
        описания.append(описание)
    return sorted(описания, key=lambda описание: описание.путь)


def среда_локального_транспорта() -> dict[str, str]:
    return {
...
def проверить_топологию_материализованного_подмодуля(
    путь_слота: Path,
    путь_подмодуля: Path,
    ожидаемый_каталог: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    метка = путь_подмодуля / ".git"
    if not путь_подмодуля.is_dir() or not метка.is_file() or метка.is_symlink():
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: materialized submodule не имеет обычного .git-файла.",
        )
    фактический_корень = Path(
        текст_команды_версий(путь_подмодуля, ["rev-parse", "--show-toplevel"])
    ).resolve()
    фактический_каталог = Path(
        текст_команды_версий(путь_подмодуля, ["rev-parse", "--absolute-git-dir"])
    ).resolve()
    фактический_общий = Path(
        текст_команды_версий(
            путь_подмодуля,
    ...
    if (
        фактический_корень != путь_подмодуля
        or фактический_каталог != ожидаемый_каталог
        or фактический_общий != ожидаемый_каталог
        or not корень_надрепозитория
        or Path(корень_надрепозитория).resolve() != путь_слота
    ):
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: materialized submodule имеет неканоническую Git-топологию.",
        )


def потребовать_отсутствие_перенаправлений_локального_источника(
    путь_подмодуля: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    результат = выполнить_команду_версий(
    ...
    if результат.код != 0:
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: локальная конфигурация submodule недоступна.",
        )
    try:
        имена = результат.вывод.decode("utf-8", errors="strict").splitlines()
    except UnicodeDecodeError as ошибка:
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: локальная конфигурация submodule не является UTF-8.",
        ) from ошибка
    if any(
        имя.casefold().startswith("url.")
        and имя.casefold().endswith((".insteadof", ".pushinsteadof"))
        for имя in имена
    ):
        raise ОшибкаПула(
            73,
            "перенаправление_локального_источника_подмодуля",
            f"{описание.путь}: url.* rewrite запрещён для локальной материализации.",
        )


def удалить_переходные_остатки_подмодуля(родитель: Path) -> None:
    for остаток in sorted(родитель.glob(".fum-переходный-подмодуль-*")):
        if остаток.is_symlink() or not остаток.is_dir():
            raise ОшибкаПула(
                73,
                "некорректный_переходный_остаток_подмодуля",
                "Переходный каталог локального clone имеет неожиданный тип.",
            )
        shutil.rmtree(остаток)
    for остаток in sorted(родитель.glob(".fum-указатель-подмодуля-*")):
        if остаток.is_symlink() or not остаток.is_file():
            raise ОшибкаПула(
                73,
                "некорректный_переходный_остаток_подмодуля",
                "Переходный .git-указатель имеет неожиданный тип.",
            )
        остаток.unlink()


def подготовить_локальный_каталог_подмодуля(
    путь_слота: Path,
    ожидаемый_каталог: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    if ожидаемый_каталог.is_symlink():
        raise ОшибкаПула(
            73,
            "остаток_материализации_подмодуля",
            f"{описание.путь}: Git-каталог materialization не может быть символической ссылкой.",
        )
    ожидаемый_каталог.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    удалить_переходные_остатки_подмодуля(ожидаемый_каталог.parent)
    остаток_устаревшего = ожидаемый_каталог / ".fum-устаревшее-рабочее-дерево"
    if ожидаемый_каталог.exists():
        if остаток_устаревшего.is_dir() and not остаток_устаревшего.is_symlink():
            shutil.rmtree(ожидаемый_каталог)
        elif ожидаемый_каталог.is_dir() and not any(ожидаемый_каталог.iterdir()):
            ожидаемый_каталог.rmdir()
        else:
            # Атомарная установка каталога могла завершиться до записи .git-файла.
            # Полноту такого состояния проверит общий readback после записи указателя.
            return
    переходный_корень = Path(
        tempfile.mkdtemp(
            prefix=".fum-переходный-подмодуль-",
            dir=ожидаемый_каталог.parent,
        )
    )
    результат = выполнить_команду_версий(
        путь_слота,
        [
            ...
            "--no-hardlinks",
            "--no-checkout",
            str(описание.каталог_источника_системы_версий),
            str(переходный_корень),
        ],
        проверять=False,
        дополнения_среды=среда_локального_транспорта(),
    )
    if результат.код != 0:
        пояснение = результат.ошибка.decode("utf-8", errors="replace").strip()
        shutil.rmtree(переходный_корень)
        raise ОшибкаПула(
            73,
            "отказ_локального_клонирования_подмодуля",
            f"{описание.путь}: локальное клонирование submodule завершилось отказом.",
            пояснение=пояснение,
        )
    переходный_каталог = переходный_корень / ".git"
    if (
        переходный_каталог.is_symlink()
        or not переходный_каталог.is_dir()
        or {запись.name for запись in переходный_корень.iterdir()} != {".git"}
    ):
        raise ОшибкаПула(
            73,
            "несовпадение_локального_клонирования_подмодуля",
            f"{описание.путь}: переходный clone имеет неожиданный рабочий каталог.",
        )
    try:
        os.rename(переходный_каталог, ожидаемый_каталог)
    except OSError as ошибка:
        raise ОшибкаПула(
            73,
            "отказ_атомарной_установки_подмодуля",
            f"{описание.путь}: Git-каталог materialization не установлен атомарно.",
            пояснение=str(ошибка),
        ) from ошибка
    переходный_корень.rmdir()


def записать_указатель_каталога_системы_версий_подмодуля(
    путь_подмодуля: Path,
    ожидаемый_каталог: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    if путь_подмодуля.is_symlink() or not путь_подмодуля.is_dir() or any(путь_подмодуля.iterdir()):
        raise ОшибкаПула(
            73,
            "путь_подмодуля_занят",
            f"{описание.путь}: путь нового submodule должен быть пустым каталогом.",
        )
    временный_файл: Path | None = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="w",
            encoding="utf-8",
            prefix=".fum-указатель-подмодуля-",
            dir=ожидаемый_каталог.parent,
            delete=False,
        ) as файл:
            временный_файл = Path(файл.name)
            файл.write(f"gitdir: {ожидаемый_каталог}\n")
            файл.flush()
            os.fsync(файл.fileno())
        os.replace(временный_файл, путь_подмодуля / ".git")
        временный_файл = None
    except OSError as ошибка:
        raise ОшибкаПула(
            73,
            "отказ_атомарной_записи_указателя_подмодуля",
            f"{описание.путь}: .git-указатель materialization не записан атомарно.",
            пояснение=str(ошибка),
        ) from ошибка
    finally:
        if временный_файл is not None:
            временный_файл.unlink(missing_ok=True)


def выполнить_локальное_клонирование_подмодуля(
    путь_слота: Path,
    путь_подмодуля: Path,
    ожидаемый_каталог: Path,
    описание: ОписаниеПодмодуля,
    ...
    подготовить_локальный_каталог_подмодуля(
        путь_слота,
        ожидаемый_каталог,
        описание,
    )
    записать_указатель_каталога_системы_версий_подмодуля(
        путь_подмодуля,
        ожидаемый_каталог,
        описание,
    )
...
def перенести_локальные_ссылки_подмодуля(
    путь_подмодуля: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    спецификации_ссылок = [
    ...
    if описание.адрес_исходного_репозитория is not None:
        спецификации_ссылок.append("+refs/remotes/upstream/*:refs/remotes/upstream/*")
    результат = выполнить_команду_версий(
        путь_подмодуля,
        [
            "-c",
    ...
    if результат.код != 0:
        ...
        raise ОшибкаПула(
            73,
            "отказ_локального_переноса_подмодуля",
            f"{описание.путь}: локальный перенос Git-объектов завершился отказом.",
            пояснение=пояснение,
        )


def записать_канонические_удалённые_источники_подмодуля(
    путь_подмодуля: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    ожидаемые = {"origin"}
    if описание.адрес_исходного_репозитория is not None:
        ожидаемые.add("upstream")
    фактические = set(
        текст_команды_версий(путь_подмодуля, ["remote"]).splitlines()
    )
    if not фактические.issubset(ожидаемые) or "origin" not in фактические:
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: переходное состояние содержит посторонний remote.",
        )
    допустимые_адреса_форка = {
        описание.адрес_форка,
        str(описание.каталог_источника_системы_версий),
    }
    адреса_форка = прочитать_локальные_значения(путь_подмодуля, "remote.origin.url")
    if len(адреса_форка) != 1 or адреса_форка[0] not in допустимые_адреса_форка:
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: переходный URL origin неизвестен.",
        )
    if прочитать_локальные_значения(путь_подмодуля, "remote.origin.pushurl"):
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: переходный pushurl origin запрещён.",
        )
    if "upstream" in фактические:
        адреса_исходного_репозитория = прочитать_локальные_значения(
            путь_подмодуля,
            "remote.upstream.url",
        )
        if адреса_исходного_репозитория != [описание.адрес_исходного_репозитория]:
            raise ОшибкаПула(
                73,
                "несовпадение_материализации_подмодуля",
                f"{описание.путь}: переходный URL upstream неизвестен.",
            )
        if прочитать_локальные_значения(путь_подмодуля, "remote.upstream.pushurl"):
            raise ОшибкаПула(
                73,
                "несовпадение_материализации_подмодуля",
                f"{описание.путь}: переходный pushurl upstream запрещён.",
            )
    настройки = [
        ("remote.origin.url", описание.адрес_форка),
        ("remote.origin.fetch", "+refs/heads/*:refs/remotes/origin/*"),
    ]
    if описание.адрес_исходного_репозитория is not None:
        настройки.extend(
            [
                ("remote.upstream.url", описание.адрес_исходного_репозитория),
                ("remote.upstream.fetch", "+refs/heads/*:refs/remotes/upstream/*"),
            ]
        )
    for ключ, значение in настройки:
        выполнить_команду_версий(
            путь_подмодуля,
            ["config", "--local", "--replace-all", ключ, значение],
        )


def является_пустым_переходным_клоном_подмодуля(
    путь_подмодуля: Path,
    описание: ОписаниеПодмодуля,
) -> bool:
    if {запись.name for запись in путь_подмодуля.iterdir()} != {".git"}:
        return False
    фактические = set(
        текст_команды_версий(путь_подмодуля, ["remote"]).splitlines()
    )
    ...
    if not фактические.issubset(ожидаемые) or "origin" not in фактические:
        return False
    адреса_форка = прочитать_локальные_значения(
        путь_подмодуля,
        "remote.origin.url",
    )
    if адреса_форка not in (
        [описание.адрес_форка],
        [str(описание.каталог_источника_системы_версий)],
    ):
        return False
    if прочитать_локальные_значения(путь_подмодуля, "remote.origin.pushurl"):
        return False
    if "upstream" in фактические:
        if прочитать_локальные_значения(
            путь_подмодуля,
            "remote.upstream.url",
        ) != [описание.адрес_исходного_репозитория]:
            return False
        if прочитать_локальные_значения(путь_подмодуля, "remote.upstream.pushurl"):
            return False
    return True
...
def материализовать_подмодуль_слота(
    контекст: КонтекстПула,
    путь_слота: Path,
    каталог_слота_системы_версий: Path,
    описание: ОписаниеПодмодуля,
) -> None:
    части_пути = проверить_имя_или_путь_подмодуля(описание.путь, "Путь")
    части_имени = проверить_имя_или_путь_подмодуля(описание.имя, "Имя")
    путь_подмодуля = потребовать_обычный_путь_внутри(
        путь_слота,
        части_пути,
        f"Путь materialized submodule {описание.путь}",
    )
    ожидаемый_каталог = потребовать_обычный_путь_внутри(
        каталог_слота_системы_версий,
        ("modules", *части_имени),
        f"Git-каталог materialized submodule {описание.имя}",
    )
    метка = путь_подмодуля / ".git"
    новый = not метка.exists() and not метка.is_symlink()
    if новый:
        выполнить_локальное_клонирование_подмодуля(
            путь_слота,
            путь_подмодуля,
            ожидаемый_каталог,
            описание,
    ...
    проверить_автономное_хранилище_объектов(
        ожидаемый_каталог,
        состояние="альтернативное_хранилище_подмодуля_слота",
        описание=описание.путь,
    )
    проверить_отсутствие_частичного_хранилища(
        путь_подмодуля,
        состояние="частичный_подмодуль_слота",
        описание=описание.путь,
    )
    if not новый:
        проверить_топологию_материализованного_подмодуля(
            путь_слота,
            путь_подмодуля,
            ожидаемый_каталог,
            описание,
        ...
        if (
            not рабочее_дерево_чисто(путь_подмодуля)
            and not является_пустым_переходным_клоном_подмодуля(
                путь_подмодуля,
                описание,
            )
        ):
            raise ОшибкаПула(
                73,
                "изменённый_подмодуль_слота",
                f"{описание.путь}: materialized submodule слота изменён.",
            )
    проверить_топологию_материализованного_подмодуля(
        путь_слота,
        путь_подмодуля,
    ...
    потребовать_отсутствие_перенаправлений_локального_источника(
        путь_подмодуля,
        описание,
    )
    перенести_локальные_ссылки_подмодуля(путь_подмодуля, описание)
    записать_канонические_удалённые_источники_подмодуля(путь_подмодуля, описание)
    выполнить_команду_версий(
        путь_подмодуля,
        [
            "-c",
    ...
    if (
        текст_команды_версий(путь_подмодуля, ["rev-parse", "HEAD"]) != описание.ревизия
        or текст_команды_версий(
            путь_подмодуля,
            ["symbolic-ref", "--quiet", "HEAD"],
            проверять=False,
        )
        or not рабочее_дерево_чисто(путь_подмодуля)
    ):
        raise ОшибкаПула(
            73,
            "несовпадение_материализации_подмодуля",
            f"{описание.путь}: финальный readback submodule не совпал.",
        )
    проверить_топологию_материализованного_подмодуля(
        путь_слота,
        путь_подмодуля,
    ...
    проверить_удалённые_источники_подмодуля(
        путь_подмодуля,
        описание,
        источник=False,
    )
    ...
    конфигурация = выполнить_команду_версий(
        путь_подмодуля,
        ["config", "--local", "--null", "--list"],
    ).вывод
    for локальный_путь in (
        описание.путь_источника,
        описание.каталог_источника_системы_версий,
    ):
        if str(локальный_путь).encode("utf-8") in конфигурация:
            raise ОшибкаПула(
                73,
                "локальный_путь_в_конфигурации_подмодуля",
                f"{описание.путь}: локальный источник не должен сохраняться в конфигурации слота.",
            )


def материализовать_подмодули_слота(
    контекст: КонтекстПула,
    путь_слота: Path,
    описания: Sequence[ОписаниеПодмодуля],
) -> None:
    if not описания:
        return
    каталог_слота = Path(
        текст_команды_версий(путь_слота, ["rev-parse", "--absolute-git-dir"])
    ).resolve()
    for описание in описания:
        материализовать_подмодуль_слота(
            контекст,
            путь_слота,
            каталог_слота,
            описание,
...
def прочитать_каталог_материализации_подмодуля(
    путь_подмодуля: Path,
    корень_каталогов: Path,
) -> Path | None:
    метка = путь_подмодуля / ".git"
    if метка.is_symlink() or not метка.is_file():
        return None
    try:
        содержимое = метка.read_text(encoding="utf-8")
    except (OSError, UnicodeError) as ошибка:
        raise ОшибкаПула(
            73,
            "некорректный_остаток_подмодуля",
            "Указатель прежнего submodule недоступен.",
        ) from ошибка
    совпадение = re.fullmatch(r"gitdir: (.+)\n", содержимое)
    if совпадение is None:
        return None
    указанный = Path(совпадение.group(1))
    if not указанный.is_absolute():
        указанный = путь_подмодуля / указанный
    каталог_подмодуля = указанный.resolve()
    try:
        каталог_подмодуля.relative_to(корень_каталогов)
    except ValueError:
        return None
    if (
        каталог_подмодуля == корень_каталогов
        or каталог_подмодуля.is_symlink()
        or not каталог_подмодуля.is_dir()
    ):
        raise ОшибкаПула(
            73,
            "некорректный_остаток_подмодуля",
            "Git-каталог прежнего submodule неканоничен.",
        )
    return каталог_подмодуля


def проверить_чистую_материализацию_для_карантина(
    путь_слота: Path,
    путь_подмодуля: Path,
    ...
    if (
        фактический_корень != путь_подмодуля
        or фактический_каталог != каталог_подмодуля
        or фактический_общий != каталог_подмодуля
        or (
            надрепозиторий
            and Path(надрепозиторий).resolve() != путь_слота
        )
        or not рабочее_дерево_чисто(путь_подмодуля)
    ):
        raise ОшибкаПула(
            73,
            "изменённый_остаток_подмодуля",
            f"{относительный}: прежний materialized submodule нельзя безопасно переместить.",
        )


def переместить_материализацию_в_карантин(
    путь_подмодуля: Path,
    каталог_подмодуля: Path,
    относительный: str,
) -> None:
    карантин = каталог_подмодуля / ".fum-устаревшее-рабочее-дерево"
    if карантин.exists() or карантин.is_symlink():
        raise ОшибкаПула(
            73,
            "некорректный_остаток_подмодуля",
            f"{относительный}: карантин прежнего submodule уже занят.",
        )
    try:
        os.rename(путь_подмодуля, карантин)
    except OSError as ошибка:
        raise ОшибкаПула(
            73,
            "отказ_карантина_устаревшего_подмодуля",
            f"{относительный}: прежняя materialization не перемещена в карантин.",
            пояснение=str(ошибка),
        ) from ошибка


def рабочее_дерево_имеет_только_удаления(
    путь_слота: Path,
    удалённые_пути: set[str],
) -> bool:
    записи = декодировать_нулевые_значения(
        выполнить_команду_версий(
            путь_слота,
        ...
        состояние="некорректное_переходное_состояние_подмодулей",
        назначение="git status переходного worktree",
    )
    return set(записи) == {f" D {путь}" for путь in удалённые_пути}


def прочитать_намерение_переключения(путь: Path) -> dict[str, Any] | None:
    if not os.path.lexists(путь):
        return None
    if путь.is_symlink() or not путь.is_file():
        raise ОшибкаПула(
            73,
            "некорректное_намерение_переключения",
            "Намерение переключения worktree занято не обычным файлом.",
        )
    try:
        данные = путь.read_bytes()
        значение = json.loads(данные)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as ошибка:
        raise ОшибкаПула(
            73,
            "некорректное_намерение_переключения",
            "Намерение переключения worktree не читается.",
        ) from ошибка
    if not isinstance(значение, dict) or канонические_байты(значение) != данные:
        raise ОшибкаПула(
            73,
            "некорректное_намерение_переключения",
            "Намерение переключения worktree неканонично.",
        )
    return значение


def подготовить_намерение_переключения(
    контекст: КонтекстПула,
    каталог_слота: Path,
    назначение: dict[str, Any],
    прежняя_вершина: str,
    ...
    путь = каталог_слота / ИМЯ_НАМЕРЕНИЯ_ПЕРЕКЛЮЧЕНИЯ
    существующее = прочитать_намерение_переключения(путь)
    общие_поля = {
        "схема": "fum.намерение-переключения-worktree.1",
        "хэш_назначения": назначение["assignment_hash"],
        "идентификатор_назначения": назначение["id"],
        "ссылка_ветки": назначение["branch_ref"],
        "целевая_вершина": целевая_вершина,
    }
    for остаток in sorted(каталог_слота.glob(f"{ИМЯ_НАМЕРЕНИЯ_ПЕРЕКЛЮЧЕНИЯ}.*")):
        if остаток.is_symlink() or not остаток.is_file():
            raise ОшибкаПула(
                73,
                "некорректное_намерение_переключения",
                "Переходный остаток намерения имеет неожиданный тип.",
            )
        остаток.unlink()
    if существующее is not None:
        if (
            set(существующее) != {*общие_поля, "исходная_вершина", "исходная_ветка"}
            or any(существующее.get(ключ) != значение for ключ, значение in общие_поля.items())
            or not isinstance(существующее.get("исходная_ветка"), str)
        ):
            raise ОшибкаПула(
                73,
                "намерение_переключения_не_совпало",
                "Намерение переключения worktree принадлежит иной materialization.",
            )
        проверить_идентификатор_объекта(
            контекст,
            str(существующее.get("исходная_вершина")),
        )
        return существующее, True
    намерение = {
        **общие_поля,
        "исходная_вершина": проверить_идентификатор_объекта(контекст, прежняя_вершина),
        "исходная_ветка": прежняя_ветка,
    }
    временный_файл: Path | None = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="wb",
            prefix=f"{ИМЯ_НАМЕРЕНИЯ_ПЕРЕКЛЮЧЕНИЯ}.",
            dir=каталог_слота,
            delete=False,
        ) as файл:
            временный_файл = Path(файл.name)
            файл.write(канонические_байты(намерение))
            файл.flush()
            os.fsync(файл.fileno())
        os.replace(временный_файл, путь)
        временный_файл = None
    except OSError as ошибка:
        raise ОшибкаПула(
            73,
            "отказ_записи_намерения_переключения",
            "Намерение переключения worktree не записано атомарно.",
            пояснение=str(ошибка),
        ) from ошибка
    finally:
        if временный_файл is not None:
            временный_файл.unlink(missing_ok=True)
    return намерение, False


def прочитать_инвентарь_дерева(
    контекст: КонтекстПула,
    вершина: str,
) -> dict[str, tuple[str, str]]:
    вывод = выполнить_команду_версий(
        контекст.основной_корень,
        ["ls-tree", "-r", "-t", "-z", "--full-tree", вершина],
    ).вывод
    инвентарь: dict[str, tuple[str, str]] = {}
    for сырая_запись in вывод.split(b"\0"):
        if not сырая_запись:
            continue
        метаданные, разделитель, сырой_путь = сырая_запись.partition(b"\t")
        try:
            поля = метаданные.decode("ascii", errors="strict").split()
            путь = сырой_путь.decode("utf-8", errors="strict")
        except UnicodeError as ошибка:
            raise ОшибкаПула(
                73,
                "некорректный_инвентарь_переключения",
                "Инвентарь дерева имеет некорректную кодировку.",
            ) from ошибка
        разобранный = PurePosixPath(путь)
        if (
            not разделитель
            or len(поля) != 3
            or поля[0] not in {"040000", "100644", "100755", "120000", "160000"}
            or not re.fullmatch(rf"[0-9a-f]{{{контекст.длина_идентификатора_объекта}}}", поля[2])
            or not путь
            or разобранный.is_absolute()
            or разобранный.as_posix() != путь
            or any(часть in {"", ".", "..", ".git"} for часть in разобранный.parts)
            or путь in инвентарь
        ):
            raise ОшибкаПула(
                73,
                "некорректный_инвентарь_переключения",
                "Инвентарь дерева неканоничен.",
            )
        инвентарь[путь] = (поля[0], поля[2])
    return инвентарь


def путь_совпадает_с_записью_дерева(
    контекст: КонтекстПула,
    путь: Path,
    запись: tuple[str, str] | None,
) -> bool:
    if запись is None:
        return not os.path.lexists(путь)
    режим, идентификатор = запись
    if режим == "040000":
        return путь.is_dir() and not путь.is_symlink()
    if режим == "160000":
        return not os.path.lexists(путь) or (путь.is_dir() and not путь.is_symlink())
    if режим == "120000":
        if not путь.is_symlink():
            return False
        данные = os.fsencode(os.readlink(путь))
    else:
        if путь.is_symlink() or not путь.is_file():
            return False
        if bool(путь.stat().st_mode & 0o111) != (режим == "100755"):
            return False
        данные = путь.read_bytes()
    фактический = выполнить_команду_версий(
        контекст.основной_корень,
        ["hash-object", "--stdin"],
        ввод=данные,
    ).вывод.decode("ascii", errors="strict").strip()
    return фактический == идентификатор


def проверить_восстанавливаемое_переключение(
    контекст: КонтекстПула,
    путь_слота: Path,
    намерение: dict[str, Any],
) -> tuple[dict[str, tuple[str, str]], dict[str, tuple[str, str]]]:
    прежняя_вершина = str(намерение["исходная_вершина"])
    целевая_вершина = str(намерение["целевая_вершина"])
    текущая_вершина = текст_команды_версий(путь_слота, ["rev-parse", "HEAD"])
    ...
    if (
        текущая_вершина not in {прежняя_вершина, целевая_вершина}
        or текущая_ветка not in {
            намерение["исходная_ветка"],
            намерение["ссылка_ветки"],
        }
    ):
        raise ОшибкаПула(
            73,
            "невосстанавливаемое_переключение",
            "HEAD worktree вышел за границы точного намерения переключения.",
        )
    индекс_совпал = False
    for вершина in (прежняя_вершина, целевая_вершина):
        результат = выполнить_команду_версий(
            путь_слота,
            ["diff-index", "--cached", "--quiet", вершина, "--"],
            проверять=False,
        )
        if результат.код == 0:
            индекс_совпал = True
            break
        if результат.код != 1:
            raise ОшибкаПула(73, "невосстанавливаемое_переключение", "Индекс worktree не читается.")
    if not индекс_совпал or выполнить_команду_версий(путь_слота, ["ls-files", "-u", "-z"]).вывод:
        raise ОшибкаПула(73, "невосстанавливаемое_переключение", "Индекс worktree не совпал с одной из ограждённых вершин.")
    прежний_инвентарь = прочитать_инвентарь_дерева(контекст, прежняя_вершина)
    целевой_инвентарь = прочитать_инвентарь_дерева(контекст, целевая_вершина)
    for относительный in sorted(set(прежний_инвентарь) | set(целевой_инвентарь)):
        путь = путь_слота.joinpath(*PurePosixPath(относительный).parts)
        if not (
            путь_совпадает_с_записью_дерева(контекст, путь, прежний_инвентарь.get(относительный))
            or путь_совпадает_с_записью_дерева(контекст, путь, целевой_инвентарь.get(относительный))
        ):
            raise ОшибкаПула(
                73,
                "невосстанавливаемое_переключение",
                f"{относительный}: содержимое не совпало ни с прежним, ни с целевым деревом.",
            )
    внешние_пути: set[str] = set()
    for аргументы in (
        ["ls-files", "--others", "--exclude-standard", "-z"],
        ["ls-files", "--others", "--ignored", "--exclude-standard", "-z"],
    ):
        внешние_пути.update(
            декодировать_нулевые_значения(
                выполнить_команду_версий(путь_слота, аргументы).вывод,
                состояние="невосстанавливаемое_переключение",
                назначение="внешние пути переходного worktree",
            )
        )
    for относительный in внешние_пути:
        фактический_путь = путь_слота.joinpath(*PurePosixPath(относительный).parts)
        фактический_каталог = фактический_путь.is_dir() and not фактический_путь.is_symlink()
        for целевой_путь, целевая_запись in целевой_инвентарь.items():
            целевой_тип_каталога = целевая_запись[0] in {"040000", "160000"}
            if (
                относительный == целевой_путь
                and not (фактический_каталог and целевой_тип_каталога)
                and not путь_совпадает_с_записью_дерева(контекст, фактический_путь, целевая_запись)
            ) or (
                целевой_путь.startswith(f"{относительный}/") and not фактический_каталог
            ) or (
                относительный.startswith(f"{целевой_путь}/") and not целевой_тип_каталога
            ):
                raise ОшибкаПула(
                    73,
                    "невосстанавливаемое_переключение",
                    f"{относительный}: внешний путь будет затронут целевым checkout.",
                )
    return прежний_инвентарь, целевой_инвентарь


def удалить_точные_остатки_прежнего_дерева(
    контекст: КонтекстПула,
    путь_слота: Path,
    прежний_инвентарь: dict[str, tuple[str, str]],
    целевой_инвентарь: dict[str, tuple[str, str]],
) -> None:
    прежние_пути = sorted(
        (путь for путь in прежний_инвентарь if путь not in целевой_инвентарь),
        key=lambda путь: len(PurePosixPath(путь).parts),
        reverse=True,
    )
    for относительный in прежние_пути:
        запись = прежний_инвентарь[относительный]
        путь = путь_слота.joinpath(*PurePosixPath(относительный).parts)
        if запись[0] == "040000":
            if путь.is_dir() and not путь.is_symlink() and not any(путь.iterdir()):
                путь.rmdir()
        elif запись[0] != "160000" and путь_совпадает_с_записью_дерева(контекст, путь, запись):
            путь.unlink()


def карантинировать_прежние_подмодули_перед_переключением(
    путь_слота: Path,
    прежние: Sequence[ОписаниеПодмодуля],
    новые: Sequence[ОписаниеПодмодуля],
    *,
    переходное_дерево_проверено: bool = False,
) -> set[str]:
    новые_по_пути = {описание.путь: описание for описание in новые}
    устаревшие = [
        описание
        for описание in прежние
        if (
            описание.путь not in новые_по_пути
            or описание.имя != новые_по_пути[описание.путь].имя
            or описание.адрес_форка != новые_по_пути[описание.путь].адрес_форка
            or описание.адрес_исходного_репозитория
            != новые_по_пути[описание.путь].адрес_исходного_репозитория
        )
    ]
    if not устаревшие:
        if not переходное_дерево_проверено and not рабочее_дерево_чисто(путь_слота):
            raise ОшибкаПула(73, "dirty_free_slot", "Свободный слот оказался грязным.")
        return set()
    каталог_слота = Path(
        текст_команды_версий(путь_слота, ["rev-parse", "--absolute-git-dir"])
    ).resolve()
    корень_каталогов = (каталог_слота / "modules").resolve()
    план: list[tuple[ОписаниеПодмодуля, Path, Path, str]] = []
    for описание in устаревшие:
        части_пути = проверить_имя_или_путь_подмодуля(описание.путь, "Путь")
        части_имени = проверить_имя_или_путь_подмодуля(описание.имя, "Имя")
        ожидаемый_каталог = потребовать_обычный_путь_внутри(
            каталог_слота,
            ("modules", *части_имени),
            f"Git-каталог прежнего submodule {описание.имя}",
        )
        карантин = ожидаемый_каталог / ".fum-устаревшее-рабочее-дерево"
        if переходное_дерево_проверено and (карантин.exists() or карантин.is_symlink()):
            путь_подмодуля = путь_слота.joinpath(*части_пути)
        else:
            путь_подмодуля = потребовать_обычный_путь_внутри(
                путь_слота,
                части_пути,
                f"Путь прежнего submodule {описание.путь}",
            )
        if карантин.exists() or карантин.is_symlink():
            if карантин.is_symlink() or not карантин.is_dir():
                raise ОшибкаПула(
                    73,
                    "некорректный_остаток_подмодуля",
                    f"{описание.путь}: карантин прежнего submodule неканоничен.",
                )
            if not переходное_дерево_проверено and os.path.lexists(путь_подмодуля):
                raise ОшибкаПула(
                    73,
                    "изменённое_переходное_состояние_подмодулей",
                    f"{описание.путь}: путь и карантин прежнего submodule существуют одновременно.",
                )
            каталог_маркера = прочитать_каталог_материализации_подмодуля(
                карантин,
                корень_каталогов,
            )
            if каталог_маркера != ожидаемый_каталог:
                raise ОшибкаПула(
                    73,
                    "некорректный_остаток_подмодуля",
                    f"{описание.путь}: карантин указывает на иной Git-каталог.",
                )
            проверить_автономное_хранилище_объектов(
                ожидаемый_каталог,
                состояние="изменённый_остаток_подмодуля",
                описание=описание.путь,
            )
            проверить_чистую_материализацию_для_карантина(
                путь_слота,
                карантин,
                ожидаемый_каталог,
                описание.путь,
            ...
            if (
                текст_команды_версий(карантин, ["rev-parse", "HEAD"])
                != описание.ревизия
                or текст_команды_версий(
                    карантин,
                    ["symbolic-ref", "--quiet", "HEAD"],
                ...
                raise ОшибкаПула(
                    73,
                    "изменённый_остаток_подмодуля",
                    f"{описание.путь}: содержимое карантина прежнего submodule изменено.",
                )
            план.append((описание, путь_подмодуля, ожидаемый_каталог, "карантин"))
            continue
        путь_отсутствует = not os.path.lexists(путь_подмодуля)
        путь_пуст = (
            путь_подмодуля.is_dir()
            and not путь_подмодуля.is_symlink()
            and not any(путь_подмодуля.iterdir())
        )
        каталог_отсутствует = not os.path.lexists(ожидаемый_каталог)
        if каталог_отсутствует and (путь_отсутствует or путь_пуст):
            план.append((описание, путь_подмодуля, ожидаемый_каталог, "пусто"))
            continue
        проверить_автономное_хранилище_объектов(
            ожидаемый_каталог,
            состояние="изменённый_подмодуль_свободного_слота",
            описание=описание.путь,
        )
        проверить_топологию_материализованного_подмодуля(
            путь_слота,
        ...
        if (
            not рабочее_дерево_чисто(путь_подмодуля)
            or текст_команды_версий(путь_подмодуля, ["rev-parse", "HEAD"])
            != описание.ревизия
            or текст_команды_версий(
                путь_подмодуля,
                ["symbolic-ref", "--quiet", "HEAD"],
            ...
            raise ОшибкаПула(
                73,
                "изменённый_подмодуль_свободного_слота",
                f"{описание.путь}: прежний submodule свободного слота изменён.",
            )
        план.append((описание, путь_подмодуля, ожидаемый_каталог, "материализован"))

    уже_в_карантине = {
        описание.путь
        for описание, _путь, _каталог, состояние in план
        if состояние == "карантин"
    }
    if переходное_дерево_проверено:
        pass
    elif уже_в_карантине:
        if not рабочее_дерево_имеет_только_удаления(
            путь_слота,
            уже_в_карантине,
        ):
            raise ОшибкаПула(
                73,
                "изменённое_переходное_состояние_подмодулей",
                "Worktree с карантином прежних submodule содержит иные изменения.",
            )
    elif not рабочее_дерево_чисто(путь_слота):
        raise ОшибкаПула(73, "dirty_free_slot", "Свободный слот оказался грязным.")

    for описание, путь_подмодуля, ожидаемый_каталог, состояние in план:
        if состояние == "карантин":
            continue
        if состояние == "пусто":
            if путь_подмодуля.is_dir():
                путь_подмодуля.rmdir()
        else:
            переместить_материализацию_в_карантин(
                путь_подмодуля,
                ожидаемый_каталог,
                описание.путь,
            )
    удалённые = {описание.путь for описание in устаревшие}
    if not переходное_дерево_проверено and not рабочее_дерево_имеет_только_удаления(путь_слота, удалённые):
        raise ОшибкаПула(
            73,
            "изменённое_переходное_состояние_подмодулей",
            "После карантина прежних submodule обнаружены иные изменения.",
        )
    return удалённые


def удалить_карантины_подмодулей(каталог_слота: Path) -> None:
    корень_карантина = каталог_слота / "карантин-подмодулей"
    if корень_карантина.is_symlink():
        raise ОшибкаПула(73, "некорректный_карантин_подмодулей", "Корень карантина не может быть ссылкой.")
    if корень_карантина.exists():
        if not корень_карантина.is_dir():
            raise ОшибкаПула(73, "некорректный_карантин_подмодулей", "Корень карантина занят не каталогом.")
        for остаток in sorted(корень_карантина.iterdir()):
            if остаток.is_symlink() or not остаток.is_dir():
                raise ОшибкаПула(73, "некорректный_карантин_подмодулей", "Остаток карантина имеет неожиданный тип.")
            shutil.rmtree(остаток)
    корень_каталогов = каталог_слота / "modules"
    отмеченные: list[Path] = []
    переходные_каталоги: list[Path] = []
    переходные_файлы: list[Path] = []
    if корень_каталогов.is_symlink() or (
        корень_каталогов.exists() and not корень_каталогов.is_dir()
    ):
        raise ОшибкаПула(
            73,
            "некорректный_карантин_подмодулей",
            "Корень Git-каталогов submodule неканоничен.",
        )
    if корень_каталогов.is_dir():
        for сырой_корень, каталоги, файлы in os.walk(корень_каталогов, followlinks=False):
            корень = Path(сырой_корень)
            if ".fum-устаревшее-рабочее-дерево" in каталоги:
                отмеченные.append(корень)
                каталоги[:] = []
                continue
            if ".fum-устаревшее-рабочее-дерево" in файлы:
                raise ОшибкаПула(
                    73,
                    "некорректный_карантин_подмодулей",
                    "Метка карантина занята не каталогом.",
                )
            if {"HEAD", "config"}.issubset(файлы) and "objects" in каталоги:
                каталоги[:] = []
                continue
            следующие_каталоги: list[str] = []
            for имя in каталоги:
                остаток = корень / имя
                if имя.startswith(".fum-переходный-подмодуль-"):
                    if остаток.is_symlink() or not остаток.is_dir():
                        raise ОшибкаПула(
                            73,
                            "некорректный_переходный_остаток_подмодуля",
                            "Переходный clone submodule имеет неожиданный тип.",
                        )
                    переходные_каталоги.append(остаток)
                    continue
                if имя.startswith(".fum-указатель-подмодуля-"):
                    raise ОшибкаПула(
                        73,
                        "некорректный_переходный_остаток_подмодуля",
                        "Переходный .git-указатель занят каталогом.",
                    )
                следующие_каталоги.append(имя)
            каталоги[:] = следующие_каталоги
            for имя in файлы:
                остаток = корень / имя
                if имя.startswith(".fum-указатель-подмодуля-"):
                    if остаток.is_symlink() or not остаток.is_file():
                    ...
                    переходные_файлы.append(остаток)
                elif имя.startswith(".fum-переходный-подмодуль-"):
                    raise ОшибкаПула(
                        73,
                        "некорректный_переходный_остаток_подмодуля",
                        "Переходный clone submodule занят файлом.",
                    )
    for остаток in sorted(переходные_файлы):
        остаток.unlink()
    for остаток in sorted(
        переходные_каталоги,
        key=lambda путь: len(путь.parts),
        reverse=True,
    ):
        shutil.rmtree(остаток)
    for каталог_подмодуля in sorted(отмеченные, key=lambda путь: len(путь.parts), reverse=True):
        метка = каталог_подмодуля / ".fum-устаревшее-рабочее-дерево"
        if метка.is_symlink() or not метка.is_dir():
            raise ОшибкаПула(73, "некорректный_карантин_подмодулей", "Метка карантина неканонична.")
        корень_карантина.mkdir(mode=0o700, exist_ok=True)
        переходный = Path(
            tempfile.mkdtemp(prefix="остаток-", dir=корень_карантина)
        )
        переходный.rmdir()
        try:
            os.rename(каталог_подмодуля, переходный)
            shutil.rmtree(переходный)
        except OSError as ошибка:
            raise ОшибкаПула(
                73,
                "отказ_очистки_карантина_подмодулей",
                "Git-каталог прежнего submodule не удалён из канонического места.",
                пояснение=str(ошибка),
            ) from ошибка
    if корень_карантина.exists() and not any(корень_карантина.iterdir()):
        корень_карантина.rmdir()


def удалить_устаревшие_материализации_подмодулей(
    путь_слота: Path,
    описания: Sequence[ОписаниеПодмодуля],
) -> None:
    зарегистрированные = {описание.путь: описание for описание in описания}
    каталог_слота = Path(
        текст_команды_версий(путь_слота, ["rev-parse", "--absolute-git-dir"])
    ).resolve()
    корень_каталогов = (каталог_слота / "modules").resolve()
    кандидаты: list[Path] = []
    for сырой_корень, каталоги, файлы in os.walk(путь_слота, followlinks=False):
        корень = Path(сырой_корень)
        if корень == путь_слота:
            continue
        if ".git" not in файлы:
            continue
        каталоги[:] = []
        кандидаты.append(корень)

    for путь_подмодуля in sorted(кандидаты, key=lambda путь: len(путь.parts), reverse=True):
        относительный = путь_подмодуля.relative_to(путь_слота).as_posix()
        каталог_подмодуля = прочитать_каталог_материализации_подмодуля(
            путь_подмодуля,
            корень_каталогов,
        )
        if каталог_подмодуля is None:
            continue
        зарегистрированное = зарегистрированные.get(относительный)
        if зарегистрированное is not None:
            части_имени = проверить_имя_или_путь_подмодуля(
            ...
            ожидаемый_каталог = потребовать_обычный_путь_внутри(
                каталог_слота,
                ("modules", *части_имени),
                f"Git-каталог зарегистрированного submodule {зарегистрированное.имя}",
            )
            if каталог_подмодуля == ожидаемый_каталог:
                continue
        else:
            отслеживаемое = выполнить_команду_версий(
                путь_слота,
                ["ls-tree", "-r", "-z", "--name-only", "HEAD", "--", относительный],
            ).вывод
            if отслеживаемое:
                raise ОшибкаПула(
                    73,
                    "устаревший_подмодуль_пересекается_с_новой_вершиной",
                    f"{относительный}: прежний submodule пересекается с отслеживаемым путём новой вершины.",
                )
        проверить_чистую_материализацию_для_карантина(
            путь_слота,
            путь_подмодуля,
    ...
    удалить_карантины_подмодулей(каталог_слота)
...
def переключить_слот(
    ...
    описания_подмодулей = описания_подмодулей_вершины(контекст, ожидаемая_вершина)
    создан = False
    if not путь.exists():
        выполнить_команду_версий(
            контекст.основной_корень,
    ...
    путь_намерения = каталог_слота / ИМЯ_НАМЕРЕНИЯ_ПЕРЕКЛЮЧЕНИЯ
    сохранённое_намерение = прочитать_намерение_переключения(путь_намерения)
    намерение: dict[str, Any] | None = None
    прежний_инвентарь: dict[str, tuple[str, str]] | None = None
    целевой_инвентарь: dict[str, tuple[str, str]] | None = None
    требуется_переключение = текущая_ветка != назначение["branch_ref"]
    if текущая_ветка == назначение["branch_ref"]:
        ...
        if сохранённое_намерение is not None:
            намерение, _ = подготовить_намерение_переключения(
                контекст,
                каталог_слота,
                назначение,
                текущая_вершина,
            ...
            прежний_инвентарь, целевой_инвентарь = проверить_восстанавливаемое_переключение(
                контекст,
                путь,
                намерение,
            )
            требуется_переключение = not рабочее_дерево_вне_подмодулей_чисто(путь)
        elif not рабочее_дерево_вне_подмодулей_чисто(путь):
            raise ОшибкаПула(73, "changed_materialized_worktree", "Восстанавливаемый worktree изменён.")
    else:
        if сохранённое_намерение is None and not рабочее_дерево_вне_подмодулей_чисто(путь):
            raise ОшибкаПула(73, "dirty_free_slot", "Свободный слот оказался грязным.")
        намерение, восстановление = подготовить_намерение_переключения(
            контекст,
            каталог_слота,
            назначение,
            текущая_вершина,
        ...
        прежние_описания = описания_подмодулей_вершины(
            контекст,
            str(намерение["исходная_вершина"]),
            проверять_источники=False,
        )
        карантинировать_прежние_подмодули_перед_переключением(
            путь,
            прежние_описания,
            описания_подмодулей,
            переходное_дерево_проверено=восстановление,
        )
    if требуется_переключение:
        if текущая_ветка == назначение["branch_ref"]:
            аргументы_переключения = ["reset", "--hard", ожидаемая_вершина]
        else:
            короткая = назначение["branch_ref"].removeprefix("refs/heads/")
            существующая_вершина = прочитать_ссылку(контекст, назначение["branch_ref"])
            if существующая_вершина is None:
                выбор_ветки = ["-c", короткая, ожидаемая_вершина]
            elif существующая_вершина == ожидаемая_вершина:
                выбор_ветки = [короткая]
            else:
                raise ОшибкаПула(
                    73,
                    "work_ref_moved_during_materialization",
                    "Рабочая ссылка materialization указывает на иную вершину.",
                )
            аргументы_переключения = [
                "switch",
                "--discard-changes",
                "--no-guess",
                "--no-track",
        ...
        if фактическая_ветка != назначение["branch_ref"] or фактическая_вершина != ожидаемая_вершина:
            ...
            raise ОшибкаПула(
                ...
                "Readback переключения слота не совпал.",
                ожидаемая_ветка=назначение["branch_ref"],
                фактическая_ветка=фактическая_ветка,
                фактическая_вершина=фактическая_вершина,
                пояснение=пояснение,
            )
    if прежний_инвентарь is not None and целевой_инвентарь is not None:
        удалить_точные_остатки_прежнего_дерева(
            контекст,
            путь,
            прежний_инвентарь,
            целевой_инвентарь,
        )
    удалить_карантины_подмодулей(
        Path(текст_команды_версий(путь, ["rev-parse", "--absolute-git-dir"])).resolve()
    )
    удалить_устаревшие_материализации_подмодулей(путь, описания_подмодулей)
    материализовать_подмодули_слота(контекст, путь, описания_подмодулей)
    удалить_карантины_подмодулей(
        Path(текст_команды_версий(путь, ["rev-parse", "--absolute-git-dir"])).resolve()
    )
    if not рабочее_дерево_чисто(путь):
        raise ОшибкаПула(
            73,
            "changed_materialized_worktree",
            "После materialization readback worktree слота не чист.",
        )
    ...
    if намерение is not None:
        if прочитать_намерение_переключения(путь_намерения) != намерение:
            raise ОшибкаПула(73, "намерение_переключения_изменено", "Намерение переключения изменилось до readback.")
        try:
            путь_намерения.unlink()
        except OSError as ошибка:
            raise ОшибкаПула(73, "отказ_завершения_переключения", "Намерение переключения не завершено.", пояснение=str(ошибка)) from ошибка
