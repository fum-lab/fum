...
public enum ОшибкаСканирования: Error, Equatable, LocalizedError, Sendable {
  case кореньНеЯвляетсяОбычнымКаталогом(String)
  case превышеноМаксимальноеЧислоПутей(Int)

  public var errorDescription: String? {
    switch self {
    case .кореньНеЯвляетсяОбычнымКаталогом(let путь):
      return "Корень сканирования не является обычным каталогом: \(путь)"
    case .превышеноМаксимальноеЧислоПутей(let предел):
      return "Сканирование остановлено: найдено более \(предел) путей."
    }
  }
}

public struct ПределыСканирования: Sendable {
  public static let стандартные = ПределыСканирования(
    максимальноеЧислоПутей: 200_000,
    максимальноеЧислоДокументов: 20_000,
    максимальнаяГлубина: 128,
    максимальныйОбъёмДокумента: 2 * 1_024 * 1_024,
    максимальныйОбщийОбъём: 64 * 1_024 * 1_024
  )

  public let максимальноеЧислоПутей: Int
  public let максимальноеЧислоДокументов: Int
  public let максимальнаяГлубина: Int
  public let максимальныйОбъёмДокумента: Int
  public let максимальныйОбщийОбъём: Int

  public init(
    максимальноеЧислоПутей: Int,
    максимальноеЧислоДокументов: Int,
    максимальнаяГлубина: Int,
    максимальныйОбъёмДокумента: Int,
    максимальныйОбщийОбъём: Int
  ) {
    precondition(максимальноеЧислоПутей > 0)
    precondition(максимальноеЧислоДокументов > 0)
    precondition(максимальнаяГлубина > 0)
    precondition(максимальныйОбъёмДокумента > 0)
    precondition(максимальныйОбщийОбъём > 0)
    self.максимальноеЧислоПутей = максимальноеЧислоПутей
    self.максимальноеЧислоДокументов = максимальноеЧислоДокументов
    self.максимальнаяГлубина = максимальнаяГлубина
    self.максимальныйОбъёмДокумента = максимальныйОбъёмДокумента
    self.максимальныйОбщийОбъём = максимальныйОбщийОбъём
  }
}
...
private struct КандидатДокумента {
  let ссылка: URL
  let относительныйПуть: String
}
...
public struct СканерРепозитория: Sendable {
  private let пределы: ПределыСканирования

  public init(пределы: ПределыСканирования = .стандартные) {
  ...
  public func сканировать(корень: URL) throws -> СнимокДерева {
    ...
    guard атрибутыКорня[.type] as? FileAttributeType == .typeDirectory else {
      throw ОшибкаСканирования.кореньНеЯвляетсяОбычнымКаталогом(корневаяСсылка.path)
    }

    let найденное = try найтиКандидатов(
      в: корневаяСсылка,
      файловаяСистема: файловаяСистема,
      пределы: пределы
    )
    var кандидаты = найденное.кандидаты.sorted {
      предшествуетПуть($0.относительныйПуть, $1.относительныйПуть)
    }
    var пропущенныеПути = найденное.пропущенныеПути

    if кандидаты.count > пределы.максимальноеЧислоДокументов {
      пропущенныеПути.append(
        contentsOf: кандидаты.dropFirst(пределы.максимальноеЧислоДокументов)
          .map(\.относительныйПуть)
      )
      кандидаты = Array(кандидаты.prefix(пределы.максимальноеЧислоДокументов))
    }

    var документы: [НайденныйДокумент] = []
    var использованныйОбъём = 0
    for кандидат in кандидаты {
      try Task.checkCancellation()
      let оставшийсяОбъём = пределы.максимальныйОбщийОбъём - использованныйОбъём
      let допустимыйОбъём = min(пределы.максимальныйОбъёмДокумента, оставшийсяОбъём)
      guard допустимыйОбъём > 0 else {
        пропущенныеПути.append(кандидат.относительныйПуть)
        continue
      }

      guard
        let данные = try? прочитатьНачалоФайла(
          кандидат.ссылка,
          неБолее: допустимыйОбъём
        ),
        данные.count <= допустимыйОбъём
      else {
        пропущенныеПути.append(кандидат.относительныйПуть)
        continue
      }

      использованныйОбъём += данные.count
      let текст = String(decoding: данные, as: UTF8.self)
      let строки = содержательныеСтроки(текста: текст)
      let название =
        первыйЗаголовок(в: строки)
        ?? кандидат.ссылка.deletingPathExtension().lastPathComponent
      документы.append(
        НайденныйДокумент(
          относительныйПуть: кандидат.относительныйПуть,
          название: название,
    ...
    пропущенныеПути = Array(Set(пропущенныеПути)).sorted(by: предшествуетПуть)
...
private func найтиКандидатов(
  в корень: URL,
  файловаяСистема: FileManager,
  пределы: ПределыСканирования
) throws -> (кандидаты: [КандидатДокумента], пропущенныеПути: [String]) {
  var кандидаты: [КандидатДокумента] = []
  ...
  guard
    let перечислитель = файловаяСистема.enumerator(
      ...
      errorHandler: { ссылка, _ in
        if let путь = относительныйПуть(ссылки: ссылка, от: корень) {
    ...
    throw ОшибкаСканирования.кореньНеЯвляетсяОбычнымКаталогом(корень.path)
  }

  var числоПутей = 0
  while let ссылка = перечислитель.nextObject() as? URL {
    try Task.checkCancellation()
    числоПутей += 1
    guard числоПутей <= пределы.максимальноеЧислоПутей else {
      throw ОшибкаСканирования.превышеноМаксимальноеЧислоПутей(
        пределы.максимальноеЧислоПутей
      )
    }

    guard let путь = относительныйПуть(ссылки: ссылка, от: корень) else {
      перечислитель.skipDescendants()
      continue
    }
    let компонентыПути = путь.split(separator: "/")
    ...
    if тип == .typeDirectory {
      if исключёнКаталог(
        имя: ссылка.lastPathComponent,
        находитсяВКорне: компонентыПути.count == 1
      ) {
        перечислитель.skipDescendants()
        continue
      }
      ...
      if компонентыПути.count >= пределы.максимальнаяГлубина {
        пропущенныеПути.append(путь)
        перечислитель.skipDescendants()
      }
      continue
    ...
    guard компонентыПути.count <= пределы.максимальнаяГлубина else {
      пропущенныеПути.append(путь)
      continue
    }
    ...
    кандидаты.append(КандидатДокумента(ссылка: ссылка, относительныйПуть: путь))
  }

  return (кандидаты, пропущенныеПути)
}

private func относительныйПуть(ссылки: URL, от корень: URL) -> String? {
  let компонентыКорня = корень.standardizedFileURL.pathComponents
  let компонентыСсылки = ссылки.standardizedFileURL.pathComponents
  guard
  ...
  return компонентыСсылки.dropFirst(компонентыКорня.count).joined(separator: "/")
}

private func прочитатьНачалоФайла(_ ссылка: URL, неБолее предел: Int) throws -> Data {
  let файл = try FileHandle(forReadingFrom: ссылка)
  defer { try? файл.close() }
  let размерЧтения = предел == Int.max ? Int.max : предел + 1
  return try файл.read(upToCount: размерЧтения) ?? Data()
}
...
private func содержитМаркерРепозитория(каталог: URL, файловаяСистема: FileManager) -> Bool {
  файловаяСистема.fileExists(
    atPath: каталог.appendingPathComponent(".git", isDirectory: false).path
  )
}
...
private func содержательныеСтроки(текста: String) -> [String] {
  ...
  for исходнаяСтрока in текста.split(separator: "\n", omittingEmptySubsequences: false) {
    ...
    if let текущееОграждение = ограждение {
      if закрываетОграждение(строка, ограждение: текущееОграждение) {
        ограждение = nil
      }
      continue
    }
    ...
    if строкаДляМаркера == "<!-- FUM-MD-RECENCY:BEGIN -->" {
      внутриБлокаСвежести = true
      continue
    }
    if строкаДляМаркера == "<!-- FUM-MD-RECENCY:END -->" {
