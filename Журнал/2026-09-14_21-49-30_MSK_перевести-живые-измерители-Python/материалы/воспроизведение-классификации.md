# Воспроизведение классификации Python

Три последовательных сценария извлечены из фактически выполненных прямых команд этого этапа. Они используют сохранённые Git-объекты и опубликованные исходники анализатора. Нужен отдельный чистый checkout итогового коммита с доступной историей; команды выполняются из его физического корня. Выходные JSON заменяются в независимой копии, поэтому в исходном дереве закрытого этапа команды не запускаются. Результаты не разрешают изменение общего снимка.

## Глобально сопоставить роли прежнего и нового Python-инвентаризатора на одних байтах
...
## Связать каждое новое наблюдение Python со строками точных исходных Git-объектов
...
## Классифицировать удалённые внешние AST-роли и конечный остаток 22 Python-путей
...
<!-- content-sha256: sha256:de1b1d909967d5e051d8bd2d3019b70e87881a2c9a8a44865057298efb817c2c -->
<!-- FUM-MD-RECENCY:END -->
