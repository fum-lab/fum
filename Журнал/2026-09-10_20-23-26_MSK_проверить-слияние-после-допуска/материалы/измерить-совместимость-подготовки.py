...
def главная():
    ...
    спецификация = importlib.util.spec_from_file_location("фикстуры_подготовки", путь_теста)
    тесты = importlib.util.module_from_spec(спецификация)
    спецификация.loader.exec_module(тесты)
    записи = []
    for пара in range(7):
        ...
        for вариант, модуль, параметры in порядок:
            ...
            try:
                ...
                def собрать(пакет, каталог, граница, **именованные_аргументы):
                    nonlocal сборок
                    сборок += 1
                    граница()
                    каталог.mkdir()
                ...
                with mock.patch.object(модуль, "РЕВИЗИЯ_ЗАВИСИМОСТИ", ревизия), \
                    ...
                    for стадия in ("холодная", "повторная"):
                        ...
                        записи.append({"пара": пара, "вариант": вариант, "стадия": стадия,
                                       ...
                                       "cpu_родителя_ns": time.process_time_ns() - начало_процессорного_времени,
                                       "всего_сборок": сборок})
                        assert сборок == 1
            finally:
                пример.завершить_испытание()
    медианы = {вариант: {стадия: statistics.median(
        запись_замера["wall_ns"] for запись_замера in записи if запись_замера["вариант"] == вариант and запись_замера["стадия"] == стадия)
