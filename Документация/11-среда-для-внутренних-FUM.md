...
## Fork-агент как исполняемая модельная среда

[Дочерний fork-агент FUM](../Глоссарий/дочерний-fork-агент-FUM.md) задаёт один инженерный статус внутреннего FUM. Относительно корня он является долговечным агентом, а для выполняемого шага порождает версионированное представление своего состояния как модельную среду с памятью, [контекстной ролью](../Глоссарий/контекстная-роль-FUM-агента.md), инструментами, ограничениями, бюджетами и допустимыми переходами. Смена масштаба не превращает моделируемый результат в внешнее действие и не стирает происхождение.

Репозиторий хранит долговечную линию среды, commit задаёт её конкретный снимок, а отдельная [сессия шага FUM](../Глоссарий/сессия-шага-FUM.md) исполняет один контекстно посильный шаг. Эти уровни не тождественны. Новая сессия восстанавливает работу из точного снимка, назначения и паспортов, не требуя скрытой истории предыдущего чата.

Модельный переход изменяет только представление и кандидаты внутри среды. Git-запись, создание host-задачи, публикация ветки и pull request являются реальными цифровыми эффектами исполняемого fork-агента; перед каждым таким переходом отдельно проверяются полномочия и preflight, а после него — наблюдаемый результат. Уверенность модели сама по себе эти эффекты не выполняет.

Такой статус проясняет только долговечные исполняемые fork-агенты. Другие внутренние FUM по-прежнему могут быть облегчёнными ролями, гипотезами или моделями внешних участников без собственного исполняемого цикла; общий вопрос о типах внутренних FUM остаётся частично открытым.
...
<!-- content-sha256: sha256:20c3c216c1447e1957ab2131f4220a9439a90658ebf3c02b3a655ab6e8cac835 -->
<!-- FUM-MD-RECENCY:END -->
