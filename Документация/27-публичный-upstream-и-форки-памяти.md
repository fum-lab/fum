...
## Автоматическая публикация рабочих сессий

Каждая обычная рабочая сессия с осмысленным diff после локальных проверок и атомарного commit+handoff автоматически предпринимает попытку опубликовать созданный коммит в настроенную GitHub-ветку. До локальной передачи корень сохраняет единственный credential-free HTTPS push URL текущего публикационного remote и отклоняет применимую конфигурацию `url.*.insteadOf` или `url.*.pushInsteadOf`. После ответа `committed` запускается публикатор из самого `new_head`, который отправляет ровно `<new_head>:<branch_ref>` по сохранённому URL. Использование подвижного `HEAD`, сокращённой ветки, remote по имени, wildcard, тегов, локального pre-push hook и force-вариантов запрещено.

Такой порядок не делает GitHub частью локальной транзакции. Если следующая FIFO-сессия уже опубликовала потомка, прежний коммит считается опубликованным по доказанной достижимости и удалённая ветка не откатывается. Расходящаяся история, запрет сервера, отсутствие доступа, тайм-аут или недоступность сети оставляют локальный коммит завершённым, но публикацию — явно отклонённой либо неподтверждённой; автоматизация не применяет `pull`, merge, rebase или force-push. При тайм-ауте вся группа транспортных процессов завершается до контрольного чтения удалённой вершины. No-op-завершение `finish-clean` не создаёт коммит и ничего не отправляет.

Проектируемое задание периодической публикации в [диспетчере автоматизаций FUM](../Глоссарий/диспетчер-автоматизаций-FUM.md) пока не изменяет этот обязательный немедленный путь. Дополнительная сверка, повтор неподтверждённого результата, политика другого репозитория и замена post-handoff-push являются разными контрактами; точный выбор, объект и право эффекта сохраняются в [открытом вопросе о границах периодической публикации](../Вопросы/2026-07-27_15-21-35_MSK_границы-периодической-публикации-ветки.md).

Публикатор не зависит от GitHub в автономных тестах: точный refspec, гонка потомка, отказ и расхождение воспроизводятся на локальном bare-remote с узким тестовым допуском URL-подстановки Git, недоступным из CLI. Фактическая публикация рабочей сессии остаётся внешним действием и подтверждается только ответом отправки либо последующим чтением точной удалённой вершины.
...
<!-- content-sha256: sha256:863f8f55af9a77922c18d9e01d2d0d43c912f3db7b1089227038c419a11d0a92 -->
<!-- FUM-MD-RECENCY:END -->
