...
| Лицензия                   | [ЛИЦЕНЗИЯ.md](../ЛИЦЕНЗИЯ.md) фиксирует CC0 1.0 Universal.                                                                                                                                                                                       |
...
Каждый содержательный этап обычной работы на `master` заканчивается локальными проверками и обычным локальным коммитом. Затем та же задача проверяет результат и продолжает доступный согласованный этап. Новую пишущую задачу запускает пользователь. Коммит сам по себе не разрешает `push` или `publish`.

Публикацию подтверждает отдельный ручной `push`, инициированный пользователем после просмотра локального результата. Его объектом служит точный проверенный commit и его предки; возможные более поздние локальные потомки не получают разрешение автоматически. Такое подтверждение публикации не выдаёт полномочий на подключение провайдера, получение новых секретов, платный доступ, чтение пользовательских данных или иной внешний эффект. Завершение задачи без изменений не создаёт коммит и ничего не отправляет; историческая команда `finish-clean` не входит в действующий ручной маршрут.

Удалённая вершина не участвует в допуске ручной пишущей сессии или готовности локального коммита. Если ручной `push` обнаруживает divergence, автоматизация не применяет `pull`, merge, rebase, force-push или force-with-lease: пользователь отдельно решает расхождение и повторно проверяет точный объект публикации. Проектируемые задания публикации для других refs и репозиториев остаются самостоятельными контрактами в [частично прояснённом вопросе о границах периодической публикации](../Вопросы/2026-07-27_15-21-35_MSK_границы-периодической-публикации-ветки.md).
...
## Отложенная локальная публикационная граница worktree-пула

Следующий профиль сохранён как целевая наработка и не действует для текущего репозитория. Он не разрешает автоматическую публикацию result-ref или `master`.
...
В отложенном профиле [обязательное продолжение ветки](../Глоссарий/обязательное-продолжение-ветки.md) было связано с точным полным ref и физическим checkout: каждый живой пишущий checkout хранил собственную FIFO и цепочку продолжений, а локальные параллельные линии использовали переиспользуемые linked worktree и уникальные полные refs. Этот механизм сохранён как архитектурная наработка и не маршрутизирует текущие записи. Действующая ручная схема использует один первичный checkout `refs/heads/master` и не создаёт продолжение после коммита; самостоятельный репозиторий по-прежнему использует отдельный клон.
...
Штатное GitHub-подключение Web ChatGPT не является таким пишущим fork-контуром: оно читает репозиторий, но не создаёт commit, ветку или pull request. Для небольшого внешнего результата действует [проверяемый приём внешнего вклада](51-проверяемый-приём-внешнего-вклада.md): внешний агент помещает полный типизированный пакет в текст share-диалога, а локальная корневая сессия архивирует источник, проверяет точные базу, manifest, хэш и применимость patch и сама оформляет принятый результат. Для большого результата допустим fork и закреплённый draft pull request, созданный Codex web/cloud или иным отдельно допущенным пишущим адаптером; это меняет транспорт, но не даёт PR права на merge и не обходит локальную приёмку.
...
<!-- content-sha256: sha256:7de46cc97316afce84e2d55f0f38e449b0a952c72a51c39a3cefd3c4665d1a75 -->
<!-- FUM-MD-RECENCY:END -->
