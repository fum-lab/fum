...
Внешняя сессия Codex не является собственным исполняемым [агентским циклом](../Глоссарий/агентский-цикл.md) FUM: модель, создание задач и оркестрация остаются во внешней среде. В действующей ручной схеме пользователь сам запускает каждую пишущую задачу, а Git-память служит носителем причинной непрерывности: локальный результат одной завершённой сессии меняет доступный контекст следующей. Заранее создаваемое [обязательное продолжение ветки](../Глоссарий/обязательное-продолжение-ветки.md), FIFO, повторный выбор шага и атомарная передача сохранены как отложенный поведенческий прототип, а не как текущий маршрут записи.

Пользовательская задача входит в этот контур как новый [наблюдаемый входной сигнал](../Глоссарий/наблюдаемый-входной-сигнал.md). Изменяя требования или память, она может перенаправить последующую наблюдаемую цель, приоритет, план, действие или проверку; одновременной второй пишущей задачи действующая схема не допускает. Коробочный runtime должен уменьшить гранулярность этой связи от отправленных сообщений к разрешённому потоку событий во время активного цикла; это не требует непрерывного inference или сохранения скрытых рассуждений.
...
<!-- content-sha256: sha256:46ae2a485a0160d13a6cc66a1213110cc60c1f1e96c02cd961f651cf75e2b710 -->
<!-- FUM-MD-RECENCY:END -->
