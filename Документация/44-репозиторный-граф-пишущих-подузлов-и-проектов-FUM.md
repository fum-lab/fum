...
Один локальный владелец удерживает advisory-блокировку внутри физического целевого bare-репозитория по полному ref. Обычное Git-слияние не использует resolver. Интеграционный commit явно получает прежнюю вершину и каждый принятый кандидат прямыми родителями, удерживается отдельным служебным ref и связывается с кандидатами и проверками каноническим паспортом до передачи объектов. Единственное изменение цели выполняется транзакцией `update-ref` с `option no-deref`, точными старым и новым OID и compare-and-swap. Движение цели, текстовый конфликт, повреждение, провал проверки и проигранный CAS не меняют целевой ref; потерянный ответ после CAS и точный повтор восстанавливаются из подготовленного паспорта и квитанции с повторной проверкой дерева.

Прототип не создаёт межрепозиторную интеграционную очередь, внешний fork, проектный репозиторий или GitHub-ресурс, не запускает модель и сеть и не обновляет настоящий gitlink. Он также не запускает несколько пишущих исполнителей и не разрешает конфликтующие деревья: ограниченный resolver, долговечная передача, проектный gitlink и сквозная приёмка остаются следующими отдельными шагами.
...
<!-- content-sha256: sha256:ef409650b0989d39f58caf78cfb450f1c25e0194d36159e31c983ac3fd5f570f -->
<!-- FUM-MD-RECENCY:END -->
