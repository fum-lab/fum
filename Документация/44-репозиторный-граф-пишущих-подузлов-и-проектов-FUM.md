...
Эта архитектура описывает целевой репозиторный граф. Автономные временные bare-репозитории уже проверяют один долговечный fork-подузел, точный submodule и передачу вверх, но не объявляют созданными внешние долговечные ресурсы, полную проектную композицию или распределённый исполнительный runtime.
...
Этот результат не создаёт GitHub fork, не обращается к сети, не выдаёт локальные bare-репозитории за внешнюю публикацию и не запускает несколько пишущих исполнителей. Самостоятельный проектный репозиторий, межрепозиторная интеграционная очередь и сквозная автономная приёмка полной композиции остаются отдельными шагами. Реестр resolver по-прежнему ограничен двумя зарегистрированными классами и не является универсальным.
...
<!-- content-sha256: sha256:bd9a96c0c3d36ccb4bd27b98d87e2813aa53646a357371127bcc9e5c473ba3c0 -->
<!-- FUM-MD-RECENCY:END -->
