...
Долговечный подузел является не процессом и не веткой родительского checkout, а отдельным fork-репозиторием ядра. Он имеет устойчивый идентификатор, профиль способностей, память, разрешения, проверки и историю. Его основная ветка может жить дольше отдельных модельных сессий, а новые сессии восстанавливают работу из сохранённых артефактов, не полагаясь на скрытую историю чата.

Профиль способностей может быть специализированным либо [универсальным](../Глоссарий/универсальный-исполнительный-подузел-FUM.md). Специализированный подузел заранее ограничен одним классом задач. Универсальный сохраняет общий контур родителя — чтение памяти, планирование, работу с инструментами, декомпозицию, проверку, передачу и при отдельном разрешении дальнейшую делегацию, — но не наследует неограниченные права родителя. Конкретное назначение всё равно задаёт конечные границы области, доступа, внешних эффектов, ресурсов и глубины рекурсии, а также критерий остановки.
...
Корневой FUM управляет долговечным исполнительным подузлом через версионированное назначение, а не через неограниченное свободное текстовое поручение. Назначение закрепляет идентичности родителя, ребёнка и репозитория, точный исходный commit, полный рабочий ref и полный целевой ref, конечный упорядоченный список карточек, область записи и исключения, доступ и внешние эффекты, бюджеты времени и ресурсов, предельные значения параллелизма и глубины рекурсии, обязательные проверки, состояния остановки и возобновления, форму результата и критерии независимого ревью и интеграции.

Дочерний узел может получить целую конечную линейную цепочку, но не превращает её в один бесконтрольный пишущий ход. Каждая карточка становится отдельной контекстно посильной попыткой: она входит в очередь собственного физического checkout, начинает работу от текущей вершины цепочки в отдельном клоне и временном ref и создаёт проверяемый commit либо типизированный отрицательный результат. Дочерний интегратор сериализованно продвигает единственный рабочий ref точным CAS только на непосредственного однородительского потомка прежней вершины. Merge-коммиты внутри цепочки запрещены. Совокупный паспорт поэтому связывает полный линейный диапазон от исходного commit до вершины, порядок шагов, частные паспорта, фактическую область изменений, бюджеты и проверки. Расширение цели, области или полномочий требует нового поколения назначения.

Жизненный цикл разделён на независимо возобновляемые границы:

1. Корень проверяет регистрацию ребёнка и принятый gitlink, создаёт уникальный рабочий ref цепочки от точного исходного commit и отдельный живой клон для первой попытки; целевой ref остаётся неизменным.
2. Отдельная дочерняя корневая задача связывает собственную идентичность и очередь с точным назначением; обычный субагент общего checkout эту роль не получает.
3. Ребёнок выполняет каждый шаг в отдельной изолированной попытке, точным CAS рабочего ref цепочки продвигает принятую вершину и сохраняет линейный паспорт в своём репозитории, а корень наблюдает только версионированное состояние и не удерживает собственное FIFO-поколение на время всей цепочки.
4. После готовности отдельная проверяющая задача в чистом клоне закрепляет точную вершину, повторяет машинные проверки, проводит смысловое ревью и возвращает одно из состояний принятия, доработки, отклонения или неопределённости.
5. Только принятую вершину отдельный интегратор сопоставляет со свежим целевым ref, повторно проверяет, интегрирует точным CAS и сохраняет квитанцию; затем отдельный родительский commit обновляет gitlink композиционной сборки.

Дочерний целевой ref и родительский gitlink принадлежат разным репозиториям, поэтому общей атомарной Git-транзакции между ними нет. Если первый CAS уже успешен, а обновление gitlink ещё не завершилось, протокол сохраняет именованное состояние «принято в ребёнке, обновление gitlink ожидается» с точными OID и квитанцией. Повтор не откатывает принятый дочерний commit и не угадывает результат, а идемпотентно продолжает родительский переход от последней доказанной границы.
...
Паспорт проекта разделён на человекочитаемый корневой `README.md` и связанный закрытый `Паспорт-проекта.json`. Машинная часть закрепляет цель, идентичности, устойчивый адрес репозитория, полный рабочий ref, доступ, публикационную границу, пути собственных правил, очереди, диспетчера, рабочего набора и карточки, хэш рабочего набора, источники, проверки и условие завершения. Она не содержит OID коммита, в который сама входит, или gitlink на проект: такой self-OID создал бы циклическую зависимость содержимого от собственного хэша.

Родительская регистрация хранит только вид `project`, идентичности, адрес и полный ref репозитория, путь submodule, точный gitlink, доступ, публикационную границу, проверки и маршрут передачи. Она не копирует цель, задачу, правила, очередь, claim, диспетчер, карточку или рабочий набор. Благодаря этой границе родитель может проверить форму регистрации и закреплённую ревизию, не присваивая внутреннее управляющее состояние проекта.

Проектный `AGENTS.md`, очередь, claim и диспетчер исполняются относительно физического checkout дочернего репозитория. Служебные refs вычисляются из его Git common-dir, не публикуются в bare-репозиторий и не переносятся в новый клон. Отдельные допуск, claim или наблюдаемый простой родителя не являются доказательством состояния проекта.
...
Краткоживущий пишущий исполнитель получает один контекстно ограниченный рабочий пакет внутри назначенной цепочки с точными идентификаторами подузла, проекта, цепочки и шага, целевым репозиторием, базовым commit, временным ref попытки, рабочим ref цепочки, целевым ref интеграции, разрешёнными путями, входами, проверками и формой передачи. Все три полных ref попарно различны; совпадение закрывает попытку до первой записи.

Для каждой попытки, в том числе последовательной, создаётся отдельный одноразовый клон целевого репозитория и уникальный для этой попытки временный ref вида `refs/heads/step/<step-id>/<attempt-id>` от закреплённого базового commit. Исполнители не меняют общий checkout, индекс или refs друг друга. Worktree допустим как оптимизация только там, где общая объектная база и refs уже защищены отдельным доказанным контрактом; базовой границей изоляции остаётся клон.

Содержательный результат попытки внутри цепочки фиксируется ровно одним непосредственным однородительским commit вместе с наблюдаемым происхождением и проверками. Отрицательный результат, возражение или неустранённый конфликт сохраняются отдельным типизированным паспортом и не двигают рабочий ref, если они дают проверяемую информацию. Скрытые рассуждения модели, машинный шум и секреты не становятся памятью только ради увеличения числа коммитов.
...
1. Пишущий подузел создаёт commit во временном ref попытки и закрепляет паспорт результата с исходным и целевым репозиториями, базовым и результирующим commit, проверками и ограничениями доступа.
2. Commit сохраняется под устойчивым ref результата в источнике, из которого его достижимость можно проверить.
3. Для шага цепочки дочерний интегратор проверяет непосредственное однородительское происхождение и точным CAS продвигает рабочий ref; целевой ref интеграции остаётся неизменным.
4. После готовности и независимого ревью всей цепочки корневой интегратор в отдельном клоне повторяет проверки, выполняет слияние или явное преобразование и точным CAS обновляет целевой ref интеграции.
5. После принятия результата assembly отдельным коммитом обновляет gitlink подузла или проекта.
...
- Длительная дочерняя цепочка не удерживает FIFO-поколение родительского checkout; запуск, наблюдение, ревью и интеграция являются отдельными fenced-задачами, связанными сохранённым назначением.
- Локальная FIFO-очередь одного checkout не выдаётся за координацию отдельных клонов или удалённых refs.
- Очередь, claim и диспетчер проекта используют только физический checkout проекта; управляющее состояние родителя не доказывает их допуск или простой.
...
Слой долговечного fork-подузла выполняет следующий межрепозиторный шаг на автономной локальной топологии. Он регистрирует fork отдельного instance-free ядра с устойчивыми идентичностями, собственными `AGENTS.md`, паспортом специализации, канонической карточкой и рабочим набором следующего шага схемы `5`; живой клон имеет разные `origin` и `upstream`, а assembly хранит только точный gitlink. Штатные сценарии выбора шага и очереди входят в commit fork. Очередь вычисляет служебный ref отдельно из Git-каталога каждого физического checkout: новый клон не наследует service ref, владельца или ожидающие билеты исходного клона. Отдельная попытка продолжает полный ref подузла и публикует кандидатный commit в его bare-репозиторий. После независимого движения ядра явная синхронизация переносит его точную проверенную вершину в fork без автоматического следования за remote. Затем канонический паспорт передачи связывает опубликованный исходный commit, область общего улучшения, проверки, доступ и новую точную родительскую базу; принятие общей пользы сохраняет кандидат в родословной ядра, а отдельный commit assembly обновляет gitlink на синхронизированную вершину fork.

Автономные сценарии восстанавливают чистый detached-снимок подузла из свежего клона assembly и сохранённую ветку со следующим шагом из нового живого клона fork. В живом клоне штатные `validate` и `show` повторно проверяют selector и карточку, а отдельный `join` с немедленным `finish-clean` подтверждает работоспособность собственной очереди физической рабочей копии без переноса билета. Конфликт, несовпавший OID, несовместимая публикационная граница, неверный `upstream`, ссылка submodule на предка и попытка recursive-init закрываются до неожиданного движения refs. Runtime-пути остаются внешним контекстом, а канонические паспорта и отчёты содержат только устойчивые идентичности.

Слой самостоятельного проекта создаёт отдельные временные bare-репозитории проекта и родительской композиции. В дочернем коммите находятся связанные `README.md` и `Паспорт-проекта.json`, собственные `AGENTS.md`, сценарии очереди и диспетчера, карточка и рабочий набор следующего шага. Машинный паспорт не содержит self-OID; родительская запись в `Проекты/регистрации/` хранит композиционный вид `project` и точный gitlink и закрыто проверяется вместе с `.gitmodules` и режимом дерева `160000`.

Живые клоны проекта и родителя независимо выполняют `validate`, `show`, `claim`, `join`, `bind-run`, `verify-run`, `rearm` и чистое завершение. Их queue-ref и claim-ref различаются, а bare-репозитории не получают служебные refs. Затем проектный шаг меняет один разрешённый файл, создаёт единственный непосредственный commit и продвигает только полный проектный ref точным CAS, оставляя родителя неподвижным.
...
<!-- content-sha256: sha256:67ccf739efbb8ca9bb26a0ee6089a2ced3c89307b9c46f80ed760429212c7053 -->
<!-- FUM-MD-RECENCY:END -->
