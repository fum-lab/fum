...
2. Для каждого шага корень создаёт отдельную [сессию шага FUM](../Глоссарий/сессия-шага-FUM.md), которая связывает собственные `CODEX_THREAD_ID`, очередь, живой клон и временный ref с точным назначением; обычный субагент общего checkout эту роль не получает.
3. Ребёнок выполняет шаг в отдельной изолированной сессии, точным CAS рабочего ref цепочки продвигает принятую вершину и сохраняет линейный паспорт в своём репозитории. Следующий зависимый шаг получает новую сессию только от принятой вершины и восстанавливает продолжение из Git и паспортов; корень наблюдает только версионированное состояние и не удерживает собственное FIFO-поколение на время всей цепочки.
...
Корневой FUM взаимодействует с одним экземпляром Codex Desktop как с host-оркестратором этой начальной сборки. Каждому контекстно посильному шагу соответствует отдельная Desktop-задача с собственными идентичностью, клоном и ref; совместимые готовые шаги разных цепочек могут исполняться параллельно. Сессия шага не является fork-агентом и не несёт его долговременную память. Эта привязка описывает наблюдаемый начальный профиль развёртывания: машинно доказываются разные задачи, checkout и refs, а единственность физического Desktop-контроллера требует устойчивой host-идентичности и авторитетного readback.
...
Текущие субагенты Codex разделяют рабочее дерево корневой задачи и поэтому не являются описанными здесь самостоятельными пишущими подузлами. Действующий запрет менять ими Git-индекс, refs и историю остаётся необходимым. Реализованный исполнитель является отдельным детерминированным однопакетным слоем прототипа: он получает заранее подготовленные записи, запрещает модельные и сетевые вызовы и создаёт один непосредственный commit. Именованные fork-репозитории `fum-yadro`, `fum-optimizator` и `fum-pisatelj`, отдельная assembly, реальные pull request и [сессии шагов FUM](../Глоссарий/сессия-шага-FUM.md) этой документационной сессией не создавались. Живая привязка корневой задачи к агенту в отдельном клоне, паспорт конечной цепочки, многокоммитная передача, независимое смысловое ревью, рекурсивные бюджеты и корневое принятие результата ещё не реализованы.
...
<!-- content-sha256: sha256:5af3114bfd519a8ca8189f3958f42bb094f8d36945b45c3ca53988fbbadd4d53 -->
<!-- FUM-MD-RECENCY:END -->
