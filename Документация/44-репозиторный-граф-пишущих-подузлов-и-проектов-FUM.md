...
Эта архитектура описывает целевой репозиторный граф и [репозиторную композицию FUM](../Глоссарий/репозиторная-композиция-FUM.md). Автономные временные bare-репозитории уже проверяют один долговечный fork-подузел, отдельный самостоятельный проект, точные submodule и две формы обновления родительского gitlink, но не объявляют созданными внешние долговечные ресурсы, реальный проектный submodule, полную совместную приёмку композиции или распределённый исполнительный runtime.
...
Этот результат не создаёт GitHub fork, внешний проект или remote, не обращается к сети, не выдаёт локальные bare-репозитории за внешнюю публикацию и не регистрирует реальный проектный submodule в текущей памяти FUM. Межрепозиторная интеграционная очередь, несколько пишущих исполнителей и сквозная автономная приёмка полной композиции FUM-STEP-0090 остаются отдельными шагами. Реестр resolver по-прежнему ограничен двумя зарегистрированными классами, не разрешает gitlink-конфликты и не является универсальным.
...
<!-- content-sha256: sha256:26d9eadc3ae539023f03c762c7c7a6665a1be78cd83ed3ee57ea3b34b2b445d2 -->
<!-- FUM-MD-RECENCY:END -->
