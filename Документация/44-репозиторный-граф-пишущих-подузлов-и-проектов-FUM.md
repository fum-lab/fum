...
[Проверяемый многоагентный контур](../Прототипы/проверяемый-многоагентный-контур/README.md) закрепляет первый исполняемый слой этой границы: закрытая схема паспорта репозиторной композиции версии `1` различает эфемерную ветку шага, долговечный специализированный подузел и самостоятельный проект, а автономный валидатор сверяет точные Git OID, gitlink и `.gitmodules` родительского снимка, полный живой ref, нерекурсивно восстановленный чистый detached-снимок, отдельный Git common-dir пишущего клона, доступ и наблюдаемую ацикличность дочерних деревьев только на локальных bare-репозиториях. Паспорт хранит устойчивые URI-идентичности; временные пути доказательной среды передаются отдельным runtime-контекстом и не становятся его содержимым.

Этот слой ещё не является пишущим исполнителем и не создаёт кандидатный commit, межрепозиторную интеграционную очередь, внешний fork, проектный репозиторий или GitHub-ресурс. Восстановление многофазной передачи, исполнение рабочего пакета, сериализованное принятие результата и обновление настоящего gitlink остаются отдельными следующими шагами.
...
<!-- content-sha256: sha256:27995d779f47317670813130a60b99e913402c2cfe605b47c0e676207edf2446 -->
<!-- FUM-MD-RECENCY:END -->
