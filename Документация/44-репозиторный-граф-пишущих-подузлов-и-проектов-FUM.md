...
[FUM](../Глоссарий/FUM.md) должен сохранять содержательную работу универсальных исполнителей с разными контекстными ролями не только в сообщениях одной сессии, но и в проверяемых Git-коммитах. Для этого долговечные [подузлы](../Глоссарий/подузел-FUM.md) и проекты получают собственные репозитории. Независимые параллельные пишущие кандидаты работают в изолированных клонах и передают результат через точные объекты Git, а последовательные владельцы одной линейной цепочки передают друг другу один живой checkout и точный полный рабочий ref.
...
1. Корень проверяет регистрацию ребёнка и принятый gitlink, создаёт уникальный рабочий ref цепочки от точного исходного commit и один отдельный живой клон для всей цепочки; целевой ref остаётся неизменным.
...
Корневой FUM взаимодействует с одним экземпляром Codex Desktop как с host-оркестратором этой начальной сборки. Каждой цепочке соответствует один отдельный живой клон и точный полный рабочий ref, а каждому её контекстно посильному шагу — отдельная Desktop-задача; совместимые цепочки могут исполняться параллельно. Корень создаёт только начальную задачу цепочки, а каждая коммитящая задача заранее создаёт [продолжение](../Глоссарий/обязательное-продолжение-ветки.md) того же точного ref в том же физическом checkout; общий активный диспетчер в этой передаче не участвует. Сессия шага не является fork-агентом и не несёт его долговременную память. Эта привязка описывает наблюдаемый начальный профиль развёртывания: машинно доказываются разные задачи, checkout и refs, а единственность физического Desktop-контроллера требует устойчивой host-идентичности и авторитетного readback.
...
Краткоживущий пишущий исполнитель получает один контекстно ограниченный рабочий пакет внутри назначенной цепочки с точными идентификаторами подузла, проекта, цепочки и шага, целевым репозиторием, фактическим базовым commit, физическим живым checkout, точным полным рабочим ref цепочки, отличным от него целевым ref интеграции, разрешёнными путями, входами, проверками и формой передачи. Все последовательные задачи одной цепочки используют этот checkout и рабочий ref и сменяют друг друга только через строгую branch-scoped FIFO и обязательное продолжение.

Отдельный одноразовый клон целевого репозитория и уникальный временный ref вида `refs/heads/step/<step-id>/<attempt-id>` сохраняются только для независимого параллельного кандидата вне последовательного владения линейной веткой. Разные такие кандидаты не меняют checkout, индекс или refs друг друга. Внутри линейной цепочки новый клон, worktree или временный ref на каждый шаг запрещены: они разорвали бы доказанную связь продолжения с тем же физическим checkout и точным рабочим ref.
...
- Независимые параллельные писатели используют разные клоны и уникальные refs; последовательные владельцы одной линейной цепочки используют один живой checkout и точный рабочий ref, а принятие в одну целевую ветку сериализуется.
...
- Длительная дочерняя цепочка не удерживает FIFO-поколение родительского checkout: корень создаёт только её начальную host-задачу, а последующие владельцы одного живого checkout и точного рабочего ref создаются обязательными продолжениями; наблюдение, ревью и интеграция являются отдельными fenced-задачами вне последовательного владения веткой и связаны сохранённым назначением.
...
Слой долговечного fork-подузла выполняет следующий межрепозиторный шаг на автономной локальной топологии. Он регистрирует fork отдельного instance-free ядра с устойчивыми идентичностями, собственными `AGENTS.md`, паспортом специализации, канонической карточкой и рабочим набором следующего шага схемы `5`; живой клон имеет разные `origin` и `upstream`, а assembly хранит только точный gitlink. Штатные сценарии выбора шага и очереди входят в commit fork. Очередь вычисляет служебный ref отдельно из Git-каталога каждого физического checkout: новый клон не наследует service ref, владельца или ожидающие билеты исходного клона. Обязательная задача-продолжение продолжает точный полный ref подузла в том же checkout, а отдельный маршрут публикации передаёт кандидатный commit в его bare-репозиторий. После независимого движения ядра явная синхронизация переносит его точную проверенную вершину в fork без автоматического следования за remote. Затем канонический паспорт передачи связывает опубликованный исходный commit, область общего улучшения, проверки, доступ и новую точную родительскую базу; принятие общей пользы сохраняет кандидат в родословной ядра, а отдельный commit assembly обновляет gitlink на синхронизированную вершину fork.
...
<!-- content-sha256: sha256:4ef753ca35a8f45eb323169aee24277ce291a708f48c3252c058c0b47da69aab -->
<!-- FUM-MD-RECENCY:END -->
