...
Одна линия допускает ровно одного владельца, но может последовательно пройти несколько сессий. Перед коммитом текущего владельца продолжение уже закреплено ожидающим FIFO-билетом. До создания result- либо handoff-объекта отдельная CAS записывает в assignment неизменяемое намерение с логическим ключом попытки, exact task и поколением, ref, родителем, деревом, хэшем сообщения, Git-идентичностями и реальными UTC-датами. Ключ не зависит от этих изменяемых входов: повтор с подменёнными деревом или сообщением закрывается mismatch, а точный recovery использует те же байты и воспроизводит тот же OID. Следующая CAS-транзакция сверяет исходную вершину, ref, очередь и intent, передаёт линию голове FIFO и сохраняет связанную квитанцию. Result-intent одновременно становится барьером позднего `join`. Новые handoff `.3` и result `.3` обязательно связываются с intent; прежние `.2` остаются неизменяемыми recovery-данными. Получатель видит `reload_required`, перечитывает правила из фактического нового `HEAD`, подтверждает точный OID и лишь затем получает новое поколение. Точный повтор и read-only-восстановление потерянного ответа возвращают прежнюю квитанцию или маршрут, не создавая второй слот, билет, коммит либо handoff.
...
Исправленная политика времени проверяется до сохранения интеграционного плана и заново перед каждым внутренним чистым либо конфликтным merge, recovery подготовленного merge, конфликтным commit, заморозкой кандидата и движением target. Барьер переснимает все канонические live refs пула, а не только выбранные results, и требует completed migration epoch `.3`, нулевой достижимый остаток raw-даты `946684800 +0000` и отсутствие stale OID-зависимостей. Обычный result `.3` доказывается exact intent и raw objects всего диапазона, migrated `.4` — неизменяемой supersession-цепью от прежней квитанции до old/new commits. Дата каждого legacy seed принимается только при совпадении exact branch reflog с секундой успешного `.jsonl`-ответа того же task, call, OID и receipt; время начала вызова, mtime и тема commit не заменяют доказательство.

Миграция начинается сухим планом и необратимой CAS-блокировкой singleton pool `.2 → .3`. Она либо проигрывает успевшему legacy writer и переснимает его новый commit, либо выигрывает и тем самым заставляет старую final-CAS отказать до движения ветки. Под lock commits переписываются топологически: у дефектных seed меняются только author/committer dates, у их уже корректно датированных потомков — только mapped parents; деревья, сообщения, идентичности и дополнительные заголовки сохраняются. Одна финальная Git-транзакция создаёт archive refs прежних OID, неизменяемую migration receipt и двигает затронутые branch-, pool- и queue-refs. Assignment identity и старые receipts не переписываются. FIFO-состав, `seq`, `next_seq`, generations, continuation hashes, порядок и приоритеты остаются прежними. Единственный C0 сохраняет этот lock и первый ordinary ticket, использует одноразовый receipt-bound новый bridge, а не понижение к старому target-протоколу; затем все bridge-вызовы снова загружаются только из нового target `HEAD`.
...
- append-only commit intent до каждого result-, handoff-, чистого и конфликтного merge-объекта с точными входами, Git-идентичностями, реальными датами и fail-closed replay mismatch;
...
<!-- content-sha256: sha256:6e4e2a9f3b469c5a168534165888f4fa256a2aa63f05f1373c7b05c123fad99e -->
<!-- FUM-MD-RECENCY:END -->
