...
Текущий документационный прототип связывает поколения через обязательное продолжение Git-ветки. Завершающаяся коммитом корневая задача сначала создаёт ровно одного ребёнка в том же сохранённом локальном проекте Codex и для точного полного ref. Точные `threadId` и `hostId` ещё не дают права записи: ребёнок первым инструментальным действием входит в ту же FIFO и ждёт за родителем. Только подтверждённый waiting-билет на исходном `HEAD` разрешает commit+handoff, который одной Git-транзакцией связывает новый commit с идентификатором продолжения и передаёт очередь.
...
<!-- content-sha256: sha256:fbe0a4acafee7b9e739045a931d668771b80395d0a5d3838665c4fd1f6758353 -->
<!-- FUM-MD-RECENCY:END -->
