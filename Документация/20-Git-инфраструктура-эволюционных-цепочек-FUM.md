...
Ближайшая GitHub-схема делает текущий репозиторий публичным upstream, от которого другие люди могут создавать форки для собственных проектов и собственной [памяти FUM](../Глоссарий/память-FUM.md). В этой схеме `master` базового репозитория несёт общую публикационно чистую память, правила, документацию, инструменты и архитектурные улучшения, а форки и отдельные ветки несут локальные проектные линии пользователей.

Периодическое слияние обновляющегося `master` в такие форки становится практическим механизмом наследования. Пользовательская ветка получает новые правила, автоматизации, глоссарные уточнения и архитектурные решения, но сохраняет собственную историю, локальные требования и уровень доступа своих материалов.

Для [FUM](../Глоссарий/FUM.md) эта схема важна как первый социальный слой [эволюционных цепочек](../Глоссарий/эволюционная-цепочка-FUM.md): базовый репозиторий распространяет устойчивые мутации, форки проверяют их в разных проектах, а полезные улучшения могут возвращаться в upstream через pull request или другой передаваемый результат. Поэтому будущий паспорт результата должен уметь различать результат, созданный в базовом репозитории, результат, созданный в форке, и результат, возвращённый обратно.

Практические правила этой схемы - публикационный аудит, рекомендуемые ветки форка, периодическое слияние `master` и обратная передача улучшений - описаны в документе [Публичный upstream и форки памяти FUM](27-публичный-upstream-и-форки-памяти.md).
...
- каталог `Проекты/` с паспортами самостоятельных проектных веток;
- реестр агентов, ролей, компетенций и ограничений доступа;
- каталог задач и входных сигналов;
- по одному выбранному [следующему шагу](../Глоссарий/следующий-шаг-ветки.md) для каждой автоматически развиваемой именованной ветки;
...
Текущий документационный прототип связывает эти элементы пятиминутным диспетчером. Если максимальный поддерживаемый recent-снимок Codex показывает любую другую активную задачу, диспетчер не резервирует шаг, не ставит плановую работу в локальную очередь и вообще не создаёт автоматическую задачу. Только наблюдаемый простой разрешает прочитать рабочий набор активной ветки, проверить ID и хэши всех его [карточек шагов](../Глоссарий/карточка-шага.md), выбрать единственного кандидата `ready`, зарезервировать его версионный `step_id` и после повторной проверки создать отдельную задачу, которая проходит общую последовательную очередь checkout. Кандидаты `paused` и `blocked` сохраняются вместе с условиями возобновления, но не перекрывают готовый шаг. Это ещё не полный [дарвиновский планировщик](../Глоссарий/дарвиновский-планировщик-FUM.md): состав и готовность кандидатов заранее записаны, полезность и бюджет не оптимизируются автоматически, а список задач не сообщает полноту и не доказывает глобальное отсутствие более старой активной задачи. Однако контур уже проверяет важные инварианты — веткозависимый выбор из общего пула, отсутствие повтора одного поколения, происхождение, критерии завершения и передачу следующего решения.
...
<!-- content-sha256: sha256:a77b21cb17fac07ea35a049b12721f256eeef35be1fc4dad8f1ca0b41841e7ba -->
<!-- FUM-MD-RECENCY:END -->
