...
Эксплуатационный статус: конкретные FIFO, continuation, worktree-пул, selector, reviewer/integrator/candidate/CAS и автоматическая публикация, описанные ниже для документационного прототипа, являются исторической и отложенной архитектурой. Они не действуют для текущей записи репозитория; обычную пишущую сессию пользователь запускает вручную в первичном checkout `refs/heads/master`.
...
Предыдущий документационный прототип связывал поколения через обязательное продолжение Git-ветки в двух согласованных профилях. Обычная корневая ветка заранее создавала одного ребёнка в том же сохранённом проекте Codex, физическом checkout и полном ref. Локальная `self_line` заранее регистрировала долговечный билет продолжения в FIFO своего точного назначения; следующая сессия переиспользовала те же слот, ref и worktree. Этот профиль сохранён для истории и будущего проектирования, но не запускает работу после перехода на ручную схему.
...
<!-- content-sha256: sha256:7e32fa82a052b9ffc35f6fff6d690411f4d322f09d25304c13ca77f6be94400c -->
<!-- FUM-MD-RECENCY:END -->
