...
Исправленная политика времени проверяется до сохранения интеграционного плана и заново перед каждым внутренним чистым либо конфликтным merge, recovery подготовленного merge и конфликтным commit. Этот pool-side-барьер требует result `.3` и доказывает по exact intent и raw commit objects весь его линейный диапазон, включая каждый handoff и terminal result. После построения integration candidate target-side bridge повторяет доказательство всех result-диапазонов и merge-intent перед каждым изменяющим target merge и допускает только candidate `.2`. Legacy candidate `.1`, result `.2` и handoff `.2` можно читать и восстанавливать без изменения их байтов, но затронутая ветка не пересекает изменяющую границу до отдельной согласованной миграции. Барьер не меняет порядок существующих FIFO-билетов; единственный C0 вводит dual-reader прежним pinned-протоколом только после полного завершения и проверки исправления.
...
<!-- content-sha256: sha256:7e90fef07a63b499b183d001f03edf00288a0b6b9ad73ae4cce170a44e6b71b8 -->
<!-- FUM-MD-RECENCY:END -->
