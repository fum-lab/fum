...
## Аттестация кандидата и продвижение состояния

Целевой контур должен разделять внутреннюю проверку результата и полномочное принятие общей ветки. Писатель сначала публикует точный кандидатный commit под отдельным неизменяемым ref, не двигая цель. Доверенный проверяющий получает exact OID и исполняет версионированный профиль из защищённого protocol-снимка; его машинная аттестация связывает base/candidate/protocol OID, проверяющего, среду, команды, коды завершения и хэши журналов и артефактов. Исполняющий тесты код кандидата не получает ключ аттестации, push-учётные данные или доступ к общим refs, а свободное утверждение «тесты прошли» либо квитанция того же неограждённого писателя не заменяют машинную связь.

Отдельный продвижитель не должен создавать аттестацию или перепроверять кандидата произвольным набором команд. Он проверяет полномочия проверяющего, подлинность, действительность, область и точные связи готового свидетельства, сравнивает ожидаемую вершину целевого ref и выполняет только old→new CAS. Для удалённого `master` эту границу должен принудительно поддерживать поставщик: обязательный status check или эквивалентная аттестация, запрет прямого и force-push и авторитетный readback правил. Пока это ограждение не реализовано, no-force push является транспортом локально принятого результата, а не серверной гарантией. Целевой контур усиливает уже реализованные локальные ревью, повторную проверку интеграционного кандидата и FIFO/CAS, но не выдаёт общую модель, host или Git common-dir за абсолютную независимость.

Целевой проверочный контур имеет три назначения. Быстрый кандидатный гейт даёт раннюю обратную связь по изменённой границе; полный гейт продвижения создаёт свидетельство принятия exact OID; периодическая глубокая приёмка добавляет полный smoke и дорогие fault-injection-сценарии. Один профиль не подменяет другой, а переход от действующего обязательного полного smoke к многоуровневой политике допустим только после собственной автономной регрессии и обновления нормативных правил.
...
## Локальный пул изолированных Git-слотов исполнения
...
Пока у линии есть ожидающее продолжение, заморозить её итоговый result-ref нельзя. После терминального результата точный commit замораживается под устойчивым локальным ref; лишь отсутствие владельца и ожидающих билетов, протокольная терминализация всех известных кооперативных писателей, чистые checkout и индекс и терминальная квитанция разрешают очистить физический слот и вернуть его в пул. Это не доказывает завершение иных host-процессов или отсутствие OS-эффектов. Рецензент и интегратор читают замороженный Git-объект, поэтому сохранность результата после терминала не зависит от каталога слота.

Локальный отбор состоит из автоматического агентского ревью отдельным назначением, отдельной агентской интеграции и автоматического агентского разрешения допустимых конфликтов. Любой разрешённый или заново собранный итог обязательно проходит повторное ревью иным назначением. Штатный маршрут продвигает принятый неизменный объект в локальный `master` только через сериализованную обычную FIFO основной ветки и точный CAS; это протокольное разделение ролей, а не отдельный корень доверия общего Git common-dir.

Все result-ref, включая заблокированные и несливаемые, остаются отдельно достижимыми. После проверки публикационной чистоты автоматический транспорт отправляет каждый такой ref без force в заранее настроенный remote этого же репозитория и точным readback подтверждает удалённый OID. Ошибка сети или аутентификации даёт возобновляемое состояние `publication_pending`, не удаляет локальный ref и не разрешает угадывать успех. Заблокированный ref публикуется только как отдельный удалённый result-ref и не попадает в `master`; штатный маршрут двигает remote `master` лишь после принятой локальной интеграции, повторного ревью отдельным назначением и обычной передачи основной ветки. До серверного ограждения это не запрещает прямой push с равными полномочиями. GitHub fork и pull request в этом профиле FUM-STEP-0148 не создаются.
...
<!-- content-sha256: sha256:07ebfe3e1f2ac1633487c6e12eaa576193405e76b8e5a9c4a9b9868e41f9a160 -->
<!-- FUM-MD-RECENCY:END -->
