...
Текущий документационный прототип связывает эти элементы пятиминутным диспетчером. Если максимальный поддерживаемый recent-снимок Codex показывает любую другую активную задачу, диспетчер не резервирует шаг, не ставит плановую работу в локальную очередь и вообще не создаёт автоматическую задачу. Только наблюдаемый простой разрешает выбрать запись активной ветки, зарезервировать её устойчивый `step_id` и после повторной проверки создать отдельную задачу, которая проходит общую последовательную очередь checkout. Это ещё не полный [дарвиновский планировщик](../Глоссарий/дарвиновский-планировщик-FUM.md): выбор заранее записан, полезность и бюджет не оптимизируются автоматически, а список не сообщает полноту и не доказывает глобальное отсутствие более старой активной задачи. Однако контур уже проверяет важные инварианты — веткозависимый выбор, отсутствие повтора одного шага, происхождение, критерии завершения и передачу следующего решения.
...
<!-- content-sha256: sha256:d918ccfb6e75cc0dc0f29f4f896b06e3ad58f758581adb5efe068ea2d761aab0 -->
<!-- FUM-MD-RECENCY:END -->
