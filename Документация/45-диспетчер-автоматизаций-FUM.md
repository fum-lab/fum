...
Идентичность запуска задаётся составным ключом `run_key = job_id + spec_generation + trigger_occurrence`. Для карточочного адаптера occurrence включает `step_id`, для интервального задания — точное временное окно, для счётного — конечный диапазон событий, а для ручного — отдельный идентификатор команды. Claim закрепляет `run_key` и свежий `lease_id`; точный повтор одной попытки идемпотентен, другой lease не присваивает занятую occurrence, TTL отсутствует. Успех, безопасный отказ и подтверждённое восстановление должны завершать occurrence fenced-переходом, согласованным с commit+handoff или отдельным проверенным протоколом внешнего эффекта.
...
Действующий [следующий шаг ветки](../Глоссарий/следующий-шаг-ветки.md) становится первым адаптером. Он сохраняет свой рабочий набор, `branch_ref`, `step_id`, хэш карточки, локальный claim, двойную проверку занятости и создание обычной задачи. До миграции действующий heartbeat остаётся специализированным и не заявляет поддержку универсального реестра.
...
<!-- content-sha256: sha256:52eff176e5fbd4e833d24cd953d3b7617291e2c2d8f95c950ecb28473a4b21ab -->
<!-- FUM-MD-RECENCY:END -->
