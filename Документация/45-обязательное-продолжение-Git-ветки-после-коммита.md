...
Описанный ниже FIFO/pool-конвейер сохранён как исторически реализованная и отложенная архитектура. Он не является действующим маршрутом репозитория после перехода на ручную последовательную схему: каждую пишущую сессию запускает пользователь, она фиксирует каждый проверенный этап локальным коммитом на `refs/heads/master` и продолжает согласованную работу в той же задаче без continuation, selector, worktree-пула или автоматической публикации. Возврат этого контура потребует отдельного пользовательского запроса и нового проверенного перехода правил.

[Обязательное продолжение ветки](../Глоссарий/обязательное-продолжение-ветки.md) было контуром последовательного развития уже выбранной именованной Git-линии. До обычного промежуточного коммита [рабочей сессии](../Глоссарий/рабочая-сессия.md) существовал ровно один первый exact-билет продолжения того же полного ref, физического worktree и FIFO. Для worktree-линии его мог заранее зарегистрировать самостоятельно открытый чат; только при отсутствии билета владелец создавал одну задачу-продолжение. Для обычной branch FIFO, включая `master`, владелец всегда заранее создавал отдельную задачу. Терминальные commits результата писателя, ревью и интеграционного кандидата локального пула завершали отдельные назначения собственными квитанциями и обязательного продолжения не создавали.

Этот контур заменял плановый автозапуск внутри последовательной линии. Для её движения не требовались расписание, heartbeat, постоянная прикреплённая задача, общий диспетчер, реестр периодических заданий, диспетчерские резервации или восстановительный тик. Источником следующего запуска служила предшествующая сессия, действительно дошедшая до обычного коммита линии. Чисто завершившаяся сессия и терминальное назначение пула новую задачу-продолжение не создавали.
...
Владелец worktree-линии вызывает `передать-линию` с exact хэшем intent. Одна CAS-транзакция сверяет assignment, поколение, исходную вершину, полный ref и первый билет, создаёт прямой коммит, двигает только ref линии, передаёт её FIFO и обновляет состояние пула. Новый владелец получает `reload_required`, перечитывает правила из закреплённого доверенного `protocol_oid`, сверяет фактические ref, вершину и `worktree_id`, вызывает `подтвердить-вершину-линии` и лишь после `admitted` продолжает то же назначение. Текущий result либо integration candidate не становится источником протокола.
...
Предыдущий контур сохранял две опоры прежней инфраструктуры: FIFO-сериализацию записи и веточный selector. После `manual-sequential-v1` они не вызываются обычной рабочей сессией и остаются исторической либо отложенной наработкой.
...
<!-- content-sha256: sha256:79a24aeaffeaefe0b8077b77d6fad192e333741a52575904177d9c4ae1599eb3 -->
<!-- FUM-MD-RECENCY:END -->
