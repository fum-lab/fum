...
Видимое название задачи Codex является производной интерфейсной проекцией этой идентичности, а не её частью. После допуска и точного выбора карточки продолжение составляет его из `card_id` и заголовка `title` в форме `FUM-STEP-NNNN — <краткое содержательное название>`; назначение без карточки получает краткое русское смысловое название без придуманного номера. Технические имена по ветке, слоту или факту продолжения не должны скрывать выполняемую работу. Изменение заголовка не выдаёт право записи и не заменяет `CODEX_THREAD_ID`, FIFO-допуск или квитанцию маршрута.
...
<!-- content-sha256: sha256:01e9af61fb5686d0738d50774476af1fb431bb5f9be595458aaa28011e48ad00 -->
<!-- FUM-MD-RECENCY:END -->
