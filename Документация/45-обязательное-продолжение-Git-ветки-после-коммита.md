...
Перед любым merge pool-side-барьер переснимает все канонические live refs, требует completed migration epoch `.3`, отсутствие достижимых commits с raw-датой `946684800 +0000` и stale OID-зависимостей и повторяется до плана, каждого чистого либо конфликтного merge, recovery, конфликтного commit, кандидата и движения target. Обычный result `.3` доказывается exact intent всего диапазона, migrated `.4` — неизменяемой supersession-цепью от старой квитанции к old/new raw objects. Дата legacy seed принимается только при совпадении exact branch reflog и секунды успешного `.jsonl`-ответа того же task/call/OID/receipt.

Необратимый pool lock `.2 → .3` останавливает старый протокол до движения refs: проигрыш lock требует нового инвентаря, выигрыш заставляет legacy final-CAS отказать. Мигратор топологически меняет только даты seed и mapped parents потомков, создаёт archive refs и одной финальной CAS двигает затронутые branch-, pool- и queue-refs, не переписывая старые receipts и assignment identity. `task_id`, `seq`, `next_seq`, generation, continuation hash, владелец, FIFO-порядок, приоритет и уже первый ordinary ticket остаются прежними. Переписанная активная линия требует exact protocol reload/ack, а reviewer старого result — нового независимого назначения. Bootstrap C0 не понижает пул и не запускает старый bridge: одноразовый receipt-bound новый bridge сохраняет FIFO, после чего следующие вызовы снова исполняются только из target `HEAD`; схема CAS-integration остаётся `.1`.
...
<!-- content-sha256: sha256:2897515e23e16ab8a2e8653b318ffc2b3d8442e4c6c9f1baa3ebd2e73e867126 -->
<!-- FUM-MD-RECENCY:END -->
