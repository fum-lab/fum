...
def render_document(config: dict[str, Any], output_path: Path, repo_root: Path) -> str:
    ...
    automation_file = config.get(
        "automation_file",
        "Инструменты/fum-doc-aggregation/SKILL.md",
    )
    ...
    lines.extend(
        [
            ...
            f"- Автоматизация: [fum-doc-aggregation]({relative_link(automation_file, output_path, repo_root)})",
            "- Статус: черновой каркас для смысловой сборки агентом.",
