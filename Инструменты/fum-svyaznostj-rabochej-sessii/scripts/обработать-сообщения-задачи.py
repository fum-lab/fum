...
from обработка_сообщений import (ОшибкаОбработки, получить_остаток,
    сохранить_обработку, сохранить_обработку_пакет)
...
def выполнить():
    ...
    пакет = команды.add_parser("сохранить-пакет", help="Проверить и атомарно сохранить JSONL-пакет решений")
    пакет.add_argument("--решения", type=Path, required=True)
    пакет.add_argument("--ожидаемая-история", required=True)
    ...
    try:
        ...
        if параметры.действие == "остаток":
        ...
        elif параметры.действие == "сохранить":
            ...
            байты = параметры.решения.read_bytes()
            if not байты.endswith(b"\n"):
                raise ОшибкаОбработки("пакет решений не завершён переводом строки")
            записи = [_разобрать(строка) for строка in байты.splitlines(keepends=True) if строка.strip()]
            результат = сохранить_обработку_пакет(параметры.исходник, параметры.задача, **общие,
                записи=записи, ожидаемая_история=параметры.ожидаемая_история)
            код = 0
