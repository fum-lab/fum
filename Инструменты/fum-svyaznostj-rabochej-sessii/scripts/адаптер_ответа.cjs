...
async function прочитать_кратко(среда, параметры) {
    ...
    try {
        ...
        const ход = значение => значение === null ? null :
            {идентификатор: значение.id, состояние: значение.status, ошибка: значение.error};
        const краткий = {схема: "fum.краткий-ответ-задачи.1", задача: результат.задача,
            ...
            есть_продолжение: результат.страница.hasMore, ходов_с_ошибкой: результат.ходов_с_ошибкой,
            снимок: {путь: снимок, хэш: запись.sha256},
