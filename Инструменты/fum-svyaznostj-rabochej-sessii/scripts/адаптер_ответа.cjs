...
async function прочитать_кратко(среда, параметры) {
    ...
    try {
        const {корень, задача, режим, снимок, ключ_кэша, максимум_байтов = 16000, профиль_представления = "прежний",
            версия_передачи = "прежняя", передача_пояснений = null, подтверждение_пояснений = null} = параметры;
        if (![корень, задача, снимок, ключ_кэша].every(значение => typeof значение === "string" && значение.length > 0 && !/[\x00-\x1f]/.test(значение)) ||
            !["новый", "сохранённый"].includes(режим) || !["прежний", "порождённый"].includes(профиль_представления) ||
            !["прежняя", "пояснения-1"].includes(версия_передачи) || !Number.isInteger(максимум_байтов) ||
        ...
        // Подтверждение получает только текущий вызов. В кэш снимка оно не записывается.
        const передача = [];
        if (версия_передачи === "пояснения-1") {
            let подтверждение = "null";
            try {
                const значение = JSON.stringify(подтверждение_пояснений);
                if (typeof значение === "string" && значение.length <= 8192) подтверждение = значение;
            } catch (_) { /* Непригодное подтверждение требует полной формы. */ }
            передача.push("--версия-передачи", версия_передачи, "--подтверждение-пояснений", подтверждение,
                "--передача-пояснений", typeof передача_пояснений === "string" && !передача_пояснений.includes("\0") ? передача_пояснений : "");
        }
        const результат = await команда(["python3", "-B", [корень, "Инструменты/fum-svyaznostj-rabochej-sessii/scripts/показать-ответ-задачи.py"].join("/"),
            ...
            "--максимум-байтов", максимум_байтов, "--путь-в-результате", "--профиль", профиль_представления, ...передача]);
