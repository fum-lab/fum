...
from захват_вывода import захватить, представить_захват, прочитать_диапазон, записать
    ...
    try:
        ...
        if параметры.режим == 'запустить':
            ...
            if команда and команда[0] == '--':
                команда = команда[1:]
            захватить(команда, параметры.каталог, параметры.корень, параметры.задача,
                тайм_аут=параметры.тайм_аут, предел_байтов=параметры.предел_байтов,
                обработчик=параметры.обработчик)
            ожидаемый = hashlib.sha256((параметры.каталог/'манифест.json').read_bytes()).hexdigest()
        else:
