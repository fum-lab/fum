...
def выполнить():
    ...
    подготовка_sha = hashlib.sha256(ПОДГОТОВКА.read_bytes()).hexdigest()
    ...
    помощник = str(Path(ФИКСТУРЫ).with_name("исходник_проверки.py"))
    байты_помощника = подготовка.объект(аргументы.источник, "blob", подготовка.найти_объект(аргументы.источник, дерево, помощник))
    ...
    try:
        ...
        for имя, данные in {**исходники, ФИКСТУРЫ: байты_фикстуры, помощник: байты_помощника}.items():
            путь = источник_фикстур / имя
            путь.parent.mkdir(parents=True, exist_ok=True)
            путь.write_bytes(данные)
        sys.path.insert(0, str((источник_фикстур / ФИКСТУРЫ).parent))
        ...
        убрать(временный)
    if подготовка_sha != hashlib.sha256(ПОДГОТОВКА.read_bytes()).hexdigest():
        raise RuntimeError("подготовитель изменился во время проверки комплекта")
    print(json.dumps({"схема": "fum.интеграция-комплекта-Stop.1", "commit": аргументы.commit, "tree": дерево,
                      ...
                      "помощник_фикстуры_sha256": hashlib.sha256(байты_помощника).hexdigest(),
                      "подготовка_sha256": подготовка_sha,
