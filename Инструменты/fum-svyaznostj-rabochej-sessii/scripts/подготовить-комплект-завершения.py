...
ПУТИ += [ПРЕФИКС + "приём_делегации.py"]
ПРЕДЕЛ = 1024 * 1024
...
def конфигурация(настройки):
    поля(настройки, ["интерпретатор", "корень", "cwd", "задача", "состояние", "прогресс", "план", "исходник", "кэш"])
    ...
    for имя in ("исходник", "кэш"):
        значение = настройки[имя]
        if имя == "кэш" and значение is None:
            continue
        кандидат = Path(строка(значение))
        # Содержимое проверяет guard после приоритетной пользовательской остановки.
        требовать(кандидат.is_absolute() and str(кандидат) == значение and ".." not in кандидат.parts,
                  "путь-диалога")
...
def проверить(каталог, хэш, адрес=True):
    ...
    требовать(манифест["схема"] == "fum.комплект-Stop.3", "схема")
    ...
    требовать(type(записи) is list and len(записи) == len(ПУТИ), "инвентарь")
    каталоги = {"."}
    for имя in ПУТИ:
...
def запустить():
    ...
    try:
        ...
        команда = [str(адаптер), "--корень-репозитория", настройки["корень"],
                   ...
                   "--исходник", настройки["исходник"]]
        if настройки["кэш"] is not None:
            команда += ["--кэш", настройки["кэш"]]
...
def подготовить(аргументы):
    ...
    настройки = {"интерпретатор": str(Path(аргументы.интерпретатор).resolve(strict=True)),
                 ...
                 "исходник": аргументы.исходник, "кэш": аргументы.кэш}
    ...
    манифест = {"схема": "fum.комплект-Stop.3", "commit": аргументы.commit, "tree": дерево,
    ...
    кандидат = {"hooks": {"Stop": [{"hooks": [{"type": "command", "command": команда, "timeout": 60}]}]}}
    return {"схема": "fum.кандидат-комплекта-Stop.3", "каталог": str(цель), "sha256": хэш,
...
def выполнить():
    ...
    for имя in ("источник", "commit", "хранилище", "интерпретатор", "корень-репозитория",
                "ожидаемый-cwd", "codex-thread-id", "каталог-состояния", "исходник"):
