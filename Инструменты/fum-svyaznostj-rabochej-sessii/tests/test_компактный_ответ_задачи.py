...
class ПроверкаОтветаЗадачи(unittest.TestCase):
    ...
    def test_цепочка_указателей_проходит_через_строку_JSON(проверка):
    ...
    def test_полный_SHA_и_двойные_ключи_проверяются(проверка):
    ...
    def test_ошибка_MCP_не_превращается_в_успешную_страницу(проверка):
        внешний = оболочка(снимок_ответа()); внешний["isError"] = True; данные = закодировать(внешний)
        with проверка.assertRaises(ValueError): представить_ответ(данные, hashlib.sha256(данные).hexdigest(), ЗАДАЧА)

    def test_типы_метаданных_не_теряются_в_Codable(проверка):
    ...
    def test_CLI_проверяет_снимок_и_не_выдаёт_частичный_ответ(проверка):
