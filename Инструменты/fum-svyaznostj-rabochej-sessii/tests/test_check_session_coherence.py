...
class CheckSessionCoherenceTests(unittest.TestCase):
    ...
    def _ячейки_запуска(себя, запись: dict[str, object]) -> tuple[str, str, str]:
        ...
        if запись["состояние"] == "осиротел":
            return (
                вызов,
                "неизвестно",
                f"{запись['статус']} — {запись['пояснение']}",
            )
    ...
    def _записать_машинный_профиль(
        ...
        *,
        ...
        if состояние == "закрыт":
            себя.assertTrue(
                all(
                    запись["состояние"] in {"завершён", "осиротел"}
                    for запись in записи
                )
            )
            снимок = {
    ...
    def _запись_осиротевшего_запуска(
        себя,
        стем: str,
    ) -> dict[str, object]:
        активная: dict[str, object] = {
            "схема": "fum.test-run.v2",
            "идентификатор": "00000000-0000-4000-8000-000000000003",
            "сессия": f"Журнал/{стем}/запрос.md",
            "порядок": 2,
            "исполнитель": "корень",
            "вызов": "осиротевшая проверка",
            "состояние": "выполняется",
            "длительность_наносекунды": None,
            "статус": None,
            "код_завершения": None,
        ...
        return {
            **активная,
            "схема": "fum.осиротевший-запуск-проверки.1",
            "состояние": "осиротел",
            "статус": "не завершено",
            "пояснение": (
                "осиротевший запуск: управляющая сессия прервана; "
                "длительность и код не наблюдались"
            ),
            "исходная_схема": "fum.test-run.v2",
            "хэш_активной_записи": (
                f"sha256:{hashlib.sha256(активные_байты).hexdigest()}"
            ),
            "явное_признание_риска": True,
        }
    ...
    def test_текущий_журнал_исключает_неизвестную_длительность_осиротевшего_запуска_из_суммы(
        себя,
    ):
        with tempfile.TemporaryDirectory() as временный:
            корень = Path(временный)
            путь_запроса = себя.write_time_profile_journal(
                ...
                invocation_rows=[
                    "| `python3 first.py` | 1,25 с | успешно |",
                    (
                        "| `python3 orphan.py` | неизвестно | "
                        "не завершено — осиротевший запуск: "
                        "управляющая сессия прервана |"
                    ),
                ],
                invocation_total=(
                    "Общее время прямых запусков проверок: 1,25 с."
                ),
            )
    ...
    def test_текущий_журнал_требует_точную_пару_для_неизвестной_длительности_осиротевшего_запуска(
        себя,
    ):
        случаи = (
            (
                "неизвестно",
                "не завершено — нет итоговой записи",
                "journal direct check run row 1 has invalid duration: неизвестно",
            ),
            (
                "1 с",
                "не завершено — осиротевший запуск: есть основание",
                (
                    "journal direct check run row 1 must use unknown duration "
                    "for an orphaned run"
                ),
            ),
        )
        for длительность, результат, ожидаемая_ошибка in случаи:
            with себя.subTest(длительность=длительность), tempfile.TemporaryDirectory() as временный:
                корень = Path(временный)
                путь_запроса = себя.write_time_profile_journal(
                    корень,
                    request_stem=(
                    ...
                    invocation_rows=[
                        f"| `python3 check.py` | {длительность} | {результат} |",
                    ],
                    invocation_total=(
                        "Общее время прямых запусков проверок: "
                        f"{'0' if длительность == 'неизвестно' else '1'} с."
                    ),
                )
                ...
                себя.assertIn(ожидаемая_ошибка, ошибки)
    ...
    def test_закрытый_снимок_допускает_осиротевший_запуск_без_длительности(себя):
        стем = (
            "2026-08-04_20-45-26_MSK_"
            "формировать-отчёты-о-запусках-тестов"
        )
        with tempfile.TemporaryDirectory() as временный:
            корень = Path(временный)
            завершённая = себя._завершённые_записи(стем)[0]
            осиротевшая = себя._запись_осиротевшего_запуска(стем)
            путь_запроса, _, путь_снимка, _ = себя._записать_машинный_профиль(
                корень,
                стем=стем,
                метка=(
                ...
                записи=[завершённая, осиротевшая],
                состояние="закрыт",
            )

            себя.assertIsNotNone(путь_снимка)
            текст_отчёта = (путь_запроса.parent / "отчёт.md").read_text(
                encoding="utf-8"
            )
            себя.assertIn(
                "| [корень] осиротевшая проверка | неизвестно",
                текст_отчёта,
            )
            себя.assertIn(
                "Общее время прямых запусков проверок: 1,25 с.",
