...
class CheckSessionCoherenceTests(unittest.TestCase):
    ...
    def test_граница_машинного_отчёта_сохраняет_исторический_ручной_профиль(себя):
        with tempfile.TemporaryDirectory() as временный:
            ...
            ошибки_контрольной_точки = check_session_coherence.validate_journal(
                корень.resolve(), путь_запроса.resolve(), контрольная_точка=True,
            )
            себя.assertTrue(ошибки_контрольной_точки)
    ...
    def test_контрольная_точка_принимает_только_точный_терминальный_предпросмотр(себя):
        стем = "2026-08-04_20-45-26_MSK_формировать-отчёты-о-запусках-тестов"
        for случай in ("точный", "активная", "искажённый", "снимок", "переход"):
            with себя.subTest(случай=случай), tempfile.TemporaryDirectory() as временный:
                корень = Path(временный)
                записи = (себя._записи_с_активной(стем) if случай == "активная"
                          else себя._завершённые_записи(стем))
                запрос, каталог, _, _ = себя._записать_машинный_профиль(
                    корень, стем=стем,
                    метка="2026-08-04 20:45:26 MSK - Формировать отчёты о запусках тестов",
                    записи=записи, состояние="открыт",
                )
                if случай == "искажённый":
                    отчёт = запрос.parent / "отчёт.md"
                    отчёт.write_text(отчёт.read_text().replace("первый тест", "другая проверка"))
                elif случай == "снимок":
                    (каталог / "снимок.json").write_text("{}\n")
                elif случай == "переход":
                    (каталог / "возобновление.json").write_text("{}\n")
                ошибки = check_session_coherence.validate_journal(
                    корень.resolve(), запрос.resolve(), контрольная_точка=True,
                )
                себя.assertEqual(bool(ошибки), случай != "точный", ошибки)
