...
def collect_repository_snapshot(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "metrics": metrics,
        "notes": [
            "Снимок собран локальной автоматизацией fum-estimates по отслеживаемым Git-файлам.",
...
def render_document(
    ...
    automation_file = config.get("automation_file", "Инструменты/fum-estimates/SKILL.md")
    ...
    lines.extend(
        [
            ...
            f"- [fum-estimates]({relative_link(automation_file, output_path, repo_root)}) - локальная автоматизация сборки и проверки оценочных материалов.",
        ]
    )
