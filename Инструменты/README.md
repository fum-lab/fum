...
Действующий маршрут работы — ручная последовательность: пользователь запускает одну пишущую сессию в первичном checkout `refs/heads/master`, она выполняет согласованные этапы, отдельно проверяет и коммитит каждый из них и продолжает доступную работу в той же задаче. Continuation, FIFO/pool, worktree-писатели, reviewer/integrator/candidate, branch-next-step, heartbeat, dispatcher, autostart и автоматическая публикация не запускаются. Их инструменты ниже сохраняются только как исторические контракты и регрессионная наработка без разрешения живых host- или Git-эффектов.
