...
class RepositoryIntegrationTests(unittest.TestCase):
    ...
    def test_контрольный_и_итоговый_коммиты_не_запускают_исторический_конвейер(это) -> None:
        ...
        это.assertIn("Коммит не создаёт следующую задачу", правила_агентов)
        это.assertIn("Промежуточный коммит постоянной сессии", правила_агентов)
        это.assertIn("После него разрешённая работа продолжается в той же задаче", правила_агентов)
        это.assertIn("Исторические queue/pool/worktree", правила_агентов)
        это.assertIn("не дают активного права записи", правила_агентов)
        это.assertIn("собственное рабочее дерево и отдельную ветку", корневая_инструкция)
