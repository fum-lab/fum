...
class RepositoryIntegrationTests(unittest.TestCase):
    ...
    def test_agents_contract_names_the_portable_fifo_protocol(self) -> None:
        ...
        self.assertIn("Дети соблюдают те же нормы Журнала, TDD, профиля, коммитов и сохранения незавершённого", agents)
        self.assertIn("`./sbrositj.sh`", agents)
        self.assertIn("--идентификатор-продолжения", agents)
        self.assertIn("`create_thread`", agents)
        self.assertIn("FIFO/pool/CAS/branch-next-step не активны.", agents)
        ...
        self.assertIn("иные внешние эффекты требуют отдельного явного запроса", agents)
        self.assertIn("самостоятельным транспортным действием", skill)
        self.assertIn("не входит в причинную цепочку продолжений", skill)

        self.assertIn("FIFO/pool/CAS/branch-next-step не активны.", agents)
        self.assertIn("Чужие файлы, индексы, refs и Git-конфигурация read-only даже при общих Git-объектах.", agents)
        ...
        self.assertIn(
            "Корневая задача заранее создавала одну задачу-продолжение",
            heartbeat_prompt,
        )
        self.assertIn("обычную пишущую задачу запускает пользователь", heartbeat_prompt)
        self.assertIn("завершается без ребёнка", heartbeat_prompt)
        self.assertNotIn("Это пятиминутный тик", heartbeat_prompt)

    def test_контрольный_и_итоговый_коммиты_не_запускают_исторический_конвейер(это) -> None:
        ...
        for текст in (правила_агентов, навык_комплексной_проверки):
            это.assertIn("AGENTS.md", текст)

        это.assertIn("<!-- FUM-WRITING-MODE: manual-sequential-v1 -->", правила_агентов)
        это.assertIn("Коммит завершает этап, не задачу", правила_агентов)
        это.assertIn("доступную согласованную работу продолжай в ней без нового запроса", правила_агентов)

        это.assertIn("регулярно фиксируя содержательные этапы локальными коммитами", правила_агентов)
        это.assertIn("Коммит и продолжение сами не запускают continuation, handoff, heartbeat, dispatcher, autostart или иной автоматический follow-up.", правила_агентов)
        это.assertIn("Контрольная точка по `FUM-ПРАВИЛО-000188`", правила_агентов)
        это.assertIn("продолжай доступное в том же ходе", правила_агентов)
        это.assertIn("Исторические queue/pool/worktree", правила_агентов)
        это.assertIn("Они не разрешают запись и не удаляются автоматически.", правила_агентов)
        это.assertIn("Пишущие исполнители работают в отдельных рабочих копиях и ветках", корневая_инструкция)
        это.assertIn("не выполняет Git-коммит", навык_комплексной_проверки)
        это.assertIn("Текущая граница приёмки — один проверенный этап режима, выбранного", навык_комплексной_проверки)
        ...
        ограждения = {
            ...
            "Инструменты/fum-pochinka-avtozapuska/SKILL.md": "не разрешает continuation, FIFO-handoff",
            ...
            "Инструменты/fum-pochinka-avtozapuska/references/промпт-задачи-починки.md": "Предыдущий контур обязательного продолжения",
            "Инструменты/реестр-системных-приложений-и-инструментов.md": "не создаёт continuation",
            "Глоссарий/диспетчер-автоматизаций-FUM.md": "Предыдущей заменой было",
            "Глоссарий/автоматизация-FUM.md": "continuation/FIFO-профиль сохранён только как отложенная наработка",
            "Глоссарий/документационный-прототип-FUM.md": "действующей ручной схеме",
            "Глоссарий/агентский-цикл.md": "пользователь вручную запускает",
            "Глоссарий/наблюдаемый-входной-сигнал.md": "следующую сессию запускает пользователь",
            "Глоссарий/ветка-работы.md": "В действующей ручной схеме",
            "Глоссарий/задача-починки-автозапуска.md": "Пользователь вручную запускает",
            "Глоссарий/подузел-FUM.md": "отложенной целевой архитектурой",
            "Глоссарий/пишущий-подузел-FUM.md": "отложенной целевой архитектурой",
            "Глоссарий/сессия-шага-FUM.md": "отложенным профилем",
            "Глоссарий/карточка-шага.md": "отложенным контуром",
            "Глоссарий/дочерний-fork-агент-FUM.md": "отложенной целевой архитектурой",
            "Глоссарий/универсальный-исполнительный-подузел-FUM.md": "отложенной целевой архитектурой",
            "Глоссарий/ветвевой-fork-FUM.md": "отложенной целевой архитектурой",
            "Документация/04-параллельная-работа-и-слияние.md": "Эксплуатационный статус",
            "Документация/01-модель-памяти-FUM.md": "В действующей ручной схеме пользователь сам запускает",
            "Документация/06-обзор-агентских-циклов.md": "пользователь сам запускает",
            "Документация/17-воспроизводимые-автоматизации.md": "Историческая воспроизводимая координация задач",
            "Документация/20-Git-инфраструктура-эволюционных-цепочек-FUM.md": "Эксплуатационный статус",
            "Документация/22-архитектура-FUM.md": "Текущая ручная схема",
            "Документация/24-локальный-агент-на-выделенной-машине.md": "пользователь вручную запускает",
            "Документация/25-интерфейс-FUM-узла.md": "В текущем ручном контуре Git + Codex",
            "Документация/27-публичный-upstream-и-форки-памяти.md": "Отложенная локальная публикационная граница worktree-пула",
            "Документация/36-паспорт-документационного-прототипа-и-первого-коробочного-среза.md": "Каждую пишущую сессию пользователь запускает вручную",
            "Документация/44-репозиторный-граф-пишущих-подузлов-и-проектов-FUM.md": "Эксплуатационный статус",
            "Документация/46-проверяемая-воспроизводимость-и-экспериментальная-приёмка-FUM.md": "В действующей ручной схеме",
            "Инструменты/fum-branch-task-gate/README.md": "Действующий контракт ручной последовательной работы",
            "Планирование/README.md": "Планирование является информационным слоем",
            "Планирование/карточки-шагов/README.md": "не является исполняемой очередью",
            "Планирование/следующие-шаги-веток/README.md": "команда `show` возвращает `done`",
            "Планирование/карточки-цепочек-шагов/README.md": "сохранены как отложенная архитектурная наработка",
            ...
            "Вопросы/2026-07-27_15-21-35_MSK_границы-периодической-публикации-ветки.md": "не создаёт и не передаёт continuation",
            "Сбои/README.md": "| FUM-СБОЙ-0002 | `снята`",
            "Сбои/FUM-СБОЙ-0002-самовольная-отмена-ожидающего-FIFO-билета.md": "ручная последовательная схема не создаёт FIFO-билеты",
            "Сбои/FUM-СБОЙ-0003-обход-HEAD-bootstrap-при-первичном-входе-в-FIFO.md": "ручная последовательная схема не выполняет первичный `join`",
            "Требования/README.md": "Действующий эксплуатационный статус",
            "Требования/✅-обязательное-продолжение-Git-ветки-после-коммита.md": "Эксплуатационный статус: отложено",
            "Требования/✅-выбор-следующего-шага-ветки-из-карточек-шагов.md": "Эксплуатационный статус: отложено",
            "Требования/🚧-веточные-цепочки-шагов-и-завершение-smoke-check-коммитом.md": "Эксплуатационный статус: отложено",
            "Требования/🚧-контекстно-посильные-исполняемые-шаги.md": "Эксплуатационный статус: частично действует только граница",
            "Требования/✅-коммитируемые-вклады-пишущих-подузлов-FUM.md": "Эксплуатационный статус: отложено",
            "Требования/✅-изолированное-параллельное-исполнение-и-проверяемая-интеграция.md": "Эксплуатационный статус: отложено",
            "Требования/✅-ограниченное-автоматическое-разрешение-Git-конфликтов.md": "Эксплуатационный статус: отложено",
            "Требования/✅-репозиторная-композиция-долговечных-подузлов-и-проектов.md": "Эксплуатационный статус: сохранённый исходный срез",
            "Требования/🚧-штатный-сброс-FIFO-очереди-и-рабочей-копии.md": "Эксплуатационный статус: отложено",
            "Требования/✅-подтверждаемый-ручной-сброс-FIFO-к-текущему-HEAD.md": "Эксплуатационный статус: отложено",
        }
        for относительный_путь, ожидаемый_маркер in ограждения.items():
            текст = корень.joinpath(относительный_путь).read_text(encoding="utf-8")
            это.assertIn(ожидаемый_маркер, текст, относительный_путь)
