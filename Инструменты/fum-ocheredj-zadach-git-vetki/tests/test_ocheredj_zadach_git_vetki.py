...
class RepositoryIntegrationTests(unittest.TestCase):
    ...
    def test_контрольный_и_итоговый_коммиты_не_запускают_исторический_конвейер(это) -> None:
        ...
        это.assertIn("доступная согласованная работа продолжается", правила_агентов)

        это.assertIn("локальным `git commit`", правила_агентов)
        это.assertIn("Коммит и продолжение работы сами по себе не запускают", правила_агентов)
        это.assertIn("Контрольная точка по `FUM-ПРАВИЛО-000188`", правила_агентов)
        это.assertIn("продолжай доступную работу в том же ходе", правила_агентов)
        ...
        это.assertIn("Приёмка этапа с осмысленным diff", навык_комплексной_проверки)
