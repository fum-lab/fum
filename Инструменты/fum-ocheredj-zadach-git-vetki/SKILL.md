...
При падении до транзакции не меняется ни одна ссылка; после успешной транзакции изменены обе. Конкурирующие `join`, `ack-head` или `cancel` могут изменить только ссылку очереди, поэтому владелец перечитывает её и повторяет транзакцию, не ослабляя проверку старого `HEAD`. `last_completion` делает повтор ответа после неоднозначного результата идемпотентным. Ответ `committed` содержит точные `new_head` и `branch_ref`, необходимые для отдельной автоматической публикации.

## Автоматическая публикация точного коммита

До `commit` корень определяет публикационный remote текущей ветки и сохраняет его единственный push URL. Это должен быть credential-free HTTPS URL репозитория `github.com` без userinfo, нестандартного порта, query или fragment. Применимые `url.*.insteadOf` и `url.*.pushInsteadOf` запрещены: иначе фактический endpoint нельзя доказать по сохранённой строке. URL не меняется между локальной передачей и публикацией. Имя remote не передаётся публикатору: прямой URL не обновляет общий remote-tracking ref после снятия владельца.

После `committed` прежняя задача выполняет ровно одно разрешённое post-handoff-действие. Она загружает сценарий из точного `new_head`, а не из подвижного `HEAD` или рабочего дерева, и передаёт значения того же JSON-ответа:
...
Состояния `diverged`, `rejected` и `unconfirmed` завершаются ошибкой и не устраняются через `pull`, merge, rebase, force-push, откат локального коммита или возврат владения. Корень сообщает такой результат пользователю. После любого результата `publish` эта задача больше не выполняет внешних мутаций. `finish-clean` публикацию не запускает.
...
Набор из 58 тестов проверяет строгий FIFO при конкурентных `join`, идемпотентность, бессрочность билетов, бесшумное ожидание до действенного состояния, чистое прерывание, пятиминутный запасной read-only `wait`, узкие состояния heartbeat без раскрытия непрозрачных полей, включая свободную очередь после завершённой ручной сессии, запрет обхода, собственную отмену до допуска, `finish-clean`, безопасное восстановление `finish-own-clean` при чужом владельце, отсутствии владельца, dirty/staged-изменениях, смене `HEAD` и гонке поколений, обязательный `reload_required`/`ack-head`, атомарные branch-fenced admission, clean finish и commit+handoff, чистоту рабочего дерева, смену ветки, SHA-1/SHA-256, отсутствие optional stat-cache записи до допуска, явные ошибки постоянных ref locks, isolated-исполнение сценария из буквального `HEAD`, закрепление корня, полную очистку Git-среды и запрет trace-файлов, игнорирование replace-объектов внутри сценария, отсутствие POSIX-примитивов, отсутствие project hooks и переходный no-op. Четырнадцать offline-сценариев с локальным bare-remote проверяют точный старый коммит при продвинувшемся `HEAD`, известного и неизвестного локально опубликованного потомка, расхождение, серверный отказ, отключение локального pre-push hook, запрет неявной отправки тегов, неоднозначный ответ, ошибки чтения и анализа удалённой вершины, завершение потомков транспорта при тайм-ауте, bootstrap из точного коммита и раннее отклонение небезопасных URL и Git URL rewrite.
...
<!-- content-sha256: sha256:7602abd189952f9a98256d221eb5e6d38590dc4c5b654e10042f5cec4e8da472 -->
<!-- FUM-MD-RECENCY:END -->
