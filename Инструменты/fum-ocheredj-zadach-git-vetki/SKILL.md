...
Набор из 35 тестов проверяет строгий FIFO при конкурентных `join`, идемпотентность, бессрочность билетов, бесшумное ожидание до действенного состояния, чистое прерывание, пятиминутный запасной read-only `wait`, запрет обхода, собственную отмену до допуска, `finish-clean`, обязательный `reload_required`/`ack-head`, атомарные branch-fenced admission, clean finish и commit+handoff, чистоту рабочего дерева, смену ветки, SHA-1/SHA-256, отсутствие optional stat-cache записи до допуска, явные ошибки постоянных ref locks, isolated-исполнение сценария из буквального `HEAD`, закрепление корня, полную очистку Git-среды и запрет trace-файлов, игнорирование replace-объектов внутри сценария, отсутствие POSIX-примитивов, отсутствие project hooks и переходный no-op.
...
<!-- content-sha256: sha256:2af088ea50f39a48bcb78da9b79f43f0e25ecec7b27c6047b6ef1a48abefd82b -->
<!-- FUM-MD-RECENCY:END -->
