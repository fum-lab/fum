...
СХЕМА_КВИТАНЦИИ_ИНТЕГРАЦИИ = "fum.квитанция-CAS-интеграции-worktree-подузлов.1"
СХЕМА_КВИТАНЦИИ_РЕВЬЮ = "fum.квитанция-агентского-ревью-worktree-подузла.1"
СХЕМА_НАМЕРЕНИЯ_ПУБЛИКАЦИИ_ИНТЕГРАЦИИ = (
    "fum.намерение-публикации-принятой-интеграции.1"
)
ПОЛЯ_КВИТАНЦИИ_C0_АКТИВАЦИИ = {
    "schema",
    "repository_identity",
    "task_id",
    "generation",
    ...
    "migration_receipt_oid",
    "migration_attestation_hash",
    "pool_ref",
    "source_pool_oid",
    "integration_hash",
    "queue_ref",
    ...
    "owner_ticket_id",
    "owner_seq",
    "first_continuation_ticket_id",
    "first_continuation_seq",
    "waiting_hash",
    "next_seq",
}
ПОЛЯ_КВИТАНЦИИ_ИНТЕГРАЦИИ = {
    "schema",
...
ОСНОВНЫЕ_ПОЛЯ_КАНДИДАТА_ИНТЕГРАЦИИ = {
    "schema",
    "integrator_assignment_id",
    "integrator_assignment_hash",
    "task_id",
    "host_id",
    "branch_ref",
    "target_ref",
...
ПОЛЯ_ОБЫЧНОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА = {
    "schema",
    "assignment_hash",
    "activation_hash",
    "task_id",
...
ПОЛЯ_ПРОИСХОЖДЕНИЯ_МИГРИРОВАННОГО_РЕЗУЛЬТАТА = {
...
ПРОСТРАНСТВО_КВИТАНЦИЙ_C0_АКТИВАЦИИ = (
...
@dataclass(frozen=True)
class ИдентичностьЗагрузкиЯдраМиграции:
    канонический_корень: str
    ревизия_протокола: str
    байты_сценария: bytes
    байты_ядра: bytes | None
...
def объединить_проверки_ссылок(
    проверки: Iterable[tuple[str, str | None]],
) -> list[tuple[str, str | None]]:
    ожидаемые: dict[str, str | None] = {}
    for ссылка, идентификатор_объекта in проверки:
        прежний = ожидаемые.get(ссылка, идентификатор_объекта)
        if прежний != идентификатор_объекта:
            raise ОшибкаПула(65, "conflicting_ref_snapshot", "Одна ссылка имеет два ожидаемых OID в exact-снимке.")
        ожидаемые[ссылка] = идентификатор_объекта
    return sorted(ожидаемые.items())
...
def эффективная_ревизия_протокола_до_активации(
    контекст: КонтекстПула,
    целевая_ссылка: str,
    исходная_ревизия: str,
    *,
    ожидаемая_вершина_цели: str | None = None,
    ожидаемый_объект_пула: str | None = None,
    назначение: dict[str, Any] | None = None,
    проверки_ссылок: list[tuple[str, str]] | None = None,
) -> str:
    пул, идентификатор_пула = прочитать_пул(контекст)
    if (
        ожидаемый_объект_пула is not None
        and идентификатор_пула != ожидаемый_объект_пула
    ):
        raise ОшибкаПула(73, "assignment_execution_pool_changed", "Пул сдвинулся до выбора ревизии исполнения.")
    миграция = пул.get("миграция_дат_коммитов")
    совпавшая_миграция = (
        пул.get("schema") != СХЕМА_ПУЛА
        or not isinstance(миграция, dict)
        or миграция.get("target_ref") != целевая_ссылка
    ) is False
    if совпавшая_миграция and миграция.get("status") == "locked":
        raise ОшибкаПула(
            73,
            "migration_execution_locked",
            "Исполнение назначений заблокировано до завершения exact миграции.",
        )
    if совпавшая_миграция and миграция.get("status") not in {"completed", "activated"}:
        raise ОшибкаПула(65, "invalid_migration_state", "Фаза миграции для выбора протокола повреждена.")
    текущая_вершина_цели = прочитать_ссылку(контекст, целевая_ссылка)
    if текущая_вершина_цели is None:
        raise ОшибкаПула(73, "migration_execution_target_missing", "Целевая ссылка миграции исчезла.")
    if (
        ожидаемая_вершина_цели is not None
        and текущая_вершина_цели != ожидаемая_вершина_цели
    ):
        raise ОшибкаПула(73, "migration_execution_target_changed", "Цель сдвинулась во время выбора ревизии исполнения.")
    if not совпавшая_миграция:
        ревизия_исполнения = исходная_ревизия
    else:
        проверить_полную_миграцию_дат_перед_слиянием(
            контекст,
            пул,
            проверки_ссылок=проверки_ссылок,
            проверять_достижимость=False,
        )
    if совпавшая_миграция and миграция["status"] == "completed":
        if текущая_вершина_цели != миграция["target_oid"]:
            raise ОшибкаПула(73, "migration_completed_target_changed", "Completed-фаза потеряла exact домиграционную цель.")
        ревизия_исполнения = проверить_идентификатор_объекта(контекст, миграция["tool_oid"])
    elif совпавшая_миграция:
        результат_C0 = проверить_квитанцию_C0_активации(
            контекст, пул, миграция, текущая_вершина_цели
        )
        if проверки_ссылок is not None:
            if not isinstance(результат_C0, tuple) or len(результат_C0) != 2:
                raise ОшибкаПула(65, "migration_activation_receipt_invalid", "C0 validator не вернул exact снимок ссылок.")
            проверки_ссылок.extend(результат_C0[1])
        ревизия_исполнения = проверить_идентификатор_объекта(контекст, текущая_вершина_цели)
        if назначение is not None and назначение_ожидает_подтверждения_протокола(
            контекст,
            пул,
            назначение,
            проверки_ссылок=проверки_ссылок,
            ...
            ревизия_исполнения = проверить_идентификатор_объекта(контекст, миграция["tool_oid"])
    фактический_объект_пула = прочитать_ссылку(контекст, контекст.ссылка_пула)
    if фактический_объект_пула != идентификатор_пула:
        raise ОшибкаПула(
            73,
            "assignment_execution_pool_changed",
            "Пул сдвинулся во время выбора протокола.",
            ожидаемый_объект_пула=идентификатор_пула,
            фактический_объект_пула=фактический_объект_пула,
        )
    if прочитать_ссылку(контекст, целевая_ссылка) != текущая_вершина_цели:
        raise ОшибкаПула(73, "assignment_execution_target_changed", "Цель сдвинулась во время выбора протокола.")
    return ревизия_исполнения
...
def нагрузка_назначения(
    ...
    *,
    ...
    if сохранённая_вершина_протокола is None:
        ...
        вершина_протокола = эффективная_ревизия_протокола_до_активации(
            контекст,
            целевая_ссылка,
            вершина_протокола,
            ожидаемая_вершина_цели=вершина_протокола,
        )
    else:
...
def ответ_назначения(
    запись: dict[str, Any],
    состояние: str,
    закреплённая_ревизия_исполнения: str,
) -> dict[str, object]:
    return {
        "state": состояние,
        ...
        "ревизия_исполнения_протокола": закреплённая_ревизия_исполнения,
        "хэш_назначения": запись["assignment_hash"],
        "промпт_запуска": сформировать_промпт_запуска(
            запись,
            закреплённая_ревизия_исполнения,
        ),
    }


def ответ_терминального_назначения(
    запись: dict[str, Any],
    состояние: str,
) -> dict[str, object]:
    ответ = ответ_назначения(
        запись,
        состояние,
        запись["protocol_oid"],
    )
    ответ.pop("промпт_запуска", None)
    ответ.pop("ревизия_исполнения_протокола", None)
    return ответ


def сформировать_промпт_запуска(запись: dict[str, Any], ревизия_исполнения: str) -> str:
    ...
    код_начальной_загрузки = (
        ...
        f'o="{ревизия_исполнения}";'
        'b=subprocess.check_output(["git","--no-replace-objects","-C",r,"show",o+":"+p],env=e,timeout=30);'
        'канонический_корень=subprocess.check_output(["git","--no-replace-objects","-C",r,"rev-parse","--show-toplevel"],env=e,timeout=30).decode("utf-8").strip();'
        'канонический_корень=os.path.realpath(канонический_корень);'
        'globals()["_ИДЕНТИЧНОСТЬ_ЗАКРЕПЛЁННОЙ_ЗАГРУЗКИ"]=(канонический_корень,o,b,None);'
    ...
    return (
        ...
        f"неизменяемая исходная ревизия протокола: {запись['protocol_oid']}; "
        f"ревизия исполнения протокола: {ревизия_исполнения}; "
        ...
        "из закреплённой ревизии исполнения протокола, сверь её с неизменяемой исходной ревизией и все поля делегации с ответом. "
        "Перейди к относительному пути_worktree из ответа и вызывай все содержательные и терминальные "
        "команды только оттуда через ту же ревизию исполнения. Для роли рецензента фактический "
...
def команда_выделить(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    ревизия_исполнения = эффективная_ревизия_протокола_до_активации(
        контекст,
        ожидаемое["target_ref"],
        ожидаемое["protocol_oid"],
        назначение=сохранённое,
    )
    ...
    return ответ_назначения(подтверждённое, состояние, ревизия_исполнения)
...
def команда_зарезервировать_себя(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    if назначение["status"] in {"registered", "bound"}:
        ...
        raise ОшибкаПула(73, "host_binding_mismatch", "Повтор самозапуска имеет иной host-id.")
    _, назначение, _ = получить_назначение(контекст, идентификатор)
    ответ = ответ_назначения(назначение, "worktree_reserved", ревизия_исполнения)
    ответ.update(
        {
            "task_id": идентификатор_задачи,
...
def допустить_самозапуск_одной_транзакцией(
    контекст: КонтекстПула,
    идентификатор: str,
    идентификатор_задачи: str,
    идентификатор_среды: str | None,
) -> dict[str, object]:
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        if назначение["status"] in {"activated", "active"}:
            return команда_получить_допуск(
                argparse.Namespace(
                    корень_репозитория=str(контекст.вызванный_корень),
                    идентификатор_назначения=идентификатор,
                    идентификатор_задачи=идентификатор_задачи,
                )
            )
        ...
        проверить_активационную_группу([назначение])
        проверить_конфликт_с_активными(пул, [назначение])
        ревизия_исполнения, проверки_исполнения = проверить_закреплённую_ревизию_исполнения(
            контекст,
            пул,
            идентификатор_объекта_пула,
            назначение,
        ...
        if очередь is None or идентификатор_объекта_очереди is None:
            raise ОшибкаПула(65, "slot_queue_missing", "Очередь самозапуска отсутствует.")
        ...
        if (
            ...
            or сведения["worktree_id"] != назначение["worktree_id"]
            or not рабочее_дерево_чисто(путь)
        ):
            raise ОшибкаПула(73, "changed_worktree_before_activation", "Worktree изменён до самодопуска.")
        if (
            очередь["status"] != "waiting_activation"
            or очередь["owner"] is not None
            or len(очередь["waiting"]) != 1
            or очередь["waiting"][0]["task_id"] != идентификатор_задачи
            or очередь["waiting"][0]["acknowledged_head"] != ожидаемая_вершина
        ):
            raise ОшибкаПула(73, "inactive_ticket_mismatch", "Неактивный ticket самозапуска не совпал.")
        данные_активации = {
        ...
        хэш_активации = хэш_объекта(данные_активации)
        прежняя_активация = пул["activations"].get(хэш_активации)
        if прежняя_активация is not None and прежняя_активация != данные_активации:
            raise ОшибкаПула(65, "activation_hash_collision", "Хэш самодопуска связан с иной активацией.")
        ...
        if транзакция_ссылок(
            ...
            [
                (
                    ...
                    записать_объект_состояния(контекст, новый_пул),
                    идентификатор_объекта_пула,
                ),
                (
                    назначение["queue_ref"],
            ...
            проверки=[
                ...
                if проверка[0] not in {контекст.ссылка_пула, назначение["queue_ref"]}
            ],
        ):
        ...
        time.sleep(0.002)
    raise ОшибкаПула(75, "self_admission_cas_exhausted", "Не удалось атомарно допустить самозапуск.")
...
def команда_подтвердить_и_войти(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    return допустить_самозапуск_одной_транзакцией(
        контекст,
        идентификатор,
        идентификатор_задачи,
        идентификатор_среды,
...
def подтвердить_снимок_восстановления(
    ...
    *,
    ...
    if not транзакция_ссылок(
        контекст,
        [],
        проверки=объединить_проверки_ссылок(проверки),
...
def команда_восстановить_сессию(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    if маршрут.get("status") == "result_frozen" and маршрут.get("result_hash"):
        ...
        ответ = ответ_терминального_назначения(назначение, "result_frozen")
        ответ.update(
            {
                "state": "result_frozen",
                "task_id": идентификатор_задачи,
    ...
    if маршрут.get("status") == "committed_handoff":
        ...
        ответ = ответ_терминального_назначения(назначение, "committed_handoff")
        ответ.update(
            {
    ...
    ответ = ответ_назначения(назначение, "session_recovered", ревизия_исполнения)
    ответ.pop("промпт_запуска", None)
    ответ.update(живые_поля_ответа)
    проверки_снимка = [
        *проверки_маршрута,
        *проверки_исполнения,
        (назначение["queue_ref"], идентификатор_объекта_очереди),
...
def команда_войти_в_линию_и_ждать(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    while True:
        ...
        if очередь is None or идентификатор_объекта_очереди is None:
            raise ОшибкаПула(65, "slot_queue_missing", "Очередь линии отсутствует.")
        проверить_очередь(очередь, назначение)
        проверки_снимка = объединить_проверки_ссылок(
            [
                *проверки_маршрута,
                *проверки_исполнения,
                (назначение["queue_ref"], идентификатор_объекта_очереди),
        ...
        if владелец is not None and владелец["task_id"] == идентификатор_задачи:
            if очередь["status"] == "reload_required":
                return ответ_маршрута_продолжения(назначение, хэш_продолжения, "reload_required")
            if очередь["status"] == "active" and владелец.get("generation"):
                if транзакция_ссылок(контекст, [], проверки=проверки_снимка):
                    return ответ_допуска(
                        контекст,
                        назначение,
                        идентификатор_задачи,
...
def команда_подтвердить_вершину_линии(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        if транзакция_ссылок(
            ...
            проверки=[
                проверка
                for проверка in проверки_снимка
                if проверка[0] not in {контекст.ссылка_пула, назначение["queue_ref"]}
            ],
        ):
            return ответ_допуска(
...
def команда_зарегистрироваться(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        try:
            пул, назначение, идентификатор_объекта_пула = получить_назначение(контекст, идентификатор)
            _, проверки_исполнения = проверить_закреплённую_ревизию_исполнения(
                контекст,
                пул,
                идентификатор_объекта_пула,
                назначение,
            ...
            if (
                ошибка.состояние != "assignment_execution_pool_changed"
                or not доступны_проверяемые_байты_исполнения()
            ):
                raise
            time.sleep(0.002)
            continue
        ...
        if транзакция_ссылок(
            ...
            проверки=[
                ...
                *(
                    проверка
                    for проверка in проверки_исполнения
                    if проверка[0] not in {контекст.ссылка_пула, назначение["queue_ref"]}
                ),
            ],
        ):
...
def команда_войти_и_ждать(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    if контекст.вызванный_корень == ожидаемый_слот:
        проверить_вызов_из_слота(контекст, назначение)
    if (
        назначение.get("registered_task_id") == аргументы_команды.идентификатор_задачи
        and назначение.get("status") in {"activated", "active"}
    ):
        return команда_получить_допуск(аргументы_команды)
...
def команда_активировать(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        for запись in записи:
            ...
            проверки_маршрутов.extend(проверки_исполнения)
        проверки_маршрутов = объединить_проверки_ссылок(проверки_маршрутов)
        для_хэша = {
            "schema": "fum.активация-группы-worktree-подузлов.1",
            "assignments": [
                {
        ...
        if all(запись["activation_hash"] == хэш_активации for запись in записи):
            проверки_повтора = list(проверки_маршрутов)
            for запись in записи:
                очередь_повтора, идентификатор_очереди_повтора = прочитать_объект_состояния(контекст, запись["queue_ref"])
                if очередь_повтора is None or идентификатор_очереди_повтора is None:
                    raise ОшибкаПула(65, "slot_queue_missing", "Очередь повторной активации отсутствует.")
                проверить_очередь(очередь_повтора, запись)
                проверки_повтора.append((запись["queue_ref"], идентификатор_очереди_повтора))
            if транзакция_ссылок(контекст, [], проверки=объединить_проверки_ссылок(проверки_повтора)):
                return {
                    "state": "activated",
        ...
        изменяемые_ссылки = {изменение[0] for изменение in изменения}
        проверки_активации = [
            проверка
            for проверка in проверки_маршрутов
            if проверка[0] not in изменяемые_ссылки
        ]
        if транзакция_ссылок(контекст, изменения, проверки=проверки_активации):
            return {
                "state": "activated",
                "хэш_активации": хэш_активации,
                "назначения": sorted(идентификаторы),
...
def команда_получить_допуск(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        try:
            пул, назначение, идентификатор_объекта_пула = получить_назначение(контекст, идентификатор)
            закреплённая_ревизия, проверки_исполнения = проверить_закреплённую_ревизию_исполнения(
                контекст,
                пул,
                идентификатор_объекта_пула,
                назначение,
        ...
        проверки_маршрута.extend(проверки_исполнения)
        проверки_маршрута.append((назначение["queue_ref"], идентификатор_объекта_очереди))
        проверки_маршрута = объединить_проверки_ссылок(проверки_маршрута)
        ...
        проверки_нового_владельца = [
            проверка
            for проверка in проверки_маршрута
            if проверка[0] not in {контекст.ссылка_пула, назначение["queue_ref"]}
        ]
        if транзакция_ссылок(контекст, изменения, проверки=проверки_нового_владельца):
            return ответ_допуска(
                контекст,
                назначение,
                идентификатор_задачи,
...
def проверить_архивное_закрепление_квитанции_миграции(
    ...
    *,
    ...
    if проверки_ссылок is not None:
        проверки_ссылок.extend(
            [(ожидаемая_ссылка_квитанции, идентификатор_блоба), *ожидаемые_архивы.items(), *ожидаемые_архивы_состояния.items()]
        )
    for старый_идентификатор, новый_идентификатор in соответствие.items():
...
def проверить_полную_миграцию_дат_перед_слиянием(
    ...
    *,
    проверки_ссылок: list[tuple[str, str]] | None = None,
    проверять_достижимость: bool = True,
) -> dict[str, object]:
    ...
    живые_ветви = миграция["branch_refs"]
    if проверять_достижимость:
        живые_ветви = канонические_живые_ветви_пула(контекст, пул)
        if проверки_ссылок is not None:
            проверки_ссылок.extend(живые_ветви.items())
...
def проверить_неизменяемое_назначение_и_активацию(
    пул: dict[str, Any],
    назначение: dict[str, Any],
    *,
    состояние: str,
    ...
    поля_нагрузки = {
        "id", "generation", "attempt", "base_oid", "branch_ref", "role",
        "project", "step", "write_paths", "target_ref", "remote",
        "protocol_oid", "lifecycle", "routing_hash",
    }
    нагрузка = {
        поле: copy.deepcopy(назначение.get(поле))
        for поле in поля_нагрузки
    }
    хэш_назначения = назначение.get("assignment_hash")
    ожидаемый_хэш_назначения = хэш_объекта(
        {"schema": "fum.назначение-worktree-подузла.1", **нагрузка}
    )
    if хэш_назначения != ожидаемый_хэш_назначения:
        raise ОшибкаПула(65, состояние, "Immutable assignment payload не совпала с canonical assignment_hash.")

    хэш_активации = назначение.get("activation_hash")
    активация = (
        пул.get("activations", {}).get(хэш_активации)
        if isinstance(хэш_активации, str)
        else None
    )
    записи_активации = активация.get("assignments") if isinstance(активация, dict) else None
    поля_записи = {"assignment_hash", "task_id", "host_id", "worktree_id", "queue_ref"}
    if (
        not isinstance(хэш_активации, str)
        or re.fullmatch(r"sha256:[0-9a-f]{64}", хэш_активации) is None
        or not isinstance(активация, dict)
        or set(активация) != {"schema", "assignments"}
        or активация.get("schema") != "fum.активация-группы-worktree-подузлов.1"
        or хэш_объекта(активация) != хэш_активации
        or not isinstance(записи_активации, list)
        or not записи_активации
        or any(
            not isinstance(запись, dict)
            or set(запись) != поля_записи
            or any(not isinstance(запись.get(поле), str) or not запись[поле] for поле in поля_записи - {"host_id"})
            or (запись.get("host_id") is not None and (not isinstance(запись["host_id"], str) or not запись["host_id"]))
            for запись in записи_активации
        )
    ):
        raise ОшибкаПула(65, состояние, "Activation receipt assignment отсутствует или повреждена.")

    идентификаторы_группы: list[str] = []
    for запись in записи_активации:
        совпадения = [
            (идентификатор, запись_назначения)
            for идентификатор, запись_назначения in пул.get("assignments", {}).items()
            if isinstance(запись_назначения, dict)
            and запись_назначения.get("assignment_hash") == запись["assignment_hash"]
        ]
        if len(совпадения) != 1:
            raise ОшибкаПула(65, состояние, "Activation entry не связана с одним assignment.")
        идентификатор, запись_назначения = совпадения[0]
        нагрузка_группы = {
            поле: copy.deepcopy(запись_назначения.get(поле))
            for поле in поля_нагрузки
        }
        ожидаемая_запись = {
            "assignment_hash": хэш_объекта({"schema": "fum.назначение-worktree-подузла.1", **нагрузка_группы}),
            "task_id": запись["task_id"],
            "host_id": запись["host_id"],
            "worktree_id": запись_назначения.get("worktree_id"),
            "queue_ref": запись_назначения.get("queue_ref"),
        }
        маршрут_активации = пул.get("session_routes", {}).get(запись["task_id"])
        задача_осталась_владельцем = запись["task_id"] == запись_назначения.get("registered_task_id")
        задача_доказана_маршрутом = (
            isinstance(маршрут_активации, dict)
            and маршрут_активации.get("task_id") == запись["task_id"]
            and маршрут_активации.get("assignment_id") == идентификатор
        )
        if (
            запись != ожидаемая_запись
            or запись_назначения.get("activation_hash") != хэш_активации
            or not (задача_осталась_владельцем or задача_доказана_маршрутом)
            or (задача_осталась_владельцем and запись["host_id"] != запись_назначения.get("host_id"))
        ):
            raise ОшибкаПула(65, состояние, "Activation entry не воспроизводит exact assignment/task/host/worktree/queue tuple.")
        идентификаторы_группы.append(идентификатор)
    if идентификаторы_группы != sorted(set(идентификаторы_группы)):
        raise ОшибкаПула(65, состояние, "Activation assignment group не имеет canonical порядка.")
    if хэш_назначения not in {запись["assignment_hash"] for запись in записи_активации}:
        raise ОшибкаПула(65, состояние, "Assignment отсутствует в своей activation group.")


def проверить_квитанцию_C0_активации(
    контекст: КонтекстПула,
    пул: dict[str, Any],
    миграция: dict[str, Any],
    текущая_вершина_цели: str,
) -> tuple[dict[str, Any], list[tuple[str, str]]]:
    def коммит_из_поля(объект: dict[str, Any], поле: str) -> str:
        значение = объект.get(поле)
        if not isinstance(значение, str):
            raise ОшибкаПула(65, "migration_activation_receipt_invalid", f"Поле { поле } C0 не является exact commit OID.")
        try:
            return проверить_идентификатор_объекта(контекст, значение)
        except ОшибкаПула as ошибка:
            raise ОшибкаПула(65, "migration_activation_receipt_invalid", f"Поле { поле } C0 не является exact commit OID.") from ошибка

    def блоб_из_поля(объект: dict[str, Any], поле: str) -> str:
        значение = объект.get(поле)
        if not isinstance(значение, str):
            raise ОшибкаПула(65, "migration_activation_receipt_invalid", f"Поле { поле } C0 не является exact blob OID.")
        try:
            return проверить_идентификатор_блоба(контекст, значение)
        except ОшибкаПула as ошибка:
            raise ОшибкаПула(65, "migration_activation_receipt_invalid", f"Поле { поле } C0 не является exact blob OID.") from ошибка

    хэш_квитанции = миграция.get("bootstrap_receipt_hash")
    if not isinstance(хэш_квитанции, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", хэш_квитанции) is None:
        raise ОшибкаПула(65, "migration_activation_receipt_invalid", "Activated-фаза потеряла хэш C0 receipt.")
    ссылка_квитанции = (
        f"{ПРОСТРАНСТВО_КВИТАНЦИЙ_C0_АКТИВАЦИИ}/"
        f"{контекст.идентификатор_репозитория}/{хэш_квитанции[7:]}"
    )
    идентификатор_квитанции = прочитать_ссылку(контекст, ссылка_квитанции)
    if идентификатор_квитанции is None:
        raise ОшибкаПула(65, "migration_activation_receipt_missing", "Activated-фаза потеряла exact C0 receipt.")
    квитанция = прочитать_канонический_блоб_по_идентификатору(
        контекст,
        проверить_идентификатор_блоба(контекст, идентификатор_квитанции),
    )
    соответствие_полей = (
        ("tool_oid", "tool_oid"),
        ...
        ("migration_receipt_hash", "receipt_hash"),
        ("migration_receipt_oid", "receipt_oid"),
        ("migration_attestation_hash", "attestation_hash"),
    )
    if (
        set(квитанция) != ПОЛЯ_КВИТАНЦИИ_C0_АКТИВАЦИИ
        or квитанция.get("schema") != СХЕМА_КВИТАНЦИИ_C0_АКТИВАЦИИ
        or хэш_объекта(квитанция) != хэш_квитанции
        or квитанция.get("repository_identity") != контекст.идентификатор_репозитория
        or квитанция.get("pool_ref") != контекст.ссылка_пула
        or any(квитанция.get(поле_квитанции) != миграция.get(поле_миграции) for поле_квитанции, поле_миграции in соответствие_полей)
    ):
        raise ОшибкаПула(65, "migration_activation_receipt_invalid", "C0 receipt не совпала с exact activated-состоянием.")
    строковые_поля = (
        "task_id",
        "generation",
        "continuation_task_id",
    ...
    if (
        any(not isinstance(квитанция.get(поле), str) or not квитанция[поле] for поле in строковые_поля)
        or any(not isinstance(квитанция.get(поле), str) or re.fullmatch(r"sha256:[0-9a-f]{64}", квитанция[поле]) is None for поле in хэши)
        or any(type(квитанция.get(поле)) is not int or квитанция[поле] < 1 for поле in ("owner_seq", "first_continuation_seq", "next_seq"))
        or not квитанция["queue_ref"].startswith("refs/fum/worktree-task-queues/")
        or not квитанция["target_ref"].startswith("refs/heads/")
    ):
        raise ОшибкаПула(65, "migration_activation_receipt_invalid", "Поля C0 receipt имеют неверные типы или форматы.")
    вершина_C0 = коммит_из_поля(квитанция, "candidate_head_oid")
    инструмент_C0 = коммит_из_поля(квитанция, "tool_oid")
    исходная_вершина_C0 = коммит_из_поля(квитанция, "target_oid")
    if not isinstance(текущая_вершина_цели, str):
        raise ОшибкаПула(65, "migration_activation_receipt_invalid", "Текущая C0-цель не является exact commit OID.")
    try:
        текущая_вершина_цели = проверить_идентификатор_объекта(контекст, текущая_вершина_цели)
    except ОшибкаПула as ошибка:
        raise ОшибкаПула(65, "migration_activation_receipt_invalid", "Текущая C0-цель не является exact commit OID.") from ошибка
    if (
        not коммит_является_предком(контекст, инструмент_C0, вершина_C0)
        or not коммит_является_предком(контекст, исходная_вершина_C0, вершина_C0)
        or not коммит_является_предком(контекст, вершина_C0, текущая_вершина_цели)
    ):
        raise ОшибкаПула(73, "migration_activation_target_diverged", "Текущая цель не продолжает exact C0-вершину.")

    исходный_объект_пула = блоб_из_поля(квитанция, "source_pool_oid")
    блоб_из_поля(квитанция, "migration_receipt_oid")
    исходный_пул = прочитать_канонический_блоб_по_идентификатору(контекст, исходный_объект_пула)
    проверить_состояние_пула(контекст, исходный_пул)
    ожидаемая_исходная_миграция = copy.deepcopy(миграция)
    ожидаемая_исходная_миграция.update(
        {"status": "completed", "candidate_hash": None, "review_hash": None, "bootstrap_receipt_hash": None}
    )
    if исходный_пул.get("миграция_дат_коммитов") != ожидаемая_исходная_миграция:
        raise ОшибкаПула(65, "migration_activation_source_pool_mismatch", "C0 receipt не связана с exact completed-пулом.")

    исходный_объект_очереди = блоб_из_поля(квитанция, "source_queue_oid")
    исходная_очередь = прочитать_канонический_блоб_по_идентификатору(контекст, исходный_объект_очереди)
    владелец = исходная_очередь.get("owner")
    ожидающие = исходная_очередь.get("waiting")
    идентификатор_рабочего_дерева = исходная_очередь.get("worktree_id")
    основные_поля_очереди = {
        "schema_version", "worktree_id", "branch_ref", "next_seq", "owner", "waiting",
        "last_completion", "updated_at",
    }
    допустимые_поля_очереди = основные_поля_очереди | {
        "текущая_цепочка", "обязательное_продолжение_активировано", "допущенная_задача_барьера",
    }
    поля_владельца = {
        "task_id", "ticket_id", "seq", "generation", "base_head", "admitted_at", "admitted_at_epoch",
    }
    поля_ожидающего = {
        "task_id", "ticket_id", "seq", "registered_at", "registered_at_epoch", "acknowledged_head",
    }
    поля_барьерного_ограждения = {
    ...
    поле_базы_барьера = "базовая_вершина_барьера"
    def непустая_строка(значение: object) -> bool:
        return isinstance(значение, str) and bool(значение)

    def корректная_задача(значение: object) -> bool:
        return (
            isinstance(значение, str)
            and bool(значение.strip())
            and len(значение) <= 1_024
            and not any(знак in значение for знак in ("\x00", "\n", "\r"))
        )

    def положительное_целое(значение: object) -> bool:
        return type(значение) is int and значение >= 1

    def числовая_эпоха(значение: object) -> bool:
        return type(значение) in {int, float} and значение >= 0

    def точный_коммит(значение: object) -> bool:
        if not isinstance(значение, str):
            return False
        try:
            return проверить_идентификатор_объекта(контекст, значение) == значение
        except ОшибкаПула:
            return False

    def точный_блоб(значение: object) -> bool:
        if not isinstance(значение, str):
            return False
        try:
            return проверить_идентификатор_блоба(контекст, значение) == значение
        except ОшибкаПула:
            return False

    проверки_барьерных_реестров: dict[str, str] = {}

    def хэш_барьерного_объекта(значение: dict[str, Any]) -> str:
        байты = json.dumps(
            значение,
            ensure_ascii=False,
            sort_keys=True,
    ...
    def корректный_реестр_барьера(участник: dict[str, Any]) -> bool:
        объект_реестра = участник.get("объект_реестра_барьеров")
        if not isinstance(объект_реестра, str) or not точный_блоб(объект_реестра):
            return False
        ссылка_реестра = f"refs/fum/реестры-барьеров-предактивации/{идентификатор_рабочего_дерева}"
        if прочитать_ссылку(контекст, ссылка_реестра) != объект_реестра:
            return False
        try:
            реестр = прочитать_канонический_блоб_по_идентификатору(контекст, объект_реестра)
        except ОшибкаПула:
            return False
        барьеры = реестр.get("барьеры") if isinstance(реестр, dict) else None
        if (
            not isinstance(реестр, dict)
            or set(реестр) != {"схема", "идентификатор_рабочей_копии", "барьеры"}
            or реестр.get("схема") != "fum.реестр-барьеров-предактивации.2"
            or реестр.get("идентификатор_рабочей_копии") != идентификатор_рабочего_дерева
            or not isinstance(барьеры, list)
            or len(барьеры) != 1
            or not isinstance(барьеры[0], dict)
        ):
            return False
        барьер = барьеры[0]
        поля_открытого_барьера = {
            "ветка", "базовая_вершина", "идентификатор_форка", "поколение_запуска",
            "идентификатор_попытки_создания", "идентификатор_назначения", "хэш_назначения",
            "идентификатор_ресурсной_попытки", "хэш_ресурсного_допуска", "состояние",
            "идентификатор_задачи", "идентификатор_среды", "хэш_конверта", "хэш_привязки",
            "связан_из_объекта", "квитанция_активации", "хэш_предактивации", "хэши_привязок_пары",
            "хэш_активации", "открыт_из_объекта",
        }
        однострочные_поля = {
            "идентификатор_форка", "поколение_запуска", "идентификатор_попытки_создания",
            "идентификатор_назначения", "идентификатор_ресурсной_попытки", "идентификатор_задачи", "идентификатор_среды",
        }
        хэшевые_поля = {
            "хэш_назначения", "хэш_ресурсного_допуска", "хэш_конверта", "хэш_привязки",
            "квитанция_активации", "хэш_предактивации", "хэш_активации",
        }
        полный_объект = rf"[0-9a-f]{{{контекст.длина_идентификатора_объекта}}}"
        пара = барьер.get("хэши_привязок_пары")
        if (
            set(барьер) != поля_открытого_барьера
            or барьер.get("состояние") != "открыт"
            or барьер.get("ветка") != квитанция["target_ref"]
            or барьер.get("базовая_вершина") != квитанция["target_oid"]
            or not точный_коммит(барьер.get("базовая_вершина"))
            or any(not корректная_задача(барьер.get(поле)) for поле in однострочные_поля)
            or any(not isinstance(барьер.get(поле), str) or re.fullmatch(r"sha256:[0-9a-f]{64}", барьер[поле]) is None for поле in хэшевые_поля)
            or any(not isinstance(барьер.get(поле), str) or re.fullmatch(полный_объект, барьер[поле]) is None for поле in ("связан_из_объекта", "открыт_из_объекта"))
            or not isinstance(пара, list)
            or len(пара) != 2
            or пара != sorted(set(пара))
            or any(not isinstance(хэш, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", хэш) is None for хэш in пара)
        ):
            return False
        ожидаемый_хэш_привязки = хэш_барьерного_объекта({
            "схема": "корневая-привязка-задачи.2", "поколение": барьер["поколение_запуска"],
            "исходная_вершина": барьер["базовая_вершина"], "форк": барьер["идентификатор_форка"],
            "попытка": барьер["идентификатор_попытки_создания"], "идентификатор_назначения": барьер["идентификатор_назначения"],
            "хэш_назначения": барьер["хэш_назначения"], "идентификатор_ресурсной_попытки": барьер["идентификатор_ресурсной_попытки"],
            "хэш_ресурсного_допуска": барьер["хэш_ресурсного_допуска"], "threadId": барьер["идентификатор_задачи"],
            "hostId": барьер["идентификатор_среды"], "хэш_конверта": барьер["хэш_конверта"],
        })
        ожидаемый_хэш_активации = хэш_барьерного_объекта({
            "схема": "корневая-активация-пары.2", "поколение": барьер["поколение_запуска"],
            "хэш_предактивации": барьер["хэш_предактивации"], "хэш_квитанции": барьер["квитанция_активации"],
            "хэши_привязок": пара,
        })
        if (
            барьер.get("хэш_привязки") != ожидаемый_хэш_привязки
            or ожидаемый_хэш_привязки not in пара
            or барьер.get("хэш_активации") != ожидаемый_хэш_активации
            or барьер.get("идентификатор_назначения") != участник.get("идентификатор_назначения_барьера")
            or барьер.get("хэш_назначения") != участник.get("хэш_назначения_барьера")
            or барьер.get("идентификатор_ресурсной_попытки") != участник.get("идентификатор_ресурсной_попытки_барьера")
            or барьер.get("хэш_ресурсного_допуска") != участник.get("хэш_ресурсного_допуска_барьера")
            or ("базовая_вершина_барьера" in участник and барьер.get("идентификатор_задачи") != участник.get("task_id"))
        ):
            return False
        прежний_объект = проверки_барьерных_реестров.get(ссылка_реестра)
        if прежний_объект is not None and прежний_объект != объект_реестра:
            return False
        проверки_барьерных_реестров[ссылка_реестра] = объект_реестра
        return True

    def корректное_барьерное_ограждение(значение: dict[str, Any], основные_поля: set[str]) -> bool:
        присутствующие = поля_барьерного_ограждения & set(значение)
        есть_база = поле_базы_барьера in значение
        ожидаемые_поля = основные_поля | (
            поля_барьерного_ограждения if присутствующие else set()
        ) | ({поле_базы_барьера} if есть_база else set())
        return (
            set(значение) == ожидаемые_поля
            and (not присутствующие or присутствующие == поля_барьерного_ограждения)
            and (not есть_база or присутствующие == поля_барьерного_ограждения)
            and (
                not присутствующие
                or all(
                    непустая_строка(значение.get(поле))
                    for поле in поля_барьерного_ограждения
                )
                and точный_блоб(значение.get("объект_реестра_барьеров"))
                and re.fullmatch(r"sha256:[0-9a-f]{64}", значение["хэш_назначения_барьера"]) is not None
                and re.fullmatch(r"sha256:[0-9a-f]{64}", значение["хэш_ресурсного_допуска_барьера"]) is not None
                and корректный_реестр_барьера(значение)
            )
            and (
                not есть_база
                or точный_коммит(значение.get(поле_базы_барьера))
                and значение.get(поле_базы_барьера) == квитанция["target_oid"]
            )
        )

    def корректный_владелец(значение: object) -> bool:
        return (
            isinstance(значение, dict)
            and корректное_барьерное_ограждение(значение, поля_владельца)
            and корректная_задача(значение.get("task_id"))
            and all(непустая_строка(значение.get(поле)) for поле in ("ticket_id", "generation", "admitted_at"))
            and положительное_целое(значение.get("seq"))
            and числовая_эпоха(значение.get("admitted_at_epoch"))
            and точный_коммит(значение.get("base_head"))
        )

    def корректный_ожидающий(значение: object) -> bool:
        return (
            isinstance(значение, dict)
            and корректное_барьерное_ограждение(значение, поля_ожидающего)
            and корректная_задача(значение.get("task_id"))
            and all(непустая_строка(значение.get(поле)) for поле in ("ticket_id", "registered_at"))
            and положительное_целое(значение.get("seq"))
            and числовая_эпоха(значение.get("registered_at_epoch"))
            and точный_коммит(значение.get("acknowledged_head"))
            and значение.get("acknowledged_head") == квитанция["target_oid"]
        )

    def корректное_завершение(значение: object) -> bool:
        if значение is None:
            return True
        if not isinstance(значение, dict):
            return False
        вид = значение.get("kind")
        основные = {"kind", "task_id", "generation", "head", "completed_at"}
        if вид == "finished_clean":
            поля = основные
        elif вид == "committed":
            поля = основные | {"base_head"}
            if "идентификатор_продолжения" in значение:
                поля.add("идентификатор_продолжения")
        elif вид == "reset":
            поля = основные | {"аннулированные_задачи"}
        else:
            return False
        if (
            set(значение) != поля
            or not корректная_задача(значение.get("task_id"))
            or any(not непустая_строка(значение.get(поле)) for поле in ("generation", "completed_at"))
            or not точный_коммит(значение.get("head"))
            or (вид == "committed" and not точный_коммит(значение.get("base_head")))
            or (
                вид == "reset"
                and re.fullmatch(r"sha256:[0-9a-f]{64}", значение["generation"]) is None
            )
            or (
                "идентификатор_продолжения" in значение
                and not корректная_задача(значение.get("идентификатор_продолжения"))
            )
        ):
            return False
        аннулированные = значение.get("аннулированные_задачи")
        return (
            вид != "reset"
            or isinstance(аннулированные, list)
            and all(корректная_задача(задача) for задача in аннулированные)
            and аннулированные == sorted(set(аннулированные))
        )

    текущая_цепочка = исходная_очередь.get("текущая_цепочка")
    допустимая_цепочка = (
        "текущая_цепочка" not in исходная_очередь
        or isinstance(текущая_цепочка, dict)
        and set(текущая_цепочка) == {"идентификатор", "путь", "хэш", "ветка"}
        and isinstance(текущая_цепочка.get("идентификатор"), str)
        and re.fullmatch(r"FUM-ЦЕПОЧКА-[0-9]{4}", текущая_цепочка["идентификатор"]) is not None
        and isinstance(текущая_цепочка.get("путь"), str)
        and bool(текущая_цепочка["путь"])
        and not any(знак in текущая_цепочка["путь"] for знак in ("\x00", "\n", "\r"))
        and not Path(текущая_цепочка["путь"]).is_absolute()
        and ".." not in Path(текущая_цепочка["путь"]).parts
        and Path(текущая_цепочка["путь"]).as_posix() == текущая_цепочка["путь"]
        and isinstance(текущая_цепочка.get("хэш"), str)
        and re.fullmatch(r"sha256:[0-9a-f]{64}", текущая_цепочка["хэш"]) is not None
        and текущая_цепочка.get("ветка") == квитанция["target_ref"]
    )
    свидетельство_барьера = исходная_очередь.get("допущенная_задача_барьера")
    владелец_имеет_базу_барьера = (
        isinstance(владелец, dict) and поле_базы_барьера in владелец
    )
    допустимое_свидетельство_барьера = (
        "допущенная_задача_барьера" not in исходная_очередь
        and not владелец_имеет_базу_барьера
        or isinstance(свидетельство_барьера, dict)
        and set(свидетельство_барьера) == {"task_id", "ticket_id", "seq", "объект_реестра_барьеров"}
        and корректная_задача(свидетельство_барьера.get("task_id"))
        and непустая_строка(свидетельство_барьера.get("ticket_id"))
        and положительное_целое(свидетельство_барьера.get("seq"))
        and точный_блоб(свидетельство_барьера.get("объект_реестра_барьеров"))
        and владелец_имеет_базу_барьера
        and свидетельство_барьера
        == {
            "task_id": владелец.get("task_id"),
            "ticket_id": владелец.get("ticket_id"),
            "seq": владелец.get("seq"),
            "объект_реестра_барьеров": владелец.get("объект_реестра_барьеров"),
        }
    )
    последовательности = (
        [владелец.get("seq"), *(билет.get("seq") for билет in ожидающие)]
        if isinstance(владелец, dict) and isinstance(ожидающие, list)
        and all(isinstance(билет, dict) for билет in ожидающие)
        else []
    )
    задачи_FIFO = (
        [владелец.get("task_id"), *(билет.get("task_id") for билет in ожидающие)]
        if isinstance(владелец, dict) and isinstance(ожидающие, list)
        and all(isinstance(билет, dict) for билет in ожидающие)
        else []
    )
    билеты_FIFO = (
        [владелец.get("ticket_id"), *(билет.get("ticket_id") for билет in ожидающие)]
        if isinstance(владелец, dict) and isinstance(ожидающие, list)
        and all(isinstance(билет, dict) for билет in ожидающие)
        else []
    )
    if (
        not основные_поля_очереди <= set(исходная_очередь) <= допустимые_поля_очереди
        or type(исходная_очередь.get("schema_version")) is not int
        or исходная_очередь["schema_version"] != 1
        or not корректный_владелец(владелец)
        or not isinstance(ожидающие, list)
        or not ожидающие
        or any(not корректный_ожидающий(билет) for билет in ожидающие)
        or not непустая_строка(идентификатор_рабочего_дерева)
        or not непустая_строка(исходная_очередь.get("updated_at"))
        or not корректное_завершение(исходная_очередь.get("last_completion"))
        or not допустимая_цепочка
        or not допустимое_свидетельство_барьера
        or (
            "обязательное_продолжение_активировано" in исходная_очередь
            and исходная_очередь["обязательное_продолжение_активировано"] is not True
        )
        or not последовательности
        or последовательности != sorted(последовательности)
        or len(последовательности) != len(set(последовательности))
        or len(задачи_FIFO) != len(set(задачи_FIFO))
        or len(билеты_FIFO) != len(set(билеты_FIFO))
        or not положительное_целое(исходная_очередь.get("next_seq"))
        or исходная_очередь["next_seq"] <= последовательности[-1]
        or исходная_очередь.get("branch_ref") != квитанция["target_ref"]
        or квитанция["queue_ref"] != f"refs/fum/worktree-task-queues/{идентификатор_рабочего_дерева}"
        or владелец.get("task_id") != квитанция["task_id"]
        or владелец.get("generation") != квитанция["generation"]
        or владелец.get("ticket_id") != квитанция["owner_ticket_id"]
        or владелец.get("seq") != квитанция["owner_seq"]
        or владелец.get("base_head") != квитанция["target_oid"]
        or ожидающие[0].get("task_id") != квитанция["continuation_task_id"]
        or ожидающие[0].get("ticket_id") != квитанция["first_continuation_ticket_id"]
        or ожидающие[0].get("seq") != квитанция["first_continuation_seq"]
        or ожидающие[0].get("acknowledged_head") != квитанция["target_oid"]
        or исходная_очередь.get("next_seq") != квитанция["next_seq"]
        or хэш_объекта({"waiting": ожидающие}) != квитанция["waiting_hash"]
    ):
        raise ОшибкаПула(65, "migration_activation_fifo_mismatch", "C0 receipt не доказала exact source-queue/FIFO.")

    проверки_доказательств: list[tuple[str, str]] = []
    прочитать_закреплённые_квитанции_миграции(
        контекст,
        проверки_ссылок=проверки_доказательств,
    )
    проверки_миграции_до = list(проверки_доказательств)

    def найти_единственное_назначение(хэш_назначения: object) -> dict[str, Any] | None:
        совпадения = [
            запись
            for запись in исходный_пул.get("assignments", {}).values()
            if isinstance(запись, dict)
            and запись.get("assignment_hash") == хэш_назначения
        ]
        if len(совпадения) != 1:
            return None
        try:
            проверить_неизменяемое_назначение_и_активацию(
                исходный_пул,
                совпадения[0],
                состояние="migration_activation_assignment_invalid",
            )
        except ОшибкаПула:
            return None
        return совпадения[0]

    def корректное_принятое_ревью(
        хэш_ревью: object,
        вид_объекта: str,
        ...
        запись_ревью = исходный_пул.get("reviews", {}).get(хэш_ревью)
        назначение_рецензента = (
            найти_единственное_назначение(запись_ревью.get("reviewer_assignment_hash"))
            if isinstance(запись_ревью, dict)
            else None
        )
        проверки = запись_ревью.get("checks") if isinstance(запись_ревью, dict) else None
        if (
            not isinstance(хэш_ревью, str)
            or re.fullmatch(r"sha256:[0-9a-f]{64}", хэш_ревью) is None
            or not isinstance(запись_ревью, dict)
            or set(запись_ревью) != ПОЛЯ_КВИТАНЦИИ_РЕВЬЮ
            or запись_ревью.get("schema") != СХЕМА_КВИТАНЦИИ_РЕВЬЮ
            or хэш_объекта(запись_ревью) != хэш_ревью
            or запись_ревью.get("verdict") != "принято"
            or not isinstance(проверки, list)
            or not проверки
            or any(not isinstance(проверка, str) or not проверка for проверка in проверки)
            or len(проверки) != len(set(проверки))
            or "публикационная чистота" not in проверки
            or not isinstance(запись_ревью.get("report_sha256"), str)
            or re.fullmatch(r"sha256:[0-9a-f]{64}", запись_ревью["report_sha256"]) is None
            or запись_ревью.get("reviewed_object_kind") != вид_объекта
            or запись_ревью.get("reviewed_object_hash") != хэш_объекта_ревью
            or запись_ревью.get("reviewed_head_oid") != вершина_объекта
            or запись_ревью.get("reviewed_branch_ref") != ссылка_объекта
            or not isinstance(назначение_рецензента, dict)
            or назначение_рецензента.get("id") != запись_ревью.get("reviewer_assignment_id")
            or назначение_рецензента.get("role") != "рецензент"
            or назначение_рецензента.get("base_oid") != вершина_объекта
            or назначение_рецензента.get("target_ref") != квитанция["target_ref"]
            or назначение_рецензента.get("registered_task_id") != запись_ревью.get("task_id")
            or назначение_рецензента.get("host_id") != запись_ревью.get("host_id")
            or назначение_рецензента.get("status") != "review_recorded"
            or назначение_рецензента.get("review_hash") != хэш_ревью
        ):
            return None
        return запись_ревью

    исходный_кандидат = исходный_пул.get("integration_candidates", {}).get(квитанция["candidate_hash"])
    ожидаемые_поля_кандидата = ОСНОВНЫЕ_ПОЛЯ_КАНДИДАТА_ИНТЕГРАЦИИ | {
        "хэши_намерений_коммитов", "интеграционные_коммиты",
    }
    назначение_интегратора = (
        найти_единственное_назначение(исходный_кандидат.get("integrator_assignment_hash"))
        if isinstance(исходный_кандидат, dict)
        else None
    )
    try:
        if (
            not isinstance(исходный_кандидат, dict)
            or исходный_кандидат.get("schema") != СХЕМА_КВИТАНЦИИ_КАНДИДАТА
            or set(исходный_кандидат) != ожидаемые_поля_кандидата
            or хэш_объекта(исходный_кандидат) != квитанция["candidate_hash"]
            or исходный_кандидат.get("target_ref") != квитанция["target_ref"]
            or исходный_кандидат.get("base_oid") != квитанция["target_oid"]
            or исходный_кандидат.get("head_oid") != квитанция["candidate_head_oid"]
            or type(исходный_кандидат.get("conflict_resolved")) is not bool
            or not isinstance(назначение_интегратора, dict)
            or назначение_интегратора.get("id") != исходный_кандидат.get("integrator_assignment_id")
            or назначение_интегратора.get("role") != "интегратор"
            or назначение_интегратора.get("registered_task_id") != исходный_кандидат.get("task_id")
            or назначение_интегратора.get("host_id") != исходный_кандидат.get("host_id")
            or назначение_интегратора.get("branch_ref") != исходный_кандидат.get("branch_ref")
            or назначение_интегратора.get("target_ref") != исходный_кандидат.get("target_ref")
            or назначение_интегратора.get("remote") != исходный_кандидат.get("remote")
            or назначение_интегратора.get("base_oid") != исходный_кандидат.get("base_oid")
            or назначение_интегратора.get("status") != "integration_candidate"
            or назначение_интегратора.get("integration_candidate_hash") != квитанция["candidate_hash"]
        ):
            raise ValueError
        база_кандидата = проверить_идентификатор_объекта(контекст, исходный_кандидат["base_oid"])
        вершина_кандидата = проверить_идентификатор_объекта(контекст, исходный_кандидат["head_oid"])
        ссылка_кандидата = исходный_кандидат.get("branch_ref")
        if (
            not isinstance(ссылка_кандидата, str)
            or not ссылка_кандидата.startswith("refs/heads/")
            or прочитать_ссылку(контекст, ссылка_кандидата) != вершина_кандидата
        ):
            raise ValueError
        проверки_доказательств.append((ссылка_кандидата, вершина_кандидата))
        ...
        записи_слияний = исходный_кандидат.get("интеграционные_коммиты")
        хэши_намерений = исходный_кандидат.get("хэши_намерений_коммитов")
        if (
            ...
            or not isinstance(записи_слияний, list)
            or not isinstance(хэши_намерений, list)
            or not (len(хэши_результатов) == len(хэши_ревью_результатов) == len(головы_результатов) == len(записи_слияний) == len(хэши_намерений))
        ):
            raise ValueError
        проверить_план_интеграции(исходный_пул, хэши_результатов, хэши_ревью_результатов)
        предыдущая_вершина = база_кандидата
        конфликты: list[bool] = []
        for индекс, (хэш_результата, хэш_ревью_результата, голова_результата, запись_слияния, хэш_намерения) in enumerate(zip(
            хэши_результатов, хэши_ревью_результатов, головы_результатов, записи_слияний, хэши_намерений,
        )):
            проверить_результат_перед_каждым_слиянием(контекст, исходный_пул, хэш_результата)
            результат = исходный_пул["results"].get(хэш_результата)
            if (
                not isinstance(результат, dict)
                or результат.get("head_oid") != голова_результата
                or not isinstance(результат.get("branch_ref"), str)
                or прочитать_ссылку(контекст, результат["branch_ref"]) != голова_результата
                or корректное_принятое_ревью(
                    хэш_ревью_результата, "result", хэш_результата, голова_результата, результат["branch_ref"],
                ) is None
                or not isinstance(запись_слияния, dict)
                or set(запись_слияния) != {"индекс_результата", "хэш_результата", "commit_oid", "хэш_намерения_коммита", "конфликт_разрешён"}
                or type(запись_слияния.get("индекс_результата")) is not int
                or запись_слияния["индекс_результата"] != индекс
                or запись_слияния.get("хэш_результата") != хэш_результата
                or запись_слияния.get("хэш_намерения_коммита") != хэш_намерения
                or type(запись_слияния.get("конфликт_разрешён")) is not bool
            ):
                raise ValueError
            намерение = получить_намерение_коммита(
                контекст, назначение_интегратора, "интеграционное_слияние", индекс_результата=индекс,
            )
            сообщение = (
                f"Разрешить конфликт результата {хэш_результата}\n"
                if запись_слияния["конфликт_разрешён"]
                else f"Интегрировать результат {хэш_результата}\n"
            )
            if (
                хэш_объекта(намерение) != хэш_намерения
                or намерение.get("task_id") != исходный_кандидат.get("task_id")
                or намерение.get("branch_ref") != назначение_интегратора.get("branch_ref")
                or намерение.get("родители") != [предыдущая_вершина, голова_результата]
                or намерение.get("хэш_сообщения") != хэш_сообщения_коммита(сообщение)
            ):
                raise ValueError
            коммит_слияния = проверить_идентификатор_объекта(контекст, запись_слияния.get("commit_oid"))
            проверить_коммит_по_намерению(контекст.основной_корень, коммит_слияния, намерение, сообщение)
            if выполнить_команду_версий(
                контекст.основной_корень, ["merge-base", "--is-ancestor", голова_результата, вершина_кандидата], проверять=False,
            ).код != 0:
                raise ValueError
            проверки_доказательств.append((результат["branch_ref"], голова_результата))
            предыдущая_вершина = коммит_слияния
            конфликты.append(запись_слияния["конфликт_разрешён"])
        фактические_коммиты = текст_команды_версий(
            контекст.основной_корень, ["rev-list", "--reverse", f"{база_кандидата}..{вершина_кандидата}"],
        ).splitlines()
        if (
            not фактические_коммиты
            or исходный_кандидат.get("commits") != фактические_коммиты
            or предыдущая_вершина != вершина_кандидата
            or any(конфликты) != исходный_кандидат.get("conflict_resolved")
        ):
            raise ValueError
    except (ОшибкаПула, ValueError, TypeError, KeyError) as ошибка:
        raise ОшибкаПула(65, "migration_activation_candidate_invalid", "C0 не связана с полным происхождением кандидата, результатов и намерений коммитов.") from ошибка

    ревью = корректное_принятое_ревью(
        квитанция["review_hash"], "integration_candidate", квитанция["candidate_hash"],
        квитанция["candidate_head_oid"], исходный_кандидат["branch_ref"],
    )
    if ревью is None:
        raise ОшибкаПула(65, "migration_activation_review_invalid", "C0 не имеет exact принятого durable-ревью чистоты с assignment provenance.")
    потребовать_отсутствие_блокирующего_ревью(
        исходный_пул,
        "integration_candidate",
        квитанция["candidate_hash"],
        "integration_candidate_blocked_by_review",
    )
    for хэш_результата in исходный_кандидат["result_hashes"]:
        потребовать_отсутствие_блокирующего_ревью(
            исходный_пул,
            "result",
            хэш_результата,
    ...
    идентичность_обычной_квитанции = {
        ...
        "ссылка_ветки": квитанция["target_ref"],
        "идентификатор_задачи": квитанция["task_id"],
        "поколение": квитанция["generation"],
    }
    отпечаток = hashlib.sha256(канонические_байты(идентичность_обычной_квитанции)).hexdigest()
    ссылка_обычной_квитанции = f"{ПРОСТРАНСТВО_КВИТАНЦИЙ_ПРИНЯТЫХ_ИНТЕГРАЦИЙ}/{идентификатор_рабочего_дерева}/{отпечаток}"
    идентификатор_обычной_квитанции = прочитать_ссылку(контекст, ссылка_обычной_квитанции)
    if идентификатор_обычной_квитанции is None:
        raise ОшибкаПула(65, "migration_activation_integration_receipt_missing", "Обычная durable-квитанция C0-интеграции отсутствует.")
    обычная_квитанция = прочитать_канонический_блоб_по_идентификатору(
        контекст,
        проверить_идентификатор_блоба(контекст, идентификатор_обычной_квитанции),
    )
    поля_обычной_квитанции = {
        "схема", "идентификатор_рабочего_дерева", "ссылка_ветки", "идентификатор_задачи", "поколение",
        "исходная_вершина", "новая_вершина", "идентификатор_продолжения", "ссылка_пула", "исходный_объект_пула",
        "новый_объект_пула", "хэш_интеграции",
    }
    if (
        set(обычная_квитанция) != поля_обычной_квитанции
        or обычная_квитанция.get("схема") != СХЕМА_КВИТАНЦИИ_ПРИНЯТОЙ_ИНТЕГРАЦИИ
        or обычная_квитанция.get("идентификатор_рабочего_дерева") != идентификатор_рабочего_дерева
        or обычная_квитанция.get("ссылка_ветки") != квитанция["target_ref"]
        or обычная_квитанция.get("идентификатор_задачи") != квитанция["task_id"]
        or обычная_квитанция.get("поколение") != квитанция["generation"]
        or обычная_квитанция.get("исходная_вершина") != квитанция["target_oid"]
        or обычная_квитанция.get("новая_вершина") != квитанция["candidate_head_oid"]
        or обычная_квитанция.get("идентификатор_продолжения") != квитанция["continuation_task_id"]
        or обычная_квитанция.get("ссылка_пула") != контекст.ссылка_пула
        or обычная_квитанция.get("исходный_объект_пула") != исходный_объект_пула
        or обычная_квитанция.get("хэш_интеграции") != квитанция["integration_hash"]
    ):
        raise ОшибкаПула(65, "migration_activation_integration_receipt_invalid", "Обычная durable-квитанция C0-интеграции повреждена.")
    if not isinstance(обычная_квитанция.get("новый_объект_пула"), str):
        raise ОшибкаПула(65, "migration_activation_integration_receipt_invalid", "Обычная C0-квитанция не имеет exact new pool blob.")
    объект_активированного_пула = проверить_идентификатор_блоба(
        контекст,
        обычная_квитанция["новый_объект_пула"],
    )
    активированный_пул = прочитать_канонический_блоб_по_идентификатору(контекст, объект_активированного_пула)
    проверить_состояние_пула(контекст, активированный_пул)
    интеграция = активированный_пул.get("integrations", {}).get(квитанция["integration_hash"])
    ожидаемая_квитанция_интеграции = {
        "schema": СХЕМА_КВИТАНЦИИ_ИНТЕГРАЦИИ,
        "integration_candidate_hash": квитанция["candidate_hash"],
        "review_hash": квитанция["review_hash"],
        "task_id": квитанция["task_id"],
        "generation": квитанция["generation"],
        "continuation_task_id": квитанция["continuation_task_id"],
        "target_ref": квитанция["target_ref"],
        "remote": исходный_кандидат["remote"],
        "base_oid": квитанция["target_oid"],
        "head_oid": квитанция["candidate_head_oid"],
        "result_hashes": исходный_кандидат["result_hashes"],
        "result_heads": исходный_кандидат["result_heads"],
    }
    ожидаемый_активированный_кандидат = {**исходный_кандидат, "integration_hash": квитанция["integration_hash"]}
    ключ_публикации = ключ_публикации_интеграции(
        квитанция["integration_hash"], исходный_кандидат["remote"], квитанция["target_ref"]
    )
    ожидаемое_намерение_публикации = {
        "schema": СХЕМА_НАМЕРЕНИЯ_ПУБЛИКАЦИИ_ИНТЕГРАЦИИ,
        "status": "publication_pending", "integration_hash": квитанция["integration_hash"],
        "review_hash": квитанция["review_hash"], "remote": исходный_кандидат["remote"],
        "remote_ref": квитанция["target_ref"], "base_oid": квитанция["target_oid"],
        "head_oid": квитанция["candidate_head_oid"], "remote_url_sha256": None,
    }
    ожидаемые_кандидаты = copy.deepcopy(исходный_пул["integration_candidates"])
    ожидаемые_кандидаты[квитанция["candidate_hash"]] = ожидаемый_активированный_кандидат
    ожидаемые_интеграции = copy.deepcopy(исходный_пул["integrations"])
    ожидаемые_интеграции[квитанция["integration_hash"]] = ожидаемая_квитанция_интеграции
    ожидаемые_публикации = copy.deepcopy(исходный_пул["publications"])
    ожидаемые_публикации[ключ_публикации] = ожидаемое_намерение_публикации
    неизменяемые_поля_пула = set(исходный_пул) - {
        "revision", "integration_candidates", "integrations", "publications", "миграция_дат_коммитов",
    }
    if (
        активированный_пул.get("миграция_дат_коммитов") != миграция
        or активированный_пул.get("revision") != исходный_пул.get("revision") + 1
        or any(активированный_пул[поле] != исходный_пул[поле] for поле in неизменяемые_поля_пула)
        or активированный_пул.get("integration_candidates") != ожидаемые_кандидаты
        or активированный_пул.get("integrations") != ожидаемые_интеграции
        or активированный_пул.get("publications") != ожидаемые_публикации
        or интеграция != ожидаемая_квитанция_интеграции
        or хэш_объекта(интеграция) != квитанция["integration_hash"]
    ):
        raise ОшибкаПула(65, "migration_activation_pool_transition_invalid", "Durable активированный пул не совпал с C0 provenance.")
    if (
        пул.get("миграция_дат_коммитов") != миграция
        or пул.get("integration_candidates", {}).get(квитанция["candidate_hash"]) != ожидаемый_активированный_кандидат
        or пул.get("reviews", {}).get(квитанция["review_hash"]) != ревью
        or пул.get("integrations", {}).get(квитанция["integration_hash"]) != интеграция
    ):
        raise ОшибкаПула(65, "migration_activation_provenance_lost", "Текущий пул потерял durable C0 candidate/review/integration provenance.")
    проверки_миграции_после: list[tuple[str, str]] = []
    прочитать_закреплённые_квитанции_миграции(
        контекст,
        проверки_ссылок=проверки_миграции_после,
    )
    if проверки_миграции_после != проверки_миграции_до:
        raise ОшибкаПула(75, "migration_activation_durable_ref_changed", "Durable-ссылки миграции сдвинулись во время проверки C0.")
    return квитанция, [
        (ссылка_квитанции, идентификатор_квитанции),
        (ссылка_обычной_квитанции, идентификатор_обычной_квитанции),
        *sorted(проверки_барьерных_реестров.items()),
        *проверки_доказательств,
    ]


def назначение_ожидает_подтверждения_протокола(
    контекст: КонтекстПула,
    пул: dict[str, Any],
    назначение: dict[str, Any],
    *,
    ...
    задача = назначение.get("registered_task_id")
    маршрут = пул.get("session_routes", {}).get(задача)
    очередь, идентификатор_очереди = прочитать_объект_состояния(контекст, назначение["queue_ref"])
    if not isinstance(очередь, dict):
        return False
    if идентификатор_очереди is None:
        raise ОшибкаПула(65, "migration_protocol_reload_identity_mismatch", "Protocol reload не имеет exact FIFO-объекта.")
    if проверки_ссылок is not None:
        проверки_ссылок.append((назначение["queue_ref"], идентификатор_очереди))
    проверить_очередь(очередь, назначение)
    владелец = очередь.get("owner")
    признаки = (
        isinstance(маршрут, dict) and маршрут.get("status") == "protocol_reload_required",
        очередь.get("status") == "reload_required",
        isinstance(владелец, dict) and bool(владелец.get("reload_required")),
    )
    if not any(признаки):
        return False
    if not all(признаки) or not isinstance(владелец, dict) or владелец.get("task_id") != задача:
        raise ОшибкаПула(65, "migration_protocol_reload_identity_mismatch", "Признаки exact protocol reload не совпали.")
    return True


def проверить_актуальность_рецензента_после_миграции(
    контекст: КонтекстПула,
    пул: dict[str, Any],
    назначение: dict[str, Any],
) -> list[tuple[str, str]]:
    if (
        назначение.get("role") != "рецензент"
        or пул.get("schema") != СХЕМА_ПУЛА
    ):
        return []
    проверки_квитанций: list[tuple[str, str]] = []
    квитанции = прочитать_закреплённые_квитанции_миграции(
        контекст,
        проверки_ссылок=проверки_квитанций,
    )
    исходная_вершина = назначение.get("base_oid")
    замещающие_вершины = {
        квитанция["oid_mapping"][исходная_вершина]
        for квитанция in квитанции
        if исходная_вершина in квитанция["oid_mapping"]
        and квитанция["oid_mapping"][исходная_вершина] != исходная_вершина
    }
    if len(замещающие_вершины) > 1:
        raise ОшибкаПула(
            65,
            "migration_oid_mapping_ambiguous",
            "Исходная вершина рецензента имеет конфликтующие durable-замещения.",
        )
    if замещающие_вершины:
        замещающая_вершина = next(iter(замещающие_вершины))
        raise ОшибкаПула(
            73,
            "stale_reviewer_assignment_after_migration",
            "Назначение рецензента закрепляет замещённый миграцией объект; требуется новое назначение.",
            исходная_вершина=исходная_вершина,
            замещающая_вершина=замещающая_вершина,
        )
    return объединить_проверки_ссылок(проверки_квитанций)


def присутствует_идентичность_закреплённой_загрузки() -> bool:
    пространство = globals()
    return (
        "_ИДЕНТИЧНОСТЬ_ЗАКРЕПЛЁННОЙ_ЗАГРУЗКИ" in пространство
        or any(имя in пространство for имя in ("r", "o", "b"))
    )


def доступны_проверяемые_байты_исполнения() -> bool:
    if присутствует_идентичность_закреплённой_загрузки():
        return True
    путь = globals().get("__file__")
    return isinstance(путь, str) and Path(путь).is_file()


def проверить_закреплённую_ревизию_исполнения(
    контекст: КонтекстПула,
    пул: dict[str, Any],
    идентификатор_объекта_пула: str | None,
    назначение: dict[str, Any],
    *,
    ревизия_явного_подтверждения: str | None = None,
) -> tuple[str, list[tuple[str, str]]]:
    if идентификатор_объекта_пула is None:
        raise ОшибкаПула(65, "assignment_execution_pool_missing", "Назначение не имеет exact объекта пула.")
    if пул.get("assignments", {}).get(назначение.get("id")) != назначение:
        raise ОшибкаПула(65, "assignment_execution_identity_mismatch", "Назначение не принадлежит exact снимку пула.")
    текущая_вершина_цели = прочитать_ссылку(контекст, назначение["target_ref"])
    if текущая_вершина_цели is None:
        raise ОшибкаПула(73, "assignment_execution_target_missing", "Цель допуска исчезла.")
    ожидаемая_вершина_ветки = ожидаемая_вершина_рабочего_дерева(назначение)
    if прочитать_ссылку(контекст, назначение["branch_ref"]) != ожидаемая_вершина_ветки:
        raise ОшибкаПула(73, "assignment_execution_branch_changed", "Рабочая ссылка назначения сдвинулась до допуска.")
    проверки: list[tuple[str, str]] = [
        (контекст.ссылка_пула, идентификатор_объекта_пула),
        (назначение["target_ref"], текущая_вершина_цели),
        (назначение["branch_ref"], ожидаемая_вершина_ветки),
    ]
    проверки.extend(проверить_актуальность_рецензента_после_миграции(контекст, пул, назначение))
    текущая_ревизия_исполнения = эффективная_ревизия_протокола_до_активации(
        контекст,
        назначение["target_ref"],
        назначение["protocol_oid"],
        ожидаемая_вершина_цели=текущая_вершина_цели,
        ожидаемый_объект_пула=идентификатор_объекта_пула,
        назначение=назначение,
        проверки_ссылок=проверки,
    )
    проверяемая_ревизия_исполнения = текущая_ревизия_исполнения
    if ревизия_явного_подтверждения is not None:
        миграция = пул.get("миграция_дат_коммитов")
        if (
            not isinstance(миграция, dict)
            or миграция.get("status") not in {"completed", "activated"}
            or миграция.get("target_ref") != назначение["target_ref"]
            or миграция.get("tool_oid") != ревизия_явного_подтверждения
        ):
            raise ОшибкаПула(
                73,
                "assignment_execution_revision_changed",
                "Явное подтверждение протокола не совпало с exact migration tool_oid.",
            )
        проверяемая_ревизия_исполнения = проверить_идентификатор_объекта(
            контекст,
            ревизия_явного_подтверждения,
        )
    путь_сценария = "Инструменты/fum-ocheredj-zadach-git-vetki/scripts/пул-worktree-подузлов.py"
    if присутствует_идентичность_закреплённой_загрузки():
        идентичность = получить_идентичность_закреплённой_загрузки()
        if идентичность.канонический_корень != str(контекст.вызванный_корень):
            raise ОшибкаПула(73, "assignment_bootstrap_repo_root_mismatch", "Repo-root допуска не совпал с закреплённой начальной загрузкой.")
        результат_сценария = выполнить_команду_версий(
            контекст.основной_корень,
            ["show", f"{идентичность.ревизия_протокола}:{путь_сценария}"],
            проверять=False,
        )
        if результат_сценария.код != 0 or результат_сценария.вывод != идентичность.байты_сценария:
            raise ОшибкаПула(73, "assignment_bootstrap_script_mismatch", "Исполняемые байты допуска не совпали с exact ревизией.")
        if идентичность.ревизия_протокола != проверяемая_ревизия_исполнения:
            raise ОшибкаПула(
                73,
                "assignment_execution_revision_changed",
                "Ревизия исполнения изменилась после выдачи production-промпта.",
                закреплённая_ревизия=идентичность.ревизия_протокола,
                текущая_ревизия=проверяемая_ревизия_исполнения,
            )
    elif доступны_проверяемые_байты_исполнения():
        путь_исполняемого_сценария = Path(str(globals()["__file__"]))
        try:
            исполняемые_байты = путь_исполняемого_сценария.read_bytes()
        except OSError as ошибка:
            raise ОшибкаПула(73, "assignment_direct_script_unreadable", "Байты прямо вызванного сценария недоступны.") from ошибка
        результат_сценария = выполнить_команду_версий(
            контекст.основной_корень,
            ["show", f"{проверяемая_ревизия_исполнения}:{путь_сценария}"],
            проверять=False,
        )
        if результат_сценария.код != 0 or результат_сценария.вывод != исполняемые_байты:
            raise ОшибкаПула(
                73,
                "assignment_direct_script_mismatch",
                "Прямо исполняемые байты не совпали с текущей effective revision.",
            )
    объединённые_проверки = объединить_проверки_ссылок(проверки)
    for ссылка, объект in объединённые_проверки:
        if прочитать_ссылку(контекст, ссылка) == объект:
            continue
        if ссылка == контекст.ссылка_пула:
            состояние = "assignment_execution_pool_changed"
        elif ссылка == назначение["target_ref"]:
            состояние = "assignment_execution_target_changed"
        elif ссылка == назначение["branch_ref"]:
            состояние = "assignment_execution_branch_changed"
        else:
            состояние = "assignment_execution_durable_ref_changed"
        raise ОшибкаПула(73, состояние, "Execution-снимок durable ссылок сдвинулся до CAS.", ссылка=ссылка)
    return проверяемая_ревизия_исполнения, объединённые_проверки
...
def получить_идентичность_закреплённой_загрузки() -> ИдентичностьЗагрузкиЯдраМиграции:
    значение = globals().get("_ИДЕНТИЧНОСТЬ_ЗАКРЕПЛЁННОЙ_ЗАГРУЗКИ")
    if значение is None:
        пространство = globals()
        присутствуют = tuple(имя in пространство for имя in ("r", "o", "b"))
        if not any(присутствуют):
            raise ОшибкаПула(
                73,
                "migration_bootstrap_identity_missing",
                "Команда миграции требует идентичность закреплённой начальной загрузки.",
            )
        if присутствуют == (True, False, True):
            корень = пространство["r"]
            байты_сценария = пространство["b"]
            if not isinstance(корень, str) or not isinstance(байты_сценария, bytes):
                raise ОшибкаПула(
                    73,
                    "migration_bootstrap_identity_invalid",
                    "Документированная HEAD-загрузка имеет неверные типы r/b.",
                )
            канонический_корень = str(Path(корень).expanduser().resolve())
            результат_HEAD = выполнить_команду_версий(
                Path(канонический_корень),
                ["rev-parse", "--verify", "HEAD^{commit}"],
                проверять=False,
            )
            try:
                ревизия_HEAD = результат_HEAD.вывод.decode("ascii", errors="strict").strip()
            except UnicodeDecodeError as ошибка:
                raise ОшибкаПула(
                    73,
                    "migration_bootstrap_identity_invalid",
                    "Документированная HEAD-загрузка не восстановила exact commit OID.",
                ) from ошибка
            if результат_HEAD.код != 0 or re.fullmatch(r"[0-9a-f]+", ревизия_HEAD) is None:
                raise ОшибкаПула(
                    73,
                    "migration_bootstrap_identity_invalid",
                    "Документированная HEAD-загрузка не восстановила exact commit OID.",
            ...
            результат_сценария = выполнить_команду_версий(
                Path(канонический_корень),
                ["show", f"{ревизия_HEAD}:{путь_сценария}"],
                проверять=False,
            )
            if результат_сценария.код != 0 or результат_сценария.вывод != байты_сценария:
                raise ОшибкаПула(
                    73,
                    "migration_bootstrap_script_mismatch",
                    "Байты документированной HEAD-загрузки не совпали с exact HEAD.",
                )
            значение = (
                канонический_корень,
                ревизия_HEAD,
        ...
        elif not all(присутствуют):
            raise ОшибкаПула(
                73,
                "migration_bootstrap_identity_invalid",
                "Идентичность закреплённой начальной загрузки неполна.",
            )
        else:
            корень = пространство["r"]
            ревизия = пространство["o"]
            байты_сценария = пространство["b"]
            if not isinstance(корень, str):
                raise ОшибкаПула(73, "migration_bootstrap_identity_invalid", "Корень исторической начальной загрузки имеет неверный тип.")
            значение = (
                str(Path(корень).expanduser().resolve()),
                ревизия,
                байты_сценария,
                None,
            )
    if (
        not isinstance(значение, tuple)
        or len(значение) != 4
        or not isinstance(значение[0], str)
        or not isinstance(значение[1], str)
        or not isinstance(значение[2], bytes)
        or (значение[3] is not None and not isinstance(значение[3], bytes))
    ):
        raise ОшибкаПула(
            73,
            "migration_bootstrap_identity_invalid",
            "Идентичность закреплённой начальной загрузки имеет неверную форму.",
        )
    канонический_корень = str(Path(значение[0]).expanduser().resolve())
    if значение[0] != канонический_корень:
        raise ОшибкаПула(
            73,
            "migration_bootstrap_repo_root_mismatch",
            "Идентичность закрепляет неканонический repo-root.",
        )
    return ИдентичностьЗагрузкиЯдраМиграции(
        канонический_корень=канонический_корень,
        ревизия_протокола=значение[1],
        байты_сценария=значение[2],
        байты_ядра=значение[3],
    )


def создать_идентичность_прямого_автономного_запуска(
    контекст: КонтекстПула,
    ревизия: str,
) -> ИдентичностьЗагрузкиЯдраМиграции:
    путь_сценария = globals().get("__file__")
    if not isinstance(путь_сценария, str):
        raise ОшибкаПула(
            73,
            "migration_autonomous_script_path_missing",
            "Прямой автономный запуск не имеет пути сценария.",
        )
    файл_сценария = Path(путь_сценария).resolve()
    файл_ядра = файл_сценария.with_name("миграция_дат_коммитов_worktree_пула.py")
    try:
        байты_сценария = файл_сценария.read_bytes()
        байты_ядра = файл_ядра.read_bytes()
    except OSError as ошибка:
        raise ОшибкаПула(
            73,
            "migration_autonomous_files_missing",
            "Прямая автономная фикстура не прочитала сценарий и ядро.",
        ) from ошибка
    return ИдентичностьЗагрузкиЯдраМиграции(
        канонический_корень=str(контекст.вызванный_корень),
        ревизия_протокола=ревизия,
        байты_сценария=байты_сценария,
        байты_ядра=байты_ядра,
    )


def загрузить_ядро_миграции_дат(
    контекст: КонтекстПула,
    ревизия: str,
    идентичность: ИдентичностьЗагрузкиЯдраМиграции,
) -> types.ModuleType:
    if идентичность.канонический_корень != str(контекст.вызванный_корень):
        raise ОшибкаПула(
            73,
            "migration_bootstrap_repo_root_mismatch",
            "Repo-root команды не совпал с закреплённой идентичностью.",
        )
    if идентичность.ревизия_протокола != ревизия:
        raise ОшибкаПула(
            73,
            "migration_bootstrap_revision_mismatch",
            "Ревизия команды не совпала с protocol_oid начальной загрузки.",
        )
    try:
        ...
        raise ОшибкаПула(
            73,
            "migration_bootstrap_revision_mismatch",
            "Protocol_oid начальной загрузки не является exact-коммитом этого репозитория.",
        ) from ошибка
    ...
    результат_пула = выполнить_команду_версий(
        контекст.основной_корень,
        ["show", f"{ревизия}:{путь_пула}"],
        проверять=False,
    )
    if результат_пула.код != 0 or идентичность.байты_сценария != результат_пула.вывод:
        raise ОшибкаПула(
            73,
            "migration_bootstrap_script_mismatch",
            "Исполняемые байты пула не совпали с exact protocol snapshot.",
        )
    результат_ядра = выполнить_команду_версий(
        контекст.основной_корень,
        ["show", f"{ревизия}:{путь_ядра}"],
        проверять=False,
    )
    if (
        результат_ядра.код != 0
        or (
            идентичность.байты_ядра is not None
            and идентичность.байты_ядра != результат_ядра.вывод
        )
    ):
        raise ОшибкаПула(
            73,
            "migration_bootstrap_core_mismatch",
            "Байты ядра не совпали с exact protocol snapshot.",
        )
    доверенные_байты_ядра = результат_ядра.вывод
    модуль = types.ModuleType("ядро_миграции_дат")
    exec(
        compile(доверенные_байты_ядра, f"{ревизия}:{путь_ядра}", "exec"),
        модуль.__dict__,
    )
    return модуль
...
def корневая_квитанция_результата(результат: dict[str, Any]) -> dict[str, Any]:
    текущая = результат
    виденные: set[str] = set()
    while текущая.get("schema") == СХЕМА_МИГРИРОВАННОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        отпечаток = хэш_объекта(текущая)
        прежняя = текущая.get("предыдущая_квитанция")
        if отпечаток in виденные or not isinstance(прежняя, dict):
            raise ОшибкаПула(65, "migrated_result_receipt_mismatch", "Цепочка previous receipt мигрированного результата повреждена.")
        виденные.add(отпечаток)
        текущая = прежняя
    return текущая


def проверить_связь_результата_с_назначением(
    пул: dict[str, Any],
    ...
    *,
    ...
    корневая = корневая_квитанция_результата(результат)
    совпадения = [
        запись
        for запись in пул.get("assignments", {}).values()
        if isinstance(запись, dict)
        and запись.get("assignment_hash") == корневая.get("assignment_hash")
    ]
    if len(совпадения) != 1:
        raise ОшибкаПула(65, состояние, "Результат не связан с одним exact assignment.")
    назначение = совпадения[0]
    проверить_неизменяемое_назначение_и_активацию(
        пул,
        назначение,
        состояние=состояние,
    ...
    if (
        назначение.get("role") != "писатель"
        or назначение.get("status") not in {"result_frozen", "released"}
        or any(корневая.get(поле_результата) != назначение.get(поле_назначения) for поле_результата, поле_назначения in соответствие)
        or корневая.get("base_oid") != назначение.get("base_oid")
        or назначение.get("result_hash") != хэш_результата
        or назначение.get("result_head") != результат.get("head_oid")
        or назначение.get("current_oid") != результат.get("head_oid")
    ):
        raise ОшибкаПула(65, состояние, "Квитанция result не воспроизводит assignment/branch/target provenance.")
    return назначение
...
def проверить_мигрированный_результат(
    ...
    ожидаемые_поля_прежней = set(ПОЛЯ_ОБЫЧНОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА)
    if прежняя_схема == СХЕМА_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        ожидаемые_поля_прежней.add("хэш_намерения_коммита")
    elif прежняя_схема == ПРЕЖНЯЯ_СХЕМА_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        pass
    elif прежняя_схема == СХЕМА_МИГРИРОВАННОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        ожидаемые_поля_прежней = set(прежняя)
    else:
        raise ОшибкаПула(65, "migrated_result_receipt_mismatch", "Previous result имеет неизвестную схему.")
    if (
        set(прежняя) != ожидаемые_поля_прежней
        or set(результат)
        != set(прежняя) | ПОЛЯ_ПРОИСХОЖДЕНИЯ_МИГРИРОВАННОГО_РЕЗУЛЬТАТА
    ):
        raise ОшибкаПула(65, "migrated_result_receipt_mismatch", "Migrated result не прошёл exact supersession schema.")
    ...
    if not isinstance(соответствие, dict) or not isinstance(доказательства, dict):
        raise ОшибкаПула(65, "migration_receipt_invalid", "Migration receipt потеряла mapping/proofs.")
    исходный_пул = прочитать_канонический_блоб_по_идентификатору(
        контекст,
        проверить_идентификатор_блоба(контекст, квитанция.get("source_pool_oid")),
    )
    проверить_состояние_пула(контекст, исходный_пул)
    if исходный_пул.get("results", {}).get(старый_хэш) != прежняя:
        raise ОшибкаПула(65, "migrated_result_previous_receipt_forged", "Embedded previous result отсутствует в immutable source pool.")
    исходное_назначение = проверить_связь_результата_с_назначением(
        исходный_пул,
        прежняя,
        старый_хэш,
        состояние="migrated_result_assignment_mismatch",
    )
    if квитанция.get("branch_refs_before", {}).get(прежняя.get("branch_ref")) != прежняя.get("head_oid"):
        raise ОшибкаПула(65, "migrated_result_assignment_mismatch", "Previous result не связан с exact branch snapshot миграции.")
    if прежняя_схема == СХЕМА_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        проверить_результат_перед_каждым_слиянием(
            контекст,
            исходный_пул,
            старый_хэш,
        )
    elif прежняя_схема == СХЕМА_МИГРИРОВАННОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА:
        проверить_мигрированный_результат(контекст, прежняя, старый_хэш)
    ...
    if (
        ...
        or any(not isinstance(объект, str) for объект in [*старые_коммиты, *новые_коммиты])
    ...
    if (
        len(set(старые_коммиты)) != len(старые_коммиты)
        or len(set(новые_коммиты)) != len(новые_коммиты)
    ):
        raise ОшибкаПула(65, "migrated_result_oid_mapping_mismatch", "Old/new result OIDs содержат повторы.")
    try:
        проверить_идентификатор_объекта(контекст, результат["old_base_oid"])
        проверить_идентификатор_объекта(контекст, результат["base_oid"])
        for объект in [*старые_коммиты, *новые_коммиты]:
            проверить_идентификатор_объекта(контекст, объект)
    except ОшибкаПула as ошибка:
        raise ОшибкаПула(65, "migrated_result_oid_mapping_mismatch", "Old/new result OIDs не являются exact commit-объектами.") from ошибка
    доказательство = проверить_связь_доказательства_миграции(
        квитанция,
    ...
    if (
        доказательство.get("schema") == СХЕМА_ДОКАЗАТЕЛЬСТВА_МИГРАЦИИ_ДАТЫ
        and доказательство.get("branch_ref") != результат.get("branch_ref")
    ):
        raise ОшибкаПула(65, "migrated_result_proof_forged", "Seed proof относится к иной result branch.")
    for старый_идентификатор in старые_коммиты:
        семя = доказательства.get(старый_идентификатор)
        if (
            not isinstance(семя, dict)
            or семя.get("schema") != СХЕМА_ДОКАЗАТЕЛЬСТВА_МИГРАЦИИ_ДАТЫ
        ):
            continue
        try:
            ожидаемое_происхождение = найти_квитанцию_исходного_коммита(
                исходный_пул,
                старый_идентификатор,
            )
        except ОшибкаПула as ошибка:
            raise ОшибкаПула(
                ...
                "Seed proof не связано с единственной exact source receipt.",
            ) from ошибка
        фактическое_происхождение = (
            семя.get("receipt_hash"),
            семя.get("task_id"),
            семя.get("branch_ref"),
            семя.get("assignment_id"),
            семя.get("operation"),
        )
        if фактическое_происхождение != ожидаемое_происхождение:
            raise ОшибкаПула(
                65,
                "migrated_result_proof_forged",
                "Seed proof изменило receipt/task/branch/assignment/operation provenance.",
            )
    ...
    предыдущая_вершина = результат["base_oid"]
    последнее_дерево: str | None = None
    последнее_сообщение: bytes | None = None
    for прежний_идентификатор_объекта_миграции, новый_идентификатор_объекта_миграции in zip(старые_коммиты, новые_коммиты):
        ...
        ожидаемые_байты = ожидаемые_байты_мигрированного_коммита(
            старые_байты,
            соответствие,
            доказательство_коммита,
        )
        заголовок, разделитель, сообщение = новые_байты.partition(b"\n\n")
        строки_дерева = [строка[5:] for строка in заголовок.splitlines() if строка.startswith(b"tree ")]
        родители = [строка[7:].decode("ascii", errors="strict") for строка in заголовок.splitlines() if строка.startswith(b"parent ")]
        if (
            новые_байты != ожидаемые_байты
            or разделитель != b"\n\n"
            or len(строки_дерева) != 1
        ...
        последнее_дерево = строки_дерева[0].decode("ascii", errors="strict")
        последнее_сообщение = сообщение
        предыдущая_вершина = новый_идентификатор_объекта_миграции
    if (
        предыдущая_вершина != результат.get("head_oid")
        or последнее_дерево != результат.get("tree_oid")
        or последнее_сообщение is None
        or результат.get("хэш_сообщения") != "sha256:" + hashlib.sha256(последнее_сообщение).hexdigest()
    ):
        raise ОшибкаПула(65, "migrated_result_receipt_mismatch", "Terminal tree/message migrated result не совпали с exact commit bytes.")
...
def проверить_результат_перед_каждым_слиянием(
    ...
    if (
        ...
        проверить_связь_результата_с_назначением(
            пул,
            результат,
            хэш_результата,
            состояние="migrated_result_assignment_mismatch",
    ...
    if (
        isinstance(результат, dict)
        and результат.get("schema") == ПРЕЖНЯЯ_СХЕМА_КВИТАНЦИИ_РЕЗУЛЬТАТА
    ):
        raise ОшибкаПула(
            65,
            "integration_commit_date_policy_required",
            "Result требует завершённую миграцию долговечной политики времени.",
        )
    if (
        not isinstance(результат, dict)
        or результат.get("schema") != СХЕМА_КВИТАНЦИИ_РЕЗУЛЬТАТА
        or set(результат)
        != ПОЛЯ_ОБЫЧНОЙ_КВИТАНЦИИ_РЕЗУЛЬТАТА | {"хэш_намерения_коммита"}
        or хэш_объекта(результат) != хэш_результата
    ):
        raise ОшибкаПула(65, "result_receipt_invalid", "Result не прошёл exact схему и canonical hash.")
    назначение = проверить_связь_результата_с_назначением(
        пул,
        результат,
        хэш_результата,
        состояние="result_receipt_invalid",
    )
    намерения = назначение.get("намерения_коммитов")
    ...
    if (
        ...
        or not коммиты
        or any(not isinstance(коммит, str) for коммит in коммиты)
        or len(set(коммиты)) != len(коммиты)
    ...
    try:
        база_результата = проверить_идентификатор_объекта(контекст, результат["base_oid"])
        вершина_результата = проверить_идентификатор_объекта(контекст, результат["head_oid"])
        for коммит in коммиты:
            проверить_идентификатор_объекта(контекст, коммит)
    except (KeyError, ОшибкаПула) as ошибка:
        raise ОшибкаПула(65, "result_receipt_invalid", "Result ссылается не на exact commit-объекты.") from ошибка
    фактические_коммиты = текст_команды_версий(
        контекст.основной_корень,
        ["rev-list", "--reverse", f"{база_результата}..{вершина_результата}"],
    ).splitlines()
    дерево_вершины = текст_команды_версий(
        контекст.основной_корень,
        ["rev-parse", "--verify", f"{вершина_результата}^{{tree}}"],
    )
    if фактические_коммиты != коммиты or результат.get("tree_oid") != дерево_вершины:
        raise ОшибкаПула(65, "result_receipt_invalid", "Result не воспроизводит exact commit/tree цепочку.")
...
def аргументы_C0_основной_очереди(
    миграция: dict[str, Any],
    хэш_кандидата: str,
    хэш_ревью: str,
) -> list[str]:
    return [
        "--bootstrap-C0",
        "--ревизия-инструмента-C0", миграция["tool_oid"],
        "--хэш-кандидата-C0", хэш_кандидата,
        "--хэш-ревью-C0", хэш_ревью,
        "--хэш-квитанции-миграции-C0", миграция["receipt_hash"],
        "--объект-квитанции-миграции-C0", миграция["receipt_oid"],
        "--хэш-аттестации-миграции-C0", миграция["attestation_hash"],
    ]


def ревизия_моста_основной_очереди(
    контекст: КонтекстПула,
    миграция: dict[str, Any],
) -> str:
    if миграция.get("status") == "completed":
        return проверить_идентификатор_объекта(контекст, миграция["tool_oid"])
    if миграция.get("status") == "activated":
        ссылка_цели = миграция.get("target_ref")
        вершина_цели = прочитать_ссылку(контекст, ссылка_цели) if isinstance(ссылка_цели, str) else None
        if вершина_цели is None:
            raise ОшибкаПула(65, "migration_activation_target_missing", "Activated bridge потерял exact target HEAD.")
        return проверить_идентификатор_объекта(контекст, вершина_цели)
    raise ОшибкаПула(65, "commit_date_migration_incomplete", "Мост интеграции не имеет completed/activated ревизии.")


def подготовить_C0_для_основной_очереди(
    контекст: КонтекстПула,
    миграция: dict[str, Any],
    *,
    идентификатор_объекта_пула: str,
    ...
    вершина_кандидата: str,
) -> tuple[dict[str, Any], list[str]]:
    ревизия_моста = ревизия_моста_основной_очереди(контекст, миграция)
    состояние_очереди = вызвать_мост_основной_очереди(контекст, ["status"], ревизия_моста)
    владелец = состояние_очереди.get("owner")
    ожидающие = состояние_очереди.get("waiting")
    идентификатор_объекта_очереди = состояние_очереди.get("queue_oid")
    ссылка_очереди = состояние_очереди.get("queue_ref")
    if (
        not isinstance(владелец, dict)
        or not isinstance(ожидающие, list)
        or not ожидающие
        ...
        or состояние_очереди.get("branch_ref") != целевая_ссылка
        or состояние_очереди.get("stored_branch_ref") != целевая_ссылка
        or not isinstance(ссылка_очереди, str)
        or not isinstance(идентификатор_объекта_очереди, str)
        or прочитать_ссылку(контекст, ссылка_очереди) != идентификатор_объекта_очереди
    ):
        raise ОшибкаПула(73, "bootstrap_C0_fifo_mismatch", "C0 требует exact owner/first-continuation snapshot основной FIFO.")
    проверить_идентификатор_блоба(контекст, идентификатор_объекта_очереди)
    квитанция_C0 = {
        "schema": СХЕМА_КВИТАНЦИИ_C0_АКТИВАЦИИ,
        "repository_identity": контекст.идентификатор_репозитория,
        "task_id": идентификатор_задачи,
        "generation": поколение,
        ...
        "source_pool_oid": идентификатор_объекта_пула,
        "integration_hash": хэш_интеграции,
        "queue_ref": ссылка_очереди,
        "source_queue_oid": идентификатор_объекта_очереди,
        "owner_ticket_id": владелец.get("ticket_id"),
        "owner_seq": владелец.get("seq"),
        "first_continuation_ticket_id": ожидающие[0].get("ticket_id"),
        "first_continuation_seq": ожидающие[0].get("seq"),
        "waiting_hash": хэш_объекта({"waiting": ожидающие}),
        "next_seq": состояние_очереди.get("next_seq"),
    }
    if set(квитанция_C0) != ПОЛЯ_КВИТАНЦИИ_C0_АКТИВАЦИИ:
        raise AssertionError("C0 builder потерял closed schema.")
    активированная_миграция = copy.deepcopy(миграция)
    активированная_миграция.update(
        {
            "status": "activated",
            "candidate_hash": хэш_кандидата,
            "review_hash": хэш_ревью,
            "bootstrap_receipt_hash": хэш_объекта(квитанция_C0),
        }
    )
    return активированная_миграция, аргументы_C0_основной_очереди(
        миграция,
        хэш_кандидата,
        хэш_ревью,
    )
...
def команда_продвинуть_цель(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        if not isinstance(миграция, dict):
            raise ОшибкаПула(65, "commit_date_migration_incomplete", "Аттестация миграции исчезла до bridge.")
        ревизия_моста = ревизия_моста_основной_очереди(контекст, миграция)
        ...
        if существующая is not None:
            ...
            if (
                миграция.get("status") == "activated"
                and миграция.get("candidate_hash") == хэш_кандидата
                and миграция.get("review_hash") == хэш_ревью
                and миграция.get("target_oid") == квитанция["base_oid"]
            ):
                аргументы_повтора.extend(аргументы_C0_основной_очереди(миграция, хэш_кандидата, хэш_ревью))
        ...
        if миграция.get("status") == "completed":
            активированная_миграция, аргументы_C0 = подготовить_C0_для_основной_очереди(
                контекст,
                миграция,
                идентификатор_объекта_пула=идентификатор_объекта_пула,
                хэш_кандидата=хэш_кандидата,
                хэш_ревью=хэш_ревью,
                хэш_интеграции=хэш_квитанции,
                идентификатор_задачи=идентификатор_задачи,
                поколение=поколение,
                идентификатор_продолжения=идентификатор_продолжения,
                целевая_ссылка=целевая_ссылка,
                исходная_вершина=ожидаемая,
                вершина_кандидата=кандидат["head_oid"],
            )
            новый_пул["миграция_дат_коммитов"] = активированная_миграция
        новый_пул["revision"] += 1
        новый_идентификатор_объекта_пула = записать_объект_состояния(контекст, новый_пул)
        try:
            вызвать_мост_основной_очереди(
...
def команда_план_миграции_дат(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    идентичность = получить_идентичность_закреплённой_загрузки()
    ядро = загрузить_ядро_миграции_дат(контекст, ревизия, идентичность)
    пул, идентификатор_пула = прочитать_пул(контекст)
    допустимая_фаза = (
...
def команда_подтвердить_протокол_после_миграции(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    хэш_плана = проверить_ограждение(аргументы_команды.хэш_плана, "хэш плана")
    идентичность = получить_идентичность_закреплённой_загрузки()
    загрузить_ядро_миграции_дат(контекст, ревизия, идентичность)

    def отклонить_сдвиг_неизменяемых_ссылок(
        проверки: Sequence[tuple[str, str | None]],
        изменяемые_ссылки: set[str],
    ) -> None:
        for ссылка, ожидаемый_объект in проверки:
            if ссылка in изменяемые_ссылки or прочитать_ссылку(контекст, ссылка) == ожидаемый_объект:
                continue
            if ссылка == назначение["target_ref"]:
                состояние = "assignment_execution_target_changed"
            elif ссылка == назначение["branch_ref"]:
                состояние = "assignment_execution_branch_changed"
            else:
            ...
            raise ОшибкаПула(
                73,
                состояние,
                "Execution-снимок protocol ack сдвинулся до атомарной проверки.",
                ссылка=ссылка,
            )
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        if not isinstance(очередь, dict) or идентификатор_очереди_миграции is None:
            raise ОшибкаПула(73, "migration_protocol_owner_mismatch", "Очередь не принадлежит exact получателю мигрированной линии.")
        проверить_очередь(очередь, назначение)
        if not isinstance(очередь.get("owner"), dict) or очередь["owner"].get("task_id") != задача or очередь["owner"].get("generation") != поколение:
            raise ОшибкаПула(73, "migration_protocol_owner_mismatch", "Очередь не принадлежит exact получателю мигрированной линии.")
        ревизия_исполнения, проверки_исполнения = проверить_закреплённую_ревизию_исполнения(
            контекст,
            пул,
            идентификатор_пула,
            назначение,
            ревизия_явного_подтверждения=ревизия,
        )
        проверки_снимка = объединить_проверки_ссылок(
            [
                *проверки_исполнения,
                *проверки_маршрута_назначения(контекст, пул, назначение, задача),
                (назначение["queue_ref"], идентификатор_очереди_миграции),
            ]
        )
        if очередь["status"] == "active" and not очередь["owner"].get("reload_required"):
            if транзакция_ссылок(контекст, [], проверки=проверки_снимка):
                return {"state": "admitted", "вершина": вершина, "ревизия_протокола": ревизия_исполнения}
            отклонить_сдвиг_неизменяемых_ссылок(
                проверки_снимка,
                {контекст.ссылка_пула, назначение["queue_ref"]},
            )
            time.sleep(0.002)
            continue
        ...
        изменяемые_ссылки = {контекст.ссылка_пула, назначение["queue_ref"]}
        проверки_без_изменяемых = [
            проверка
            for проверка in проверки_снимка
            if проверка[0] not in изменяемые_ссылки
        ]
        if транзакция_ссылок(
            контекст,
            [
                (контекст.ссылка_пула, новый_идентификатор_пула, идентификатор_пула),
                (назначение["queue_ref"], новый_идентификатор_очереди, идентификатор_очереди_миграции),
            ],
            проверки=проверки_без_изменяемых,
        ):
            return {"state": "admitted", "вершина": вершина, "ревизия_протокола": ревизия_исполнения}
        отклонить_сдвиг_неизменяемых_ссылок(
            проверки_снимка,
...
def команда_состояние(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    результат = ответ_назначения(назначение, назначение["status"], ревизия_исполнения)
    результат.update(
        {
