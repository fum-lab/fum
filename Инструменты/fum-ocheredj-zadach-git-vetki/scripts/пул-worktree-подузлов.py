...
КАТАЛОГ_РАБОЧИХ_НАБОРОВ = PurePosixPath("Планирование/следующие-шаги-веток")
ПУТЬ_РАБОЧЕГО_НАБОРА_МАСТЕР = (КАТАЛОГ_РАБОЧИХ_НАБОРОВ / "master.md").as_posix()
...
def подготовить_маршрут_задачи(
    ...
    ожидаемый_объект = выполнить_команду_версий(
        контекст.основной_корень,
        ["hash-object", "--stdin"],
        ввод=канонические_байты(нагрузка),
    ).вывод.decode("ascii").strip()
...
def материализовать_маршрут_задачи(
    контекст: КонтекстПула,
    идентификатор_задачи: str,
    вид_маршрута: str,
    идентичность_маршрута: str,
    ...
    записанный = записать_объект_состояния(контекст, нагрузка)
    if len(изменения) != 1 or изменения[0][1] != записанный:
        raise ОшибкаПула(65, "task_route_materialization_mismatch", "Материализованный runtime-маршрут не совпал с read-only планом.")
...
def прочитать_строку_транзакции(
    процесс: subprocess.Popen[bytes],
    *,
    таймаут_секунд: float = 30.0,
) -> bytes:
    if процесс.stdout is None:
        return b""
    try:
        готовые, _, _ = select.select([процесс.stdout], [], [], таймаут_секунд)
...
def отправить_строки_транзакции(
    процесс: subprocess.Popen[bytes],
    строки: Sequence[str],
) -> bool:
    if процесс.stdin is None:
        return False
    данные = ("\n".join(строки) + "\n").encode("utf-8")
    try:
        записано = процесс.stdin.write(данные)
        процесс.stdin.flush()
    except (BrokenPipeError, OSError, ValueError):
        return False
    return записано == len(данные)


def закрыть_процесс_транзакции(процесс: subprocess.Popen[bytes]) -> None:
    if процесс.stdin is not None and not процесс.stdin.closed:
        try:
            процесс.stdin.close()
        except OSError:
            pass
    try:
        процесс.wait(timeout=2.0)
...
def дождаться_процесса_после_фиксации(процесс: subprocess.Popen[bytes]) -> None:
    """Не посылает сигнал Git после полной отправки команды `commit`.

    Подготовленная HEAD-транзакция остаётся живой у вызывающего
    до возврата этой функции. Каждое ожидание ограничено, но после
    полной отправки ``commit`` процесс нельзя прервать без риска
    частично записанного многоссылочного перехода.
    """
    if процесс.stdin is not None and not процесс.stdin.closed:
        try:
            процесс.stdin.close()
    ...
    while потоки or процесс.poll() is None:
        if not потоки:
            try:
                процесс.wait(timeout=2.0)
            except subprocess.TimeoutExpired:
            ...
            continue
        try:
            готовые, _, _ = select.select(list(потоки), [], [], 2.0)
        except (OSError, ValueError):
            готовые = []
        for поток in готовые:
            try:
                часть = os.read(поток.fileno(), 65_536)
            except (OSError, ValueError):
                часть = b""
            if not часть:
                потоки.discard(поток)
                try:
                    поток.close()
                except OSError:
                    pass
...
def подготовить_процесс_транзакции(
    ...
    if (
        отправить_строки_транзакции(процесс, [*строки, "prepare"])
        and прочитать_строку_транзакции(процесс) == b"start: ok\n"
        and прочитать_строку_транзакции(процесс) == b"prepare: ok\n"
    ):
        return процесс
    закрыть_процесс_транзакции(процесс)
    return None


def прервать_подготовленную_транзакцию(процесс: subprocess.Popen[bytes]) -> None:
    if отправить_строки_транзакции(процесс, ["abort"]):
        прочитать_строку_транзакции(процесс)
    закрыть_процесс_транзакции(процесс)


def проверить_результат_сомнительной_фиксации(
    контекст: КонтекстПула,
    изменения: Sequence[tuple[str, str, str | None]],
) -> bool:
    if not изменения:
    ...
    после = True
    до = True
    for ссылка, новый_идентификатор, прежний_идентификатор in изменения:
        фактический = прочитать_ссылку(контекст, ссылка)
        после = после and фактический == новый_идентификатор
        до = до and фактический == прежний_идентификатор
    if после and not до:
        return True
    if до and not после:
        return False
    raise ОшибкаПула(
        75,
        "ref_transaction_commit_indeterminate",
        "После потерянной квитанции refs не совпали ни с exact до-, ни с exact после-состоянием.",
    )


def подготовить_нейтральную_среду_ссылок(
    контекст: КонтекстПула,
    каталог: Path,
    целевая_ссылка: str,
    отделённая_вершина: str,
) -> tuple[Path, dict[str, str]]:
    каталог = каталог.resolve()
    рабочий_каталог = каталог / "worktree"
    рабочий_каталог.mkdir(mode=0o700)
    (каталог / "HEAD").write_text(f"{отделённая_вершина}\n", encoding="ascii")
    (каталог / "commondir").write_text(
        f"{контекст.общий_каталог_системы_версий}\n",
        encoding="utf-8",
    )
    файл_системы_версий = рабочий_каталог / ".git"
    файл_системы_версий.write_text(f"gitdir: {каталог}\n", encoding="utf-8")
    (каталог / "gitdir").write_text(f"{файл_системы_версий}\n", encoding="utf-8")
    дополнения = {
        "GIT_DIR": str(каталог),
        "GIT_COMMON_DIR": str(контекст.общий_каталог_системы_версий),
        "GIT_WORK_TREE": str(рабочий_каталог),
        "GIT_INDEX_FILE": str(каталог / "index"),
        "GIT_OPTIONAL_LOCKS": "0",
    }
    абсолютный_каталог = разрешить_путь_системы_версий(
        рабочий_каталог,
        текст_команды_версий(
            рабочий_каталог,
            ["rev-parse", "--absolute-git-dir"],
            дополнения_среды=дополнения,
        ),
    )
    общий_каталог = разрешить_путь_системы_версий(
        рабочий_каталог,
        текст_команды_версий(
            рабочий_каталог,
    ...
    символическая_голова = выполнить_команду_версий(
        рабочий_каталог,
    ...
    фактическая_цель = текст_команды_версий(
        рабочий_каталог,
        ["rev-parse", "--verify", целевая_ссылка],
        дополнения_среды=дополнения,
    )
    формат = текст_команды_версий(
        рабочий_каталог,
        ["rev-parse", "--show-object-format"],
        дополнения_среды=дополнения,
    )
    ожидаемый_формат = "sha256" if контекст.длина_идентификатора_объекта == 64 else "sha1"
    if (
        абсолютный_каталог != каталог
        or общий_каталог != контекст.общий_каталог_системы_версий
        or символическая_голова.код != 1
        or символическая_голова.вывод
        or фактическая_голова != отделённая_вершина
        or not re.fullmatch(
            rf"[0-9a-f]{{{контекст.длина_идентификатора_объекта}}}",
            фактическая_цель,
        )
        or формат != ожидаемый_формат
    ):
        raise ОшибкаПула(
            65,
            "invalid_neutral_ref_environment",
            "Нейтральная среда Git не доказала exact common-dir, detached HEAD и формат объектов.",
        )
    return рабочий_каталог, безопасная_среда(дополнения)


def выполнить_двойную_подготовленную_транзакцию(
    контекст: КонтекстПула,
    корень_основной_транзакции: Path,
    строки_общих_ссылок: Sequence[str],
    изменения: Sequence[tuple[str, str, str | None]],
    ссылки_общей_транзакции: Sequence[str],
    целевая_ссылка: str,
    ожидаемая_вершина: str,
) -> bool:
    if any(
        not (ссылка.startswith("refs/heads/") or ссылка.startswith("refs/fum/"))
        for ссылка in ссылки_общей_транзакции
    ):
        raise ОшибкаПула(
            65,
            "invalid_neutral_transaction_ref",
            "Нейтральная транзакция не принимает HEAD или per-worktree refs.",
        )
    with tempfile.TemporaryDirectory(prefix="fum-neutral-ref-transaction-") as сырой_каталог:
        рабочий_каталог, нейтральная_среда = подготовить_нейтральную_среду_ссылок(
            контекст,
            Path(сырой_каталог),
            целевая_ссылка,
            ожидаемая_вершина,
        )
        команда_общих_ссылок = [
            "git",
            "--no-replace-objects",
            "-C",
            str(рабочий_каталог),
        ...
        процесс_общих_ссылок = подготовить_процесс_транзакции(
            команда_общих_ссылок,
            нейтральная_среда,
            строки_общих_ссылок,
        )
        if процесс_общих_ссылок is None:
            return False

        процесс_головы: subprocess.Popen[bytes] | None = None
        фиксация_отправлена = False
        процесс_общих_ссылок_закрыт = False
        try:
            процесс_головы = подготовить_процесс_транзакции(
                [
                    "git",
                    "--no-replace-objects",
                    "-C",
                    str(корень_основной_транзакции),
                    "update-ref",
                    "--no-deref",
                    "--stdin",
                ],
                безопасная_среда(),
                ["start", f"symref-verify HEAD {целевая_ссылка}"],
            )
            if процесс_головы is None:
                return False

            фиксация_отправлена = отправить_строки_транзакции(
                процесс_общих_ссылок,
                ["commit"],
            )
            if not фиксация_отправлена:
                return False
            квитанция = прочитать_строку_транзакции(процесс_общих_ссылок)
            дождаться_процесса_после_фиксации(процесс_общих_ссылок)
            процесс_общих_ссылок_закрыт = True
            if квитанция == b"commit: ok\n":
                return True
            return проверить_результат_сомнительной_фиксации(контекст, изменения)
        finally:
            if not процесс_общих_ссылок_закрыт:
                if фиксация_отправлена:
                    дождаться_процесса_после_фиксации(процесс_общих_ссылок)
                else:
                    прервать_подготовленную_транзакцию(процесс_общих_ссылок)
            if процесс_головы is not None:
                прервать_подготовленную_транзакцию(процесс_головы)
...
def транзакция_ссылок(
    ...
    *,
    ...
    ожидаемая_целевая_вершина: str | None = None
    ...
    if символические_проверки:
        ...
        ожидаемая_целевая_вершина = совпадающие[0]
    строки = ["start"]
    ...
    if ожидаемая_рабочая_ссылка is not None:
        if ожидаемая_целевая_вершина is None:
            raise ОшибкаПула(65, "invalid_symref_verification", "HEAD потерял exact вершину рабочей ссылки.")
        return выполнить_двойную_подготовленную_транзакцию(
            контекст,
            корень_транзакции or контекст.основной_корень,
            строки,
            изменения,
            все_ссылки,
            ожидаемая_рабочая_ссылка,
            ожидаемая_целевая_вершина,
        )
...
def проверить_путь_рабочего_набора(
    целевая_ссылка: str,
    сырой_путь: str | None,
) -> str:
    if сырой_путь is None:
        if целевая_ссылка != "refs/heads/master":
            raise ОшибкаПула(
                64,
                "working_set_path_required",
                "Немастерская цель требует exact путь веточного рабочего набора.",
            )
        return ПУТЬ_РАБОЧЕГО_НАБОРА_МАСТЕР
    проверить_ограждение(сырой_путь, "путь рабочего набора")
    if unicodedata.normalize("NFC", сырой_путь) != сырой_путь:
        raise ОшибкаПула(64, "invalid_working_set_path", "Путь рабочего набора должен быть NFC.")
    путь = PurePosixPath(сырой_путь)
    if (
        путь.is_absolute()
        or путь.as_posix() != сырой_путь
        or сырой_путь.endswith("/")
        or any(часть in {"", ".", ".."} for часть in путь.parts)
        or путь.parent != КАТАЛОГ_РАБОЧИХ_НАБОРОВ
        or путь.suffix != ".md"
        or путь.name == "README.md"
    ):
        raise ОшибкаПула(
            64,
            "invalid_working_set_path",
            "Путь рабочего набора неканоничен или выходит за каталог веточных наборов.",
        )
    нормализованный = путь.as_posix()
    if целевая_ссылка == "refs/heads/master" and нормализованный != ПУТЬ_РАБОЧЕГО_НАБОРА_МАСТЕР:
        raise ОшибкаПула(
            64,
            "master_working_set_mismatch",
            "Ветка master использует только канонический master.md.",
        )
    return нормализованный


def проверить_выбранный_шаг(
    целевая_ссылка: str,
    сырой_шаг: str | None,
) -> str | None:
    if целевая_ссылка == "refs/heads/master":
        if сырой_шаг is not None:
            raise ОшибкаПула(64, "master_route_step_forbidden", "Legacy-маршрут master не принимает веточный --шаг.")
        return None
    if сырой_шаг is None:
        raise ОшибкаПула(64, "working_set_step_required", "Немастерская цель требует exact --шаг из рабочего набора.")
    return проверить_ограждение(сырой_шаг, "выбранный шаг")


def проверить_рабочий_набор_в_вершине(
    контекст: КонтекстПула,
    вершина: str,
    целевая_ссылка: str,
    путь: str,
    выбранный_шаг: str | None,
) -> str:
    сырая_запись = выполнить_команду_версий(
        контекст.основной_корень,
        ["ls-tree", "-z", вершина, "--", путь],
        проверять=False,
    )
    строка = сырая_запись.вывод.decode("utf-8", errors="strict")
    if сырая_запись.код != 0 or not строка:
        raise ОшибкаПула(64, "working_set_missing", "Рабочий набор отсутствует в exact целевой вершине.")
    if not строка.endswith("\0") or строка.count("\0") != 1:
        raise ОшибкаПула(65, "invalid_working_set_entry", "Git-запись рабочего набора неканонична.")
    строка = строка[:-1]
    try:
        описание, фактический_путь = строка.split("\t", 1)
        режим, тип_объекта, идентификатор_объекта = описание.split(" ", 2)
    except ValueError as ошибка:
        raise ОшибкаПула(65, "invalid_working_set_entry", "Git-запись рабочего набора повреждена.") from ошибка
    if (
        "\n" in строка
        or фактический_путь != путь
        or режим not in {"100644", "100755"}
        or тип_объекта != "blob"
    ):
        raise ОшибкаПула(64, "working_set_not_regular_blob", "Рабочий набор должен быть обычным Git-blob по exact пути.")
    ...
    try:
        ...
        raise ОшибкаПула(64, "invalid_working_set", "Рабочий набор не является текстом UTF-8.") from ошибка
    if not текст.startswith("+++\n"):
        raise ОшибкаПула(64, "invalid_working_set", "Рабочий набор не начинается TOML-блоком.")
    граница = текст.find("\n+++\n", 4)
    if граница < 0:
        raise ОшибкаПула(64, "invalid_working_set", "TOML-блок рабочего набора не закрыт.")
    try:
        метаданные = tomllib.loads(текст[4:граница])
    except tomllib.TOMLDecodeError as ошибка:
        raise ОшибкаПула(64, "invalid_working_set", "TOML-блок рабочего набора некорректен.") from ошибка
    if type(метаданные.get("schema_version")) is not int or метаданные["schema_version"] != 5:
        raise ОшибкаПула(64, "working_set_schema_mismatch", "Рабочий набор должен иметь схему 5.")
    if метаданные.get("branch_ref") != целевая_ссылка:
        raise ОшибкаПула(64, "working_set_branch_mismatch", "branch_ref рабочего набора не совпал с exact целью.")
    if целевая_ссылка != "refs/heads/master":
        if метаданные.get("state") != "open":
            raise ОшибкаПула(64, "working_set_not_open", "Немастерский рабочий набор должен иметь state=open.")
        if выбранный_шаг is None:
            raise ОшибкаПула(65, "missing_selected_step", "Снимок немастерской цели потерял exact шаг.")
        кандидаты = метаданные.get("candidates")
        if not isinstance(кандидаты, list) or any(not isinstance(кандидат, dict) for кандидат in кандидаты):
            raise ОшибкаПула(64, "invalid_working_set_candidates", "Кандидаты рабочего набора повреждены.")
        совпадения = [
            кандидат
            for кандидат in кандидаты
            if кандидат.get("step_id") == выбранный_шаг
        ]
        if not совпадения:
            raise ОшибкаПула(64, "working_set_step_missing", "Exact шаг отсутствует среди кандидатов рабочего набора.")
        if len(совпадения) != 1:
            raise ОшибкаПула(64, "working_set_step_ambiguous", "Exact шаг повторён в рабочем наборе.")
        if совпадения[0].get("dispatch") != "automatic":
            raise ОшибкаПула(
                64,
                "working_set_step_not_automatic",
                "Exact шаг не имеет dispatch=automatic.",
                режим_диспетчеризации=совпадения[0].get("dispatch"),
            )
    return идентификатор_объекта
...
def совпадает_назначение(запись: dict[str, Any], ожидаемое: dict[str, Any]) -> bool:
    ...
    return (
        all(запись.get(поле) == ожидаемое.get(поле) for поле in поля)
        and запись.get("working_set_path") == ожидаемое.get("working_set_path")
        and запись.get("selected_working_set_step") == ожидаемое.get("selected_working_set_step")
    )


def потребовать_современное_немастерское_назначение(запись: dict[str, Any]) -> None:
    if (
        запись.get("target_ref") == "refs/heads/master"
        or запись.get("lifecycle") != "self_line"
    ):
        return
    if isinstance(запись.get("working_set_path"), str) and isinstance(
        запись.get("selected_working_set_step"),
        str,
    ):
        return
    вершина_протокола = запись.get("protocol_oid")
    raise ОшибкаПула(
        73,
        "legacy_nonmaster_assignment_v1",
        "Legacy-немастерское назначение не мигрируется; завершите его закреплённым protocol_oid.",
        идентификатор_назначения=запись.get("id"),
        доверенная_ревизия_протокола=вершина_протокола,
    )
...
def нагрузка_назначения(
    ...
    *,
    ...
    путь_рабочего_набора = getattr(аргументы_команды, "путь_рабочего_набора", None)
    if путь_рабочего_набора is not None:
        нагрузка["working_set_path"] = путь_рабочего_набора
    выбранный_шаг = getattr(аргументы_команды, "выбранный_шаг_рабочего_набора", None)
    if выбранный_шаг is not None:
        нагрузка["selected_working_set_step"] = выбранный_шаг
    return нагрузка
...
def ответ_назначения(запись: dict[str, Any], состояние: str) -> dict[str, object]:
    return {
        ...
        "путь_рабочего_набора": запись.get("working_set_path") or (
            ПУТЬ_РАБОЧЕГО_НАБОРА_МАСТЕР
            if запись.get("lifecycle") == "self_line" and запись["target_ref"] == "refs/heads/master"
            else None
        ),
        "выбранный_шаг": запись.get("selected_working_set_step"),
...
def потребовать_немастерскую_основную_копию_цели(
    контекст: КонтекстПула,
    целевая_ссылка: str,
    *,
    ожидаемая_вершина_протокола: str | None = None,
) -> str:
    целевая_вершина = прочитать_ссылку(контекст, целевая_ссылка)
    if целевая_вершина is None:
        raise ОшибкаПула(64, "target_ref_missing", "Целевая ссылка немастерского маршрута отсутствует.")
    if целевая_ссылка == "refs/heads/master":
        return целевая_вершина

    абсолютный_каталог = разрешить_путь_системы_версий(
        контекст.основной_корень,
        текст_команды_версий(контекст.основной_корень, ["rev-parse", "--absolute-git-dir"]),
    )
    надрепозиторий = текст_команды_версий(
        контекст.основной_корень,
        ["rev-parse", "--show-superproject-working-tree"],
        проверять=False,
    )
    if абсолютный_каталог != контекст.общий_каталог_системы_версий or надрепозиторий:
        raise ОшибкаПула(
            73,
            "autonomous_primary_checkout_required",
            "Немастерская цель разрешает пул только в автономном primary checkout без superproject.",
        )
    символическая_голова = текст_команды_версий(
        контекст.основной_корень,
        ["symbolic-ref", "--quiet", "HEAD"],
        проверять=False,
    )
    if not символическая_голова:
        raise ОшибкаПула(73, "nonmaster_primary_detached", "Немастерская цель не маршрутизируется из detached HEAD.")
    if символическая_голова != целевая_ссылка:
        raise ОшибкаПула(73, "nonmaster_primary_ref_mismatch", "Symbolic HEAD основной копии не совпал с exact немастерской целью.")
    фактическая_голова = текст_команды_версий(контекст.основной_корень, ["rev-parse", "HEAD"])
    ожидаемая_вершина = ожидаемая_вершина_протокола or целевая_вершина
    if целевая_вершина != ожидаемая_вершина or фактическая_голова != ожидаемая_вершина:
        raise ОшибкаПула(73, "nonmaster_primary_oid_mismatch", "HEAD, целевая ссылка и pinned protocol_oid не совпали.")
    return ожидаемая_вершина
...
def активные_линии_маршрутизации(
    ...
    for запись in sorted(пул["assignments"].values(), key=lambda значение: значение["id"]):
        if запись["status"] in ТЕРМИНАЛЬНЫЕ_СОСТОЯНИЯ_НАЗНАЧЕНИЯ:
            continue
        потребовать_современное_немастерское_назначение(запись)
        очередь = None
        ...
        линии.append(
            {
                ...
                "целевая_ссылка": запись["target_ref"],
                "путь_рабочего_набора": запись.get("working_set_path"),
                "выбранный_шаг": запись.get("selected_working_set_step"),
                "разрешённые_пути": запись["write_paths"],
...
def построить_снимок_маршрутизации(
    ...
    источники = {
        ...
        for путь in ОБЩИЕ_ИСТОЧНИКИ_МАРШРУТИЗАЦИИ
    }
    идентификатор_рабочего_набора = проверить_рабочий_набор_в_вершине(
        контекст,
        вершина_протокола,
        целевая_ссылка,
    ...
    источники[путь_рабочего_набора] = идентификатор_рабочего_набора
    снимок: dict[str, object] = {
        "schema": "fum.снимок-маршрутизации-worktree-подузла.3",
        "task_id": идентификатор_задачи,
        "protocol_oid": вершина_протокола,
        "repository_identity": контекст.идентификатор_репозитория,
        "target_ref": целевая_ссылка,
        "working_set_path": путь_рабочего_набора,
        "selected_step": выбранный_шаг,
...
def команда_маршрутизировать(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    целевая_ссылка = проверить_целевую_ссылку(контекст, аргументы_команды.целевая_ссылка)
    путь_рабочего_набора = проверить_путь_рабочего_набора(
        целевая_ссылка,
        аргументы_команды.путь_рабочего_набора,
    )
    выбранный_шаг = (
        None
        if целевая_ссылка == "refs/heads/master"
        else проверить_выбранный_шаг(целевая_ссылка, аргументы_команды.шаг)
    )
    пул, идентификатор_объекта_пула = прочитать_пул(контекст)
    потребовать_немастерскую_основную_копию_цели(контекст, целевая_ссылка)
    ...
    снимок, хэш_снимка = построить_снимок_маршрутизации(
        контекст,
        идентификатор_задачи,
        целевая_ссылка,
        путь_рабочего_набора,
    ...
    потребовать_немастерскую_основную_копию_цели(
        контекст,
        целевая_ссылка,
        ожидаемая_вершина_протокола=str(снимок["protocol_oid"]),
    )
    return {
        ...
        "путь_рабочего_набора": снимок["working_set_path"],
        "выбранный_шаг": снимок["selected_step"],
...
def зарезервировать_самозапуск_по_маршруту(
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        потребовать_немастерскую_основную_копию_цели(
            ...
            ожидаемая_вершина_протокола=ожидаемое["protocol_oid"],
        )
        ...
        if сохранённый_маршрут is not None:
            ...
            потребовать_современное_немастерское_назначение(назначение)
            if not совпадает_назначение(назначение, ожидаемое):
                raise ОшибкаПула(65, "lost_session_route_assignment", "Маршрут сессии потерял exact назначение.")
            ...
            if изменения_маршрута:
                материализовать_маршрут_задачи(
                    контекст,
                    идентификатор_задачи,
                    "worktree_self",
                    хэш_объекта({"schema": "fum.назначение-worktree-подузла.1", **ожидаемое}),
                ...
                if транзакция_ссылок(
                    ...
                    проверки=[
                        ...
                        (ожидаемое["target_ref"], ожидаемое["protocol_oid"]),
                    ],
                    корень_транзакции=контекст.основной_корень,
                    символические_проверки=(
                        [("HEAD", ожидаемое["target_ref"])]
                        if ожидаемое["target_ref"] != "refs/heads/master"
                        else ()
                    ),
                ):
                    return copy.deepcopy(назначение), True
        ...
        снимок, фактический_хэш = построить_снимок_маршрутизации(
            ...
            идентификатор_задачи,
            ожидаемое["target_ref"],
            проверить_путь_рабочего_набора(
                ожидаемое["target_ref"],
                ожидаемое.get("working_set_path"),
            ),
            проверить_выбранный_шаг(
                ожидаемое["target_ref"],
                ожидаемое.get("selected_working_set_step"),
            ),
            пул,
            идентификатор_объекта_пула,
        )
...
def команда_зарезервировать_себя(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    потребовать_немастерскую_основную_копию_цели(
        контекст,
        целевая_ссылка,
        ожидаемая_вершина_протокола=None if существующее is None else существующее.get("protocol_oid"),
    )
    ...
    параметры_выделения = argparse.Namespace(
        ...
        путь_рабочего_набора=(
            None
            if целевая_ссылка == "refs/heads/master"
            else путь_рабочего_набора
        ),
        выбранный_шаг_рабочего_набора=выбранный_шаг,
    )
    ожидаемое = нагрузка_назначения(
        параметры_выделения,
        контекст,
...
def команда_присоединиться_к_линии(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        потребовать_немастерскую_основную_копию_цели(
            ...
            ожидаемая_вершина_протокола=назначение["protocol_oid"],
        )
        маршрут = пул["session_routes"].get(идентификатор_задачи)
        ...
        if маршрут is not None:
            ...
            if маршрут.get("status") == "committed_handoff":
                if изменения_маршрута:
                    материализовать_маршрут_задачи(
                        контекст, идентификатор_задачи, "worktree_continuation", назначение["assignment_hash"], изменения_маршрута,
                    )
                    if транзакция_ссылок(
                        контекст,
                        изменения_маршрута,
                        проверки=[
                            (контекст.ссылка_пула, идентификатор_объекта_пула),
                            (назначение["queue_ref"], идентификатор_объекта_очереди),
                            (назначение["target_ref"], назначение["protocol_oid"]),
                        ],
                        корень_транзакции=контекст.основной_корень,
                        символические_проверки=([("HEAD", назначение["target_ref"])] if назначение["target_ref"] != "refs/heads/master" else ()),
                    ):
        ...
        if существующие:
            if маршрут is None:
                raise ОшибкаПула(
                    65,
                    "lost_continuation_route",
                    "FIFO-намерение продолжения потеряло exact маршрут сессии.",
                )
        ...
        фактический_снимок, фактический_хэш = построить_снимок_маршрутизации(
            ...
            проверить_путь_рабочего_набора(
                назначение["target_ref"],
                назначение.get("working_set_path"),
            ),
            проверить_выбранный_шаг(
                назначение["target_ref"],
                назначение.get("selected_working_set_step"),
            ),
            пул,
            идентификатор_объекта_пула,
        )
...
def команда_зарегистрироваться(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    for _ in range(МАКСИМУМ_ПОПЫТОК_СРАВНЕНИЯ):
        ...
        if существующий is not None:
            if изменения_маршрута:
                вид_маршрута = "worktree_self" if назначение.get("admission_mode") == "self" else "worktree_delegated"
                материализовать_маршрут_задачи(
                    контекст, идентификатор_задачи, вид_маршрута, назначение["assignment_hash"], изменения_маршрута,
                )
                if транзакция_ссылок(
                    контекст,
                    изменения_маршрута,
...
def построить_парсер() -> argparse.ArgumentParser:
    ...
    маршрутизация.add_argument("--путь-рабочего-набора")
    маршрутизация.add_argument("--шаг")
    ...
    саморезервация.add_argument("--путь-рабочего-набора")
