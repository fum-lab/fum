...
def первичная_рабочая_копия_чиста_для_старта(путь: Path) -> bool:
    return текст_команды_версий(
        путь,
        [
            "status",
...
def команда_маршрутизировать(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    целевая_ссылка = проверить_целевую_ссылку(контекст, аргументы_команды.целевая_ссылка)
    if not первичная_рабочая_копия_чиста_для_старта(контекст.основной_корень):
        raise ОшибкаПула(
            73,
            "dirty_primary_bootstrap",
            "Новая сессия не маршрутизируется через первичный checkout с изменениями вне корневой .obsidian/.",
        )
...
def команда_зарезервировать_себя(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    if not первичная_рабочая_копия_чиста_для_старта(контекст.основной_корень):
        raise ОшибкаПула(
            73,
            "dirty_primary_bootstrap",
            "Новая сессия не резервирует слот при изменениях вне корневой .obsidian/.",
        )
    идентификатор_задачи = проверить_ограждение(аргументы_команды.идентификатор_задачи, "task-id")
    идентификатор_среды = (
        None
...
def команда_присоединиться_к_линии(аргументы_команды: argparse.Namespace) -> dict[str, object]:
    ...
    if not первичная_рабочая_копия_чиста_для_старта(контекст.основной_корень):
        raise ОшибкаПула(
            73,
            "dirty_primary_bootstrap",
            "Продолжение не маршрутизируется при изменениях вне корневой .obsidian/.",
        )
    идентификатор_задачи = проверить_ограждение(аргументы_команды.идентификатор_задачи, "task-id")
