...
def сформировать_промпт_запуска(запись: dict[str, Any]) -> str:
    ...
    код_начальной_загрузки = (
        ...
        f'p="{путь_сценария}";'
        f'путь_ядра="{путь_ядра_миграции}";'
        'r=sys.argv[1];'
...
def получить_идентичность_закреплённой_загрузки() -> ИдентичностьЗагрузкиЯдраМиграции:
    ...
    if значение is None:
        ...
        if not isinstance(корень, str):
            raise ОшибкаПула(73, "migration_bootstrap_identity_invalid", "Корень legacy-bootstrap имеет неверный тип.")
        значение = (
