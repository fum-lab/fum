...
def завершить_временную_запись_перехода(
    ...
    *,
    ...
    if живая_ветка == исходная_ветка:
        if current_head(исходный_контекст.root) != исходная_вершина:
            raise QueueError(
                EXIT_HEAD_CHANGED,
                "head_changed",
                "HEAD изменился до переключения на ветку цепочки.",
            )
        run_git(
            исходный_контекст.root,
            ["symbolic-ref", "HEAD", целевая_ветка],
        )
    elif живая_ветка != целевая_ветка:
        raise QueueError(
            EXIT_CONTEXT,
            "branch_changed",
    ...
    if (
        symbolic_branch(исходный_контекст.root) != целевая_ветка
        or current_head(исходный_контекст.root) != исходная_вершина
    ):
        raise QueueError(
            EXIT_HEAD_CHANGED,
            "head_changed",
            "HEAD не закрепился на целевой ветке исходной вершины.",
        )
    ...
    команды = (
        ...
        f"verify {целевая_ветка} {исходная_вершина}\n"
    ...
    if результат.returncode != 0:
        ...
        if наблюдаемый_объект != итоговый_объект:
            подробности = результат.stderr.decode("utf-8", errors="replace").strip()
