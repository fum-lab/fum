...
После допуска сначала через safe HEAD-bootstrap пула вызови `повторить-ожидающие-публикации` со своими exact `task_id` и `generation`, затем прямо вызови `python3 Инструменты/fum-sleduyusjhij-shag-vetki/scripts/branch-next-step.py show --repo-root . --json`. Если ответ означает `done` или `not_ready`, ничего не пиши, останови всех писателей и выполни `finish-clean`. Если выбран готовый шаг, до его выполнения вызови `codex_app.set_thread_title` без `threadId` и составь название из точных полей ответа `card_id` и `title`: `FUM-STEP-NNNN — <краткое содержательное название>`. Название задачи не доказывает маршрут или допуск. Затем выполни точную карточку по новым правилам HEAD. Если твоя работа завершается `committed`, до собственного commit+handoff создай ровно одну новую сессию-продолжение этой же ветки и повтори весь этот протокол.
""".strip()
    if re.search(
        r"(?:^|[\s`'\"(])(?:/[^\s]|[A-Za-z]:[\\/]|\\\\)",
        промпт,
