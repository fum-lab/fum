...
def validate_ticket(ticket: object, *, owner: bool) -> dict[str, object]:
    ...
    барьерные_поля = {
        "базовая_вершина_барьера",
        "объект_реестра_барьеров",
    }
    присутствующие_барьерные_поля = барьерные_поля.intersection(ticket)
    if присутствующие_барьерные_поля and присутствующие_барьерные_поля != барьерные_поля:
        raise QueueError(
            EXIT_CONTEXT,
            "corrupt_queue",
            "Первый барьерный билет содержит неполное ограждение.",
        )
...
def задача_барьера_уже_вошла_в_очередь(
    ...
    if int(состояние_очереди["next_seq"]) > 1:
        return True
    владелец = состояние_очереди["owner"]
    if isinstance(владелец, dict) and владелец["task_id"] == идентификатор_задачи:
        return True
    if any(
        билет["task_id"] == идентификатор_задачи
        for билет in состояние_очереди["waiting"]
    ):
        return True
    завершение = состояние_очереди.get("last_completion")
    return (
...
def потребовать_открытый_барьер_предактивации(
    ...
    if идентификатор_задачи == задача_барьера:
        if барьер["состояние"] == "открыт":
            ...
            return объект_реестра, базовая_вершина
        raise QueueError(
            EXIT_OWNERSHIP,
...
def проверить_барьерное_ограждение_участника(
    ...
    if базовая_вершина is None:
        return
    if current_head(контекст.root) != базовая_вершина:
        raise QueueError(
            EXIT_HEAD_CHANGED,
            "несовпавшая_вершина_барьера",
