...
SYSTEM_RUNTIME_CODE_PATHS = frozenset(
    {
        ...
        "Прототипы/проверяемый-многоагентный-контур/Sources/FUMVerifiableMultiAgentContour/WritingSubnodeSystemRuntime.swift",
...
ИДЕНТИФИКАТОР_КОРНЕВОГО_АГЕНТА = "Codex " + "/" + "root"
ФОРМА_ИСПОЛНИТЕЛЯ_АГЕНТА = (
    rf"(?:{re.escape(ИДЕНТИФИКАТОР_КОРНЕВОГО_АГЕНТА)}|"
    rf"{re.escape(КОРЕНЬ_ИДЕНТИФИКАТОРА_СУБАГЕНТА)}{ФОРМА_ИМЕНИ_ЗАДАЧИ_СУБАГЕНТА})"
)
ШАБЛОН_ИСПОЛНИТЕЛЯ_АГЕНТА = re.compile(
    ФОРМА_ИСПОЛНИТЕЛЯ_АГЕНТА + r"\Z"
)
...
def _каноническая_запись_исполнителя(
    ...
    if (
        ...
        or ШАБЛОН_ИСПОЛНИТЕЛЯ_АГЕНТА.fullmatch(исполнитель) is None
...
def _границы_пути_исполнителя(исполнитель: str) -> tuple[int, int]:
    начало = исполнитель.index("/")
    return начало, len(исполнитель)


def _позиции_исполнителя_агента(
    путь: str,
    текст: str,
    ...
    if запись is not None:
        ...
        for номер, строка in enumerate(текст.splitlines(), start=1):
            ...
            if строка.startswith('  "исполнитель": "') and начало >= 0:
                начало_пути, конец_пути = _границы_пути_исполнителя(исполнитель)
                return frozenset(
                    {(номер, начало + начало_пути, начало + конец_пути)}
                )
        return frozenset()
    ...
    for номер in range(начала[0] + 1, концы[0]):
        строка = строки[номер]
        совпадение = re.match(
            rf"^\| \[({ФОРМА_ИСПОЛНИТЕЛЯ_АГЕНТА})\] "
            r".+\| .+ \| .+ \|$",
            строка,
        )
        if совпадение is None or совпадение.group(1) not in известные_исполнители:
            continue
        начало_пути, конец_пути = _границы_пути_исполнителя(совпадение.group(1))
        позиции.add(
            (
                номер + 1,
                совпадение.start(1) + начало_пути,
                совпадение.start(1) + конец_пути,
            )
        )
...
def scan_text(
    ...
    позиции_исполнителя = _позиции_исполнителя_агента(
        path,
        text,
