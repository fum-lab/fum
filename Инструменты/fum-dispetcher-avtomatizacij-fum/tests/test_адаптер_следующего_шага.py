...
def загрузить_исторический_активный_реестр() -> dict[str, object]:
    """Собрать автономную фикстуру снятого диспетчера поколения 3."""
    реестр = json.loads(КАНОНИЧЕСКИЙ_РЕЕСТР.read_text(encoding="utf-8"))
    реестр["поколение_реестра"] = 3
    for задание in реестр["задания"]:
        задание["состояние"] = "active"
        задание["поколение"] = 1
        задание["защита_поколения"]["поколение"] = 1
    return реестр
...
class АдаптерСледующегоШага(unittest.TestCase):
    ...
    def создать_репозиторий(
        ...
        реестр = загрузить_исторический_активный_реестр()
    ...
    def test_канонические_задания_диспетчера_сняты(
        сам,
    ) -> None:
        реестр = json.loads(КАНОНИЧЕСКИЙ_РЕЕСТР.read_text(encoding="utf-8"))

        сам.assertEqual(реестр["поколение_реестра"], 4)
        сам.assertEqual(len(реестр["задания"]), 2)
        задание = реестр["задания"][0]
        сам.assertEqual(задание["job_id"], "master.next-step")
        сам.assertEqual(задание["состояние"], "retired")
        ...
        сам.assertEqual(аналитическое["состояние"], "retired")
        сам.assertEqual(аналитическое["поколение"], 2)
        сам.assertEqual(
