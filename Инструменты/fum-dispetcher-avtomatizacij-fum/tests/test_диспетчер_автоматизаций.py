...
class КонтрактДиспетчераАвтоматизаций(unittest.TestCase):
    ...
    def test_канонические_задания_диспетчера_сняты(сам) -> None:
        результат = сам.выполнить(
            "проверить",
            "--реестр",
        ...
        сам.assertEqual(ответ["поколение_реестра"], 4)
        реестр = json.loads(
            КАНОНИЧЕСКИЙ_РЕЕСТР.read_text(encoding="utf-8")
        )
        сам.assertEqual(
            {задание["job_id"]: задание["состояние"] for задание in реестр["задания"]},
            {
                "master.next-step": "retired",
                "master.completed-step-analysis": "retired",
            },
        )
