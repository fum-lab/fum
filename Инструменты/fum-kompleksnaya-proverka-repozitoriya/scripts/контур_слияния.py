...
def граница_зависимостей(источник):
    путь = источник / "Инструменты/fum-otchyotyi-o-zapuskakh-proverok/scripts/граница_зависимостей_слияния.py"
    модуль = types.ModuleType("граница_зависимостей_контура")
    # Источник уже сверён с M: кэш байткода не является его частью.
...
def проверить_контур(источник: Path, коммит: str, кандидат: Path, ведущая: str) -> dict[str, str]:
    ...
    зависимости = граница_зависимостей(источник)
    профиль_зависимости = зависимости.проверить_объекты(прочитать_git, кандидат, коммит, ведущая)
    if (обычный_путь(кандидат, ".gitmodules").read_bytes()
            != прочитать_git(кандидат, "show", ":.gitmodules")):
        raise ValueError("рабочие байты .gitmodules кандидата отличаются от принятого индекса")
    ...
    if профиль_зависимости is not None:
        путь = кандидат / зависимости.ТЕЛЕГРАМ_ЗАВИСИМОСТЬ
        текущий = кандидат
        for часть in Path(зависимости.ТЕЛЕГРАМ_ЗАВИСИМОСТЬ).parts:
            текущий = текущий / часть
            if текущий.is_symlink() or not текущий.is_dir():
                raise ValueError("символическая ссылка или необычный каталог TDLib")
        if (Path(строка_git(путь, "rev-parse", "--show-toplevel")) != путь
                or строка_git(путь, "rev-parse", "HEAD") != профиль_зависимости["TDLib"]):
            raise ValueError("иная материализация закреплённой TDLib")
        проверить_байты_источника(путь, профиль_зависимости["TDLib"], (".",))
        if прочитать_git(путь, "ls-files", "--others", "--ignored", "--exclude-standard", "-z"):
            raise ValueError("игнорируемые данные в материализации TDLib")
