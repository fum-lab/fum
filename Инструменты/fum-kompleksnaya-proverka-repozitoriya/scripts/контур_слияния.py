...
def параметры_полного_запуска(
    ...
    while индекс < len(команда) and команда[индекс] in ("-B", "-E", "-I"):
        индекс += 1
    if индекс >= len(команда) or команда[индекс] != str(источник / СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ):
    ...
    for ключ in ("--источник-проверок", "--ведущая-основа"):
        if ПОЛНЫЙ_ИДЕНТИФИКАТОР_ОБЪЕКТА.fullmatch(значения[ключ]) is None:
...
def проверить_свидетельство(кандидат: Path, относительный: str, ожидаемое: dict) -> None:
    ...
    индекс = прочитать_гит(кандидат, "ls-files", "--stage", "-z", "--", относительный)
    ...
    if прочитать_гит(кандидат, "show", ":" + относительный) != байты:
...
def прочитать_гит(корень: Path, *аргументы: str) -> bytes:
...
def строка_гита(корень: Path, *аргументы: str) -> str:
    return прочитать_гит(корень, *аргументы).decode("utf-8").strip()
...
def проверить_инвентарь_реализации(корень: Path) -> None:
    ...
    for имя in прочитать_гит(корень, "ls-files", "--others", "-z", "--", "Инструменты").split(b"\0"):
    ...
    for запись in прочитать_гит(корень, "ls-files", "--stage", "-z", "--", "Инструменты").split(b"\0"):
        if not запись:
            continue
        заголовок, имя = запись.split(b"\t", 1)
        режим, идентификатор_объекта, стадия = заголовок.split()
...
def проверить_байты_источника(корень: Path, коммит: str, области: tuple[str, ...]) -> None:
    ...
    алгоритм = строка_гита(корень, "rev-parse", "--show-object-format")
    if алгоритм not in ("sha1", "sha256"):
        raise ValueError("неизвестный формат объектов проверяющего контура")
    дерево = прочитать_гит(корень, "ls-tree", "-rz", "--full-tree", коммит, "--", *области)
    if not дерево:
    ...
    фактические_пути = set(прочитать_гит(корень, "ls-files", "--cached", "--others", "-z", "--", *области).split(b"\0")) - {b""}
...
def проверить_контур(источник: Path, коммит: str, кандидат: Path, ведущая: str) -> dict[str, str]:
    ...
    for идентификатор_объекта in (коммит, ведущая):
        if not isinstance(идентификатор_объекта, str) or ПОЛНЫЙ_ИДЕНТИФИКАТОР_ОБЪЕКТА.fullmatch(идентификатор_объекта) is None:
            raise ValueError("контур слияния требует полные OID")
    ...
    for корень in (источник, кандидат):
        if Path(строка_гита(корень, "rev-parse", "--show-toplevel")) != корень:
            raise ValueError("нужен точный корень рабочего дерева")
        if строка_гита(корень, "rev-parse", "--is-shallow-repository") != "false":
            raise ValueError("неполная история проверяющего контура")
        подмены_истории = Path(строка_гита(корень, "rev-parse", "--path-format=absolute", "--git-path", "info/grafts"))
        if os.path.lexists(подмены_истории):
            raise ValueError("подмена истории через grafts")
    общий = строка_гита(источник, "rev-parse", "--path-format=absolute", "--git-common-dir")
    if (строка_гита(источник, "rev-parse", "--absolute-git-dir") != общий
            or строка_гита(кандидат, "rev-parse", "--path-format=absolute", "--git-common-dir") != общий
            or строка_гита(источник, "symbolic-ref", "HEAD") != "refs/heads/master"
            or строка_гита(источник, "rev-parse", "HEAD") != коммит
            or строка_гита(источник, "rev-parse", "refs/heads/master") != коммит):
        raise ValueError("источник проверок не является закреплённым первичным master")
    if строка_гита(кандидат, "rev-parse", "HEAD") != ведущая:
        raise ValueError("сдвинулась ведущая основа кандидата")
    вершина_слияния = Path(строка_гита(кандидат, "rev-parse", "--path-format=absolute", "--git-path", "MERGE_HEAD"))
    if вершина_слияния.is_symlink() or вершина_слияния.read_text().strip() != коммит:
    ...
    обёртка = строка_гита(источник, "rev-parse", коммит + ":" + АРХИВИРУЕМАЯ_ОБЁРТКА)
    if строка_гита(кандидат, "rev-parse", ведущая + ":" + АРХИВИРУЕМАЯ_ОБЁРТКА) != обёртка:
        raise ValueError("архивируемая обёртка кандидата отличается от master")
    запись_зависимости = прочитать_гит(источник, "ls-tree", "-z", коммит, "--", ЗАВИСИМОСТЬ)
    ...
    for корень in (источник, кандидат):
        ...
        if (Path(строка_гита(путь, "rev-parse", "--show-toplevel")) != путь
                or строка_гита(путь, "rev-parse", "HEAD") != зависимость
                or строка_гита(путь, "cat-file", "-t", зависимость) != "commit"):
    ...
    if (происхождение.get("схема") != "fum.политика-слияния.1"
            ...
            or происхождение.get("дерево_основы") != строка_гита(кандидат, "rev-parse", ведущая + "^{tree}")
    ...
    return {"схема": "fum.контур-проверки-слияния.1", "источник": коммит,
            "дерево_источника": строка_гита(источник, "rev-parse", коммит + "^{tree}"),
