...
""",
    ...
    def test_история_берётся_только_из_закрытого_хэшированного_снимка(сам):
        with tempfile.TemporaryDirectory() as временный:
            ...
            сам.assertEqual(статистика.успешные, 0)
            сам.assertEqual(статистика.неуспешные, 1)
            сам.assertEqual(статистика.суммарная_длительность_наносекунды, 2_000_000_000)
            сам.assertEqual(проверка_целостности.call_count, 1)

    def test_осиротевший_запуск_из_закрытого_снимка_не_становится_аналитикой(сам):
        with tempfile.TemporaryDirectory() as временный:
            корень = Path(временный)
            имя_сессии = "2026-08-15_00-00-00_MSK_проверить-осиротевший-запуск"
            сессия = f"Журнал/{имя_сессии}/запрос.md"
            каталог_сессии = корень / "Журнал" / имя_сессии
            ...
            осиротевшая = {
                ...
                "хэш_активной_записи": f"sha256:{'a' * 64}",
                "явное_признание_риска": True,
            }
            имя_записи = "000001_00000000-0000-4000-8000-000000000001.json"
            байты_записи = run_smoke_check.канонические_машинные_байты(осиротевшая)
            (каталог / имя_записи).write_bytes(байты_записи)
            снимок = {
                "схема": "fum.test-run-report.v1",
                "сессия": сессия,
            ...
            хэш_снимка = hashlib.sha256(байты_снимка).hexdigest()
            (каталог_сессии / "отчёт.md").write_text(
                "<!-- FUM-CHECK-RUNS:BEGIN состояние=закрыт; "
                "снимок=материалы/запуски-проверок/снимок.json; "
                f"sha256=sha256:{хэш_снимка} -->\n"
                "<!-- FUM-CHECK-RUNS:END -->\n",
                encoding="utf-8",
            )
