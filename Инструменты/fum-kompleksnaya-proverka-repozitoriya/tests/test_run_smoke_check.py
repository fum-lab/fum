...
"""
    ...
    def write_script_fixture(self, root: Path) -> None:
        for path in [
            ...
            root / "Инструменты" / "fum-svyaznostj-rabochej-sessii" / "scripts" / "проверить-охват-запроса.py",
...
""",
        ...
        with tempfile.TemporaryDirectory() as временный_каталог:
            ...
            сам.assertEqual(
                ...
                [
                    ...
                    "Ранняя проверка охвата запроса",
    ...
    def test_builds_full_plan_from_local_automation_tests(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [шаг.name for шаг in упорядоченные[:17]],
                сам_ранний_префикс,
            )
            self.assertTrue(
                all(шаг.ранняя_проверка for шаг in упорядоченные[:17])
            )
            self.assertEqual(
                [шаг.name for шаг in упорядоченные[17:]],
