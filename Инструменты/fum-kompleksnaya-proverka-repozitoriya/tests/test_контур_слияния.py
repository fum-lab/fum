...
class КонтурСлияния(unittest.TestCase):
    ...
    def test_замена_основной_ветки_и_неполный_идентификатор_объекта_отклоняются(сам):
        вершина = сам.вершина
    ...
    def test_одинаковый_идентификатор_объекта_не_заменяет_режим_ссылки_зависимости(сам):
        ...
        путь_состояния_слияния = Path(сам.гит(сам.кандидат, "rev-parse", "--path-format=absolute", "--git-path", "MERGE_HEAD"))
        путь_состояния_слияния.write_text(сам.вершина + "\n")
    ...
    def test_свидетельство_заранее_индексировано_и_связано_с_идентификатором_запуска(сам):
        запрос = "Журнал/2026-09-10_00-00-00_MSK/запрос.md"
        идентификатор_запуска = "00000000-0000-4000-8000-000000000001"
        относительный = str(Path(запрос).parent / "материалы/контур-слияния.json")
        путь = сам.кандидат / относительный
        путь.parent.mkdir(parents=True)
        свидетельство = сам.проверить() | {"запрос": запрос, "идентификатор_запуска": идентификатор_запуска}
        ...
        сам.модуль.проверить_свидетельство(сам.кандидат, относительный, свидетельство)
        объект_файла = сам.гит(сам.кандидат, "rev-parse", ":" + относительный)
        сам.гит(сам.кандидат, "update-index", "--cacheinfo", "120000," + объект_файла + "," + относительный)
        with сам.assertRaises(ValueError):
            сам.модуль.проверить_свидетельство(сам.кандидат, относительный, свидетельство)
        сам.гит(сам.кандидат, "update-index", "--cacheinfo", "100644," + объект_файла + "," + относительный)
        with сам.assertRaises(ValueError):
    ...
    def test_полный_запуск_третьей_версии_использует_основной_источник_на_кандидате(сам):
        сам.полный_запуск(контур=True)

    def test_обычный_полный_запуск_не_подтверждает_происхождение_из_основной_ветки(сам):
    ...
    def полный_запуск(сам, *, контур, сбой_после=False):
        ...
        for относительный in (сам.модуль.СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ, сам.модуль.СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ.with_name("контур_слияния.py"), Path(обёртка),
        ...
        идентификатор_запуска = "00000000-0000-4000-8000-000000000099"
        относительный = str(Path(запрос).parent / "материалы/контур-слияния.json")
        путь = сам.кандидат / относительный
        путь.parent.mkdir(parents=True)
        ожидаемое = сам.модуль.канонические_байты(сам.проверить() | {"запрос": запрос, "идентификатор_запуска": идентификатор_запуска})
        подготовка = subprocess.run([sys.executable, "-B", str(сам.источник / сам.модуль.СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ.with_name("контур_слияния.py")),
                                    "--корень-кандидата", str(сам.кандидат), "--источник", сам.вершина,
                                    "--ведущая-основа", сам.ведущая, "--запрос", запрос, "--идентификатор-запуска", идентификатор_запуска], capture_output=True)
        ...
        команда = [sys.executable, "-B", str(сам.источник / сам.модуль.СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ), "--repo-root", str(сам.кандидат),
                   "--request", запрос, "--commit-message-file", str(сообщение), "--codex-thread-id", идентификатор_запуска,
        ...
        if контур:
            ...
            команда = команда[:-6]
            команда[2] = str(сам.кандидат / сам.модуль.СКРИПТ_КОМПЛЕКСНОЙ_ПРОВЕРКИ)
        вызов = [sys.executable, "-E", "-B", str(источник_запуска / обёртка), "запустить", "--корень-репозитория", str(сам.кандидат),
                               ...
                               "--идентификатор-запуска", идентификатор_запуска, "--", *команда]
        ...
        сам.assertEqual(допуск["происхождение_проверок"]["идентификатор_запуска"], идентификатор_запуска)
        сам.assertEqual(допуск["связь"]["родители"], [сам.ведущая, сам.вершина])
        сам.assertFalse(допуск["завершение_обязательства_доказано"])
        исходное_сопровождающее_свидетельство = путь.read_bytes()
        варианты = [None, исходное_сопровождающее_свидетельство + b" ", сам.модуль.канонические_байты(json.loads(исходное_сопровождающее_свидетельство) | {"идентификатор_запуска": "00000000-0000-4000-8000-000000000100"}),
                    сам.модуль.канонические_байты(json.loads(исходное_сопровождающее_свидетельство) | {"источник": сам.ведущая}),
                    сам.модуль.канонические_байты(json.loads(исходное_сопровождающее_свидетельство) | {"лишнее": True})]
        for номер, содержимое in enumerate(варианты):
            ...
            with сам.subTest(номер=номер):
                сам.assertEqual(итог.returncode, 1, итог.stdout + итог.stderr)
            путь.write_bytes(исходное_сопровождающее_свидетельство)
