...
def git(рабочий_корень, *аргументы):
    """Сохранённый вход M с динамическим вызовом русской реализации."""
    return выполнить_гит(рабочий_корень, *аргументы)


def идентификатор_объекта(рабочий_корень, ссылка):
    return git(рабочий_корень, "rev-parse", "--verify", ссылка).decode().strip()


def дерево(рабочий_корень, ссылка):
    результат = {}
    каталоги = []
    for запись in git(рабочий_корень, "ls-tree", "-rtz", ссылка).split(b"\0"):
        if запись:
...
def снимок_объекта(рабочий_корень, ссылка, кэш):
    ...
    if ключ_кэша not in кэш:
        кэш[ключ_кэша] = (дерево(рабочий_корень, ссылка), идентификатор_объекта(рабочий_корень, ссылка + "^{tree}"),
                      git(рабочий_корень, "rev-parse", "--show-object-format").decode().strip())
...
def проверить_копию(рабочий_корень, ссылка, кэш=None):
    ...
    фактическое = {}
    for запись in git(рабочий_корень, "ls-files", "--stage", "-z").split(b"\0"):
        if not запись:
            continue
        метаданные, имя = запись.split(b"\t", 1)
    ...
    if фактическое != ожидаемое or git(рабочий_корень, "write-tree").decode().strip() != идентификатор_дерева:
        raise Отказ("Индекс не равен точному дереву")
    for запись in git(рабочий_корень, "ls-files", "-v", "-z").split(b"\0"):
    ...
    for имя, (режим, значение) in ожидаемое.items():
        ...
        if режим == "160000":
            ...
            if Path(git(путь, "rev-parse", "--show-toplevel").decode().strip()).resolve() != путь.resolve():
                raise Отказ("Неверный физический корень gitlink")
            проверить_копию(путь, значение, кэш)
            if git(путь, "ls-files", "--others", "--ignored", "--exclude-standard", "-z"):
    ...
    if git(рабочий_корень, "ls-files", "--others", "--exclude-standard", "-z"):
...
def проверить_границу(рабочий_корень, прежний_коммит, новый_коммит, кэш=None):
    кэш = {} if кэш is None else кэш
    if git(рабочий_корень, "rev-parse", "--is-shallow-repository").strip() != b"false":
        raise Отказ("Shallow-история")
    общий_каталог = Path(git(рабочий_корень, "rev-parse", "--git-common-dir").decode().strip())
    ...
    игнорируемые_пути = git(рабочий_корень, "ls-files", "--others", "--ignored", "--exclude-standard", "-z")
...
def перейти(рабочий_корень, прежний_коммит, ведущий_коммит, новый_коммит, путь_намерения, корень_транзакции, *, наблюдатель=lambda фаза: None):
    ...
    if git(рабочий_корень, "symbolic-ref", "HEAD").strip() != b"refs/heads/master":
        raise Отказ("HEAD не указывает на master")
    родители = git(рабочий_корень, "show", "-s", "--format=%P", новый_коммит).decode().strip().split()
    ...
    общий_каталог = git(рабочий_корень, "rev-parse", "--path-format=absolute", "--git-common-dir").strip()
    if (общий_каталог != git(рабочий_корень, "rev-parse", "--absolute-git-dir").strip()
            or общий_каталог != git(корень_транзакции, "rev-parse", "--path-format=absolute", "--git-common-dir").strip()
            or общий_каталог == git(корень_транзакции, "rev-parse", "--absolute-git-dir").strip()
    ...
    try:
        ...
        git(рабочий_корень, "read-tree", "-m", "-u", прежний_коммит, новый_коммит)
        ...
        if процесс.returncode or идентификатор_объекта(рабочий_корень, "HEAD") != новый_коммит or git(рабочий_корень, "symbolic-ref", "HEAD").strip() != b"refs/heads/master":
        ...
        try:
            ...
            состояние["фактическое_дерево_индекса"] = git(рабочий_корень, "write-tree").decode().strip()
...
# Публичные имена принятой основы сохраняют совместимость вызывающего кода.
команда_git = команда_гита
