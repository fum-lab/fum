...
class ПроверкаМедиапакета(unittest.TestCase):
    ...
    def test_ненулевые_комиссии_возвраты_и_расходы_вычитаются(self):
        запись = self.свидетельство()
        запись['операции']['комиссии']['сумма'] = '10,00'
        запись['операции']['возвраты']['сумма'] = '20,00'
        запись['операции']['расходы']['сумма'] = '30,00'
        данные = self.финансовый_вход(запись)
        данные['отчёт'].update(комиссии=1000, возвраты=2000, расходы=3000)
        результат = собрать(self.корень, данные)
        self.assertEqual(результат['конечный_остаток_копейки'], 24000)
        self.assertIn('Конечный остаток: 240.00 ₽', результат['черновики']['отчёт'])
