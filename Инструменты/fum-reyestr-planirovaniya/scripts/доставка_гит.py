...
from приём_направления import безопасный_путь, требовать, идентификатор_задачи, поля
...
def проверить_назначение(назначение):
    поля(назначение, {"корень", "ref", "владелец", "корневая_задача"})
    требовать(назначение["ref"] == "refs/heads/fuma"
              and назначение["корневая_задача"] == "01a07d3d-d376-7ad2-aafc-67e4c25a67eb"
              and идентификатор_задачи(назначение["владелец"]), "Нет точного назначения постоянной задачи FUMA")
    путь = назначение["корень"]
    требовать(isinstance(путь, str) and Path(путь).is_absolute() and str(Path(путь)) == путь,
              "Неверный корень назначения")
    return назначение


def допуск_ветки(корень, задача, ветка, назначение=None):
    требовать(isinstance(ветка, str), "Нужен полный ref")
    if ветка == "refs/heads/fuma":
        проверить_назначение(назначение)
        требовать((str(корень), задача, ветка) == (назначение["корень"], назначение["владелец"], назначение["ref"]),
                  "Дерево или владелец отличаются от назначения fuma")
    else:
        требовать(ветка.startswith("refs/heads/codex/"), "Нужна собственная codex-ветка либо точное назначение fuma")


def снимок(корень, задача, ветка, назначение=None):
    корень = физический_корень(корень)
    требовать(идентификатор_задачи(задача), "Нужен UUID владельца")
    допуск_ветки(корень, задача, ветка, назначение)
    ...
    деревья = выполнить(корень, "worktree", "list", "--porcelain", "-z").stdout.decode().split("\0\0")
    записи = [запись.split("\0") for запись in деревья if запись]
    владельцы = [запись for запись in записи if "branch " + ветка in запись]
    требовать(len(владельцы) == 1 and владельцы[0][0] == "worktree " + str(корень)
              and "HEAD " + коммит in владельцы[0], "Ref не закреплён единственным образом за указанным физическим деревом")
...
def поддержанное_дерево(корень, источник, *, конфликт=False):
    ...
    for путь, объект in ссылки[0].items():
        ...
        if (папка / ".git").exists():
            ...
            поддержанное_дерево(папка, объект.decode())
            чисто(папка)
