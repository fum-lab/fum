...
final class ПроверкиКанала: XCTestCase {
    func testПринятыйНеблокирующийДескрипторНормализуется() throws {
    ...
    func testКадрСКириллицейИОтказОтПустогоКадра() throws {
        var файлы: [Int32] = [-1, -1]
    ...
    func testMACНеПринимаетЧужойКлючИИзменённыйКадр() throws {
    ...
    func testОжиданиеКадраОграниченоОбщимВременем() throws {
        var файлы: [Int32] = [-1, -1]
        XCTAssertEqual(socketpair(AF_UNIX, SOCK_STREAM, 0, &файлы), 0)
        defer { файлы.forEach { close($0) } }
        файлы.forEach { Канал.настроить($0) }
