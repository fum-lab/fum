...
final class ПроверкиОбраза: XCTestCase {
    func testПодменаИДубликатТаблицыОтклоняются() throws {
    ...
    func testПодписьПроверяетсяПоМашинномуСтатусуИПолномуКлючу() {
    ...
    func testОтпечатокКлючаНеБерётсяИзПроизвольногоUID() {
        let ключ = "D2EB44626FDDC30B513D5BB71A5D6C4C7DB87C81"
    ...
    func testРазмерQCOWНеМожетБытьБулевымДробнымИлиВнешним() throws {
        XCTAssertEqual(try размерQCOW(Data("{\"format\":\"qcow2\",\"virtual-size\":4294967296}".utf8), предел: 32 << 30), 4 << 30)
        for текст in ["{\"format\":\"qcow2\",\"virtual-size\":true}", "{\"format\":\"qcow2\",\"virtual-size\":1.5}", "{\"format\":\"qcow2\",\"virtual-size\":1024,\"backing-filename\":\"чужое\"}"] {
            XCTAssertThrowsError(try размерQCOW(Data(текст.utf8), предел: 32 << 30))
        }
    }
}
