...
final class ПроверкиПлана: XCTestCase {
    ...
    func testНедостатокРесурсовИЧужойХост() {
        var плохие = хорошие
    ...
    func testПланДетерминированИНеСоздаётКаталог() throws {
    ...
    func testИзменениеПланаОтклоняется() throws {
    ...
    func testПересчитанныйХэшНеРазрешаетНедопустимыйПуть() throws {
