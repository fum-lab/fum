...
public final class КлиентМашины {
    ...
    public func подключитьсяКГостю(_ команда: String?) throws -> Never {
        let живой = try состояние()
        let аргументы = ["/usr/bin/ssh", "-F", читатель.паспорт.план.каталог + "/идентичность/ssh_config", "-p", String(живой.портДоступаКГостю), "fum"] + (команда.map { [$0] } ?? [])
