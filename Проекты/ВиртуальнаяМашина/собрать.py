...
def выполнить_команду(аргументы, *, предел=600):
    процесс = subprocess.Popen(аргументы, stdout=subprocess.PIPE, stderr=subprocess.PIPE, start_new_session=True)
    try:
        вывод, ошибки = процесс.communicate(timeout=предел)
        if процесс.returncode != 0:
            raise ValueError('Команда завершилась с кодом ' + str(процесс.returncode) + ': ' + ошибки[-4000:].decode(errors='replace'))
        return вывод
    finally:
...
def собрать(проект, сборка, поставка, свифт, *, система='xcode', выполнить=выполнить_команду):
    ...
    try:
        ...
        return собрать_под_замком(проект, сборка, поставка, свифт, система, выполнить)
    finally: os.close(замок)


def собрать_под_замком(проект, сборка, поставка, свифт, система, выполнить):
    ...
    вход = {'исходники': исходники, 'упаковщик': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
