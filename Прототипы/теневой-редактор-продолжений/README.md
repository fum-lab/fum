...
Общий [smoke-check репозитория](../../Инструменты/fum-smoke-check/SKILL.md) автоматически находит пакет, сверяет оба исполняемых продукта с проверяемым инвентарём, запускает 30 автономных тестов и отдельно собирает оба продукта. Строгий `swift format lint` для исторического снимка пакета временно заменён видимым хэш-привязанным исключением в [политике SwiftPM-пакетов](../../Инструменты/fum-smoke-check/swift-package-policy.json): любое изменение Swift-исходников, манифеста или центральной конфигурации форматтера делает исключение устаревшим. Критерий снятия — отдельная механическая нормализация всего пакета, после которой строгий lint, тесты и обе сборки остаются зелёными.
...
<!-- content-sha256: sha256:6e1d262d0da6bc02977b137ac4acfeff5db57e14f34a1da045f9eb2efa467fee -->
<!-- FUM-MD-RECENCY:END -->
