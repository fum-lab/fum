...
public enum LocalRuntimeError: Error, Equatable, Sendable, CustomStringConvertible, LocalizedError {
  ...
  public var description: String {
    ...
    case .invalidModelName:
      return
        "Имя модели Ollama имеет недопустимый формат. Используйте каноническое имя из `ollama list`."
    case .invalidModelOutput(.echoedContext):
      return
        "Локальная LLM повторила замороженный префикс и не вернула надёжно отделимое продолжение."
    ...
    for pathComponent in pathComponents {
      ...
      for component in colonComponents {
        ...
        guard let first = bytes.first,
          ...
          bytes.allSatisfy(isAllowedModelNameByte)
        else {
          throw LocalRuntimeError.invalidModelName
        }
      }
