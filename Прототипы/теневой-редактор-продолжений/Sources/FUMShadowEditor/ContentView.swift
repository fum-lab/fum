      ...
      VStack(alignment: .leading, spacing: 16) {
        ...
        GroupBox("Суффиксно-контекстный индекс") {
          VStack(alignment: .leading, spacing: 5) {
            ...
            Text(
              "Глубина: \(model.indexConfiguration.maxDepth), бюджет: \(model.indexConfiguration.maxNodes)"
            )
            .font(.caption)
            .foregroundStyle(.secondary)
          }
        ...
        GroupBox("Граница интерпретации") {
          Text(
            "Прототип измеряет наблюдаемое расхождение двух текстовых продолжений и одинаково построенных структур. Это не прямое чтение мысли человека и не доказательство равенства или различия внутренних состояний."
          )
          .font(.caption)
          .frame(maxWidth: .infinity, alignment: .leading)
        }
        ...
        GroupBox("Локальная трасса") {
          VStack(alignment: .leading, spacing: 8) {
            Toggle("Сохранять завершённые сравнения", isOn: $model.persistComparisonTrace)
              .toggleStyle(.checkbox)
            Text(
              "По умолчанию выключено. При включении рядом с документом сохраняются два коротких продолжения и метрики, но не модельный контекст."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
