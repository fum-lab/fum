...
final class ТестыСканераРепозитория: XCTestCase {
  ...
  func test_маркерыИКомментарииВнутриОградыНеСкрываютДокумент() throws {
    try воВременномКаталоге { корень in
      let текст = """
        ```text
        <!--
        ...
        [видимая ссылка](цель.md)
        """
      try записать(текст, в: корень.appendingPathComponent("ограда.md"))

      let снимок = try СканерРепозитория().сканировать(корень: корень)
      let документ = снимок.узел(идентификатор: "ограда.md")
  ...
  func test_пропускаетДокументПревысившийПределРазмера() throws {
    try воВременномКаталоге { корень in
      try записать(
        String(repeating: "б", count: 256),
        в: корень.appendingPathComponent("большой.md")
      )
      try записать("# Малый", в: корень.appendingPathComponent("малый.md"))

      let пределы = ПределыСканирования(
        максимальноеЧислоПутей: 100,
        максимальноеЧислоДокументов: 100,
        максимальнаяГлубина: 8,
        максимальныйОбъёмДокумента: 64,
        максимальныйОбщийОбъём: 128
      )
      let снимок = try СканерРепозитория(пределы: пределы).сканировать(корень: корень)

      XCTAssertEqual(снимок.числоДокументов, 1)
      XCTAssertNotNil(снимок.узел(идентификатор: "малый.md"))
      XCTAssertEqual(снимок.пропущенныеПути, ["большой.md"])
    }
  }

  func test_общийПределОбъёмаПрименяетсяВДетерминированномПорядке() throws {
    try воВременномКаталоге { корень in
      try записать(
        String(repeating: "x", count: 40),
        в: корень.appendingPathComponent("а.md")
      )
      try записать(
        String(repeating: "y", count: 40),
        в: корень.appendingPathComponent("б.md")
      )
      ...
      let пределы = ПределыСканирования(
        ...
        максимальнаяГлубина: 8,
        максимальныйОбъёмДокумента: 64,
        максимальныйОбщийОбъём: 60
      )
      let снимок = try СканерРепозитория(пределы: пределы).сканировать(корень: корень)

      XCTAssertEqual(снимок.узлы.filter { $0.вид == .документ }.map(\.id), ["а.md"])
      XCTAssertEqual(снимок.пропущенныеПути, ["б.md"])
    }
  }

  func test_ограничиваетГлубинуИЧислоПосещённыхПутей() throws {
    try воВременномКаталоге { корень in
      try записать("# Корневой", в: корень.appendingPathComponent("корневой.md"))
      let уровень = корень.appendingPathComponent("уровень", isDirectory: true)
      try FileManager.default.createDirectory(at: уровень, withIntermediateDirectories: true)
      try записать("# Глубокий", в: уровень.appendingPathComponent("глубокий.md"))

      let пределГлубины = ПределыСканирования(
        максимальноеЧислоПутей: 100,
        максимальноеЧислоДокументов: 100,
        максимальнаяГлубина: 1,
        максимальныйОбъёмДокумента: 1_024,
        максимальныйОбщийОбъём: 2_048
      )
      let снимок = try СканерРепозитория(пределы: пределГлубины)
        .сканировать(корень: корень)
      XCTAssertEqual(снимок.числоДокументов, 1)
      XCTAssertEqual(снимок.пропущенныеПути, ["уровень"])

      let пределПутей = ПределыСканирования(
        максимальноеЧислоПутей: 1,
        максимальноеЧислоДокументов: 100,
        максимальнаяГлубина: 8,
        максимальныйОбъёмДокумента: 1_024,
        максимальныйОбщийОбъём: 2_048
      ...
      XCTAssertThrowsError(
        try СканерРепозитория(пределы: пределПутей).сканировать(корень: корень)
      ) { ошибка in
        XCTAssertEqual(
          ошибка as? ОшибкаСканирования,
          .превышеноМаксимальноеЧислоПутей(1)
        )
      }
    }
  }
