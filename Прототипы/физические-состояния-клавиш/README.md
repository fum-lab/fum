...
`FUMInputGuide` получает актуальный путь рабочей копии из полного компиляционного литерала `#filePath`, поднимается до корня и принимает его только при одновременном наличии `AGENTS.md`, `.git` и точного `Прототипы/физические-состояния-клавиш/Package.swift`. Если маркеры не подтверждены, GUI отказывает в записи. Абсолютный путь показывается человеку, но не сериализуется в данные.
...
- fail-closed-поиск корня репозитория по `#filePath`, обязательным маркерам и отклонение символьной ссылки каталога записи;
...
- Корень репозитория берётся из значения `#filePath`, встроенного при сборке. Перемещённый уже собранный бинарник закрывает запись, если прежний путь больше не подтверждается; автоматический выбор другого каталога намеренно отсутствует.
...
<!-- content-sha256: sha256:685ea91defa7bfe77e8d9ce0bcb2817b08f7eb1720df9fd2596ab54435b4892e -->
<!-- FUM-MD-RECENCY:END -->
