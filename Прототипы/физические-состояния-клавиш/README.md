...
Этот Swift-прототип выбирает стек наблюдения клавиатуры для требования о [максимально сырой записи событий устройств ввода](../../Требования/🚧-максимально-сырая-запись-событий-устройств-ввода.md). Результат первого этапа — гибридная архитектура: `IOHIDManager` как первичный macOS-источник, `GCKeyboard` как переносимый слой платформ Apple, а `CGEventTap` и `NSEvent` как диагностические источники для измерения потерь и поведения системного потока.
...
Воспроизводимая матрица и предварительная рекомендация:
...
swift run \
  --package-path Прототипы/физические-состояния-клавиш \
  FUMInputProbe record \
  --source iohid-manager \
  --seconds 30
```

Доступны также `gc-keyboard`, `cg-event-tap` и `ns-event`. Последние два требуют разрешения macOS Input Monitoring. Успешный запуск не означает согласия на долговременное хранение или передачу трассы.

## Проверенный результат

Тесты были сначала запущены в красном состоянии при отсутствующем ядре и фабриках платформенных наблюдений. Реализация доведена до прохождения 16 автономных тестов. Они проверяют:
...
Переносимое Swift-ядро, четыре macOS-адаптера, двухслойная фильтрация автоповтора, версия схемы, headless-пробник и автономные тесты уже реализованы. Карточка требования переведена в статус `🚧`.
...
После физических измерений схема расширяется событиями подключения, отключения, конфигурации, разрыва и переполнения. Хранилище получает атомарную дозапись, версию адаптера и API, политику срока хранения, шифрование чувствительного клавиатурного потока и проверяемое удаление. Диагностика остаётся отделена от первичных переходов.
...
- `IOHIDManager` реализован только для клавиатурной usage page; мышь, трекпад, стилус и планшет остаются следующими адаптерами этой карточки.
...
- `GCKeyboard.coalesced` не сохраняет идентичность нескольких физических клавиатур.
...
Статус: действующий исследовательский прототип и принятый предварительный выбор стека. Каркас и автоматическое сравнение работают; завершение требования зависит от физических прогонов и переносимых приложений-стендов.
...
<!-- last-content-edit: 2026-07-17 11:00:20 MSK -->
<!-- content-sha256: sha256:5d569bfa4998d3ff0c2d62a943ad769effff4dd0602c23a016140768e09db806 -->
<!-- FUM-MD-RECENCY:END -->
