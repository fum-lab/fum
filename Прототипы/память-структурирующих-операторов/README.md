...
## Ограниченное операторное внимание

[Два определения внимания](операторное-внимание.md) расширяют тот же `AutomationExecutor` схемой `fum.определение-графа.2`: обязательная ссылка README и потребность включения поставки через fuma к master. Вход, предыдущая память и параметры чувствительности передаются как JSON. Результат содержит причину, текущие свидетельства, обязательные условия, вклады, трассу и предложение действия. Проверяемая поставка этого расширения отмечается отдельно от прежних результатов v1.
...
<!-- content-sha256: sha256:760d7e4ccbb057d61b7444c5e2542d6da083d431baa105bc591d3e89c3a54bef -->
<!-- FUM-MD-RECENCY:END -->
