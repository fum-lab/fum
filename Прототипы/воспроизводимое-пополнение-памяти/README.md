...
`MemoryGeneration` версии `1` сохраняет версию политики `fum.memory.policy.v1`, SHA-256 входа текущего перехода, ссылку на предыдущее поколение, канонические снимок, трассу и модель представления с их хэшами, а также идентификаторы входных и всех принятых событий, версию интерпретатора и версию оператора проекции. Хэш самого канонического файла становится идентификатором неизменяемого поколения. Полные тела принятых событий в поколение версии `1` не входят, поэтому хэши и идентификаторы ещё не образуют самодостаточный журнал для повторного исполнения редукторов.
...
Продолжение восстанавливает записи и трассу из подтверждённого поколения, требует тот же `dataset_id`, неизменный префикс трассы и записей, продолжает `sequence`, запрещает повторные события и цели и снова проверяет все хэши и происхождение. Валидатор связывает каждый шаг трассы с единственной породившей записью, её хэшем и упорядоченными событиями-вкладами, но не вычисляет `remember` и `compose` заново из тел событий. Автономный тест сопоставляет полный replay отдельно переданных шести событий с запусками четырёх базовых и двух продолжающих событий: снимок, трасса и модель представления совпадают канонически. Это доказательство сходимости путей по внешней фикстуре, а не самодостаточное [воспроизведение принятого эпизода FUM](../../Глоссарий/воспроизведение-принятого-эпизода-FUM.md) из поколения.
...
Автономные тесты проверяют побайтовую повторяемость в текущем Swift-runtime, наблюдаемое изменение снимка после изменения входа, сортировку записей, происхождение результата `compose`, связность и хэши трассы, отказ при неизвестном чтении, запрет повторной цели, непрерывность событий, логически атомарный отказ до `CURRENT`, отклонение неродственного преемника, восстановление между штатно завершившимися процессами, сходимость полного и инкрементального пути, происхождение модели представления и обратное версионированное событие намерения.
...
Прототип доказывает логическую атомарность файлового указателя на уровне операции замены и восстановление между штатно завершившимися процессами, но не самодостаточное переисполнение сохранённых событий, межпроцессный CAS, process-crash- или power-loss-durability, языконейтральность канонических байтов, миграцию совместимого поколения, сборку мусора, масштабирование, разграничение доступа, качество автоматически найденных структурирующих операторов, агентский цикл или способность FUM самостоятельно синтезировать следующий исполняемый пакет. Фикстура является версионированным инженерным сценарием, а не записью живой модели.

Статус: действующий безоконный Swift-прототип со структурно проверяемыми поколениями, сходящимися по внешней фикстуре полным и продолжающим путями и инертной декларативной моделью представления; самодостаточный replay поколений, CAS, аварийная durability, GUI и renderer отсутствуют.
...
<!-- content-sha256: sha256:498ad2d7f8666398bf386abe4a0ef0f35b0bac2cec012faea45a0834c3a2a3b8 -->
<!-- FUM-MD-RECENCY:END -->
