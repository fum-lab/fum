...
Файловая часть выделена в публичный схемонезависимый `ContentAddressedGenerationStore`. Он один владеет адресацией поколений, указателем `CURRENT`, межпроцессным CAS и аварийной финализацией, а `MemoryGenerationStore` сохраняет прежний API как доменный адаптер схемы памяти. То же ядро может принять другой канонический носитель с собственными валидаторами схемы и линии происхождения; семантика `remember` и `compose` при этом не переносится в общий слой.
...
`ContentAddressedGenerationStore` использует следующий порядок фиксации, а `MemoryGenerationStore` добавляет к нему проверку схемы и происхождения памяти:
...
Отдельный обобщённый тест подтверждает, что то же ядро сохраняет не относящееся к памяти каноническое поколение, продолжает его через `CURRENT`, идемпотентно принимает точный повтор и отклоняет устаревший CAS, не меняя подтверждённое состояние. Прежние тесты памяти и общий Swift↔Python corpus проходят через адаптер без изменения закреплённых байтов.
...
## Переиспользование ядра общей памятью эпизода

`Прототипы/проверяемый-многоагентный-контур` подключает этот пакет как прямую локальную SwiftPM-зависимость. Он переиспользует `CanonicalMemoryJSON` и `ContentAddressedGenerationStore` без изменения байтового профиля, `CURRENT`, межпроцессного CAS и восьми аварийных точек. Доменные поколение, событие вклада, происхождение, reducer и проверка точного префикса остаются в пакете-потребителе и не подменяются операциями `remember` и `compose`.

Поставщик не требует от потребителя сканировать `generations/` и не удаляет неизвестные файлы. Оба доменных адаптера должны использовать разные физические корни: `CURRENT` закрепляет канонический профиль, но не доменный тип поколения.

## Структура

- `Sources/FUMReproducibleMemoryPopulation/` — типизированный вход, закрытый интерпретатор, поколения, схемонезависимое адресное ядро, доменный адаптер памяти, декларативная проекция, канонический кодировщик и три встроенные фикстуры;
...
- [исходный запрос о подтверждённом хранилище и безоконных интерфейсах эпизода](../../Журнал/2026-08-01_11-56-54_MSK_реализовать-подтверждённое-хранилище-и-безоконные-интерфейсы-эпизода/запрос.md)
...
- [исходный запрос о канонических событиях и самодостаточном replay](../../Журнал/2026-07-27_22-17-40_MSK_сохранить-канонические-события-и-доказать-воспроизведение/запрос.md)
...
- [исходный запрос о начальной стадии без GUI с управлением через Codex](../../Журнал/2026-07-27_20-10-35_MSK_разрешить-начальную-коробочную-FUM-без-GUI-через-Codex/запрос.md)
- [исходный запрос о восстанавливаемых поколениях и декларативной проекции](../../Журнал/2026-07-25_09-09-06_MSK_добавить-восстанавливаемые-поколения-памяти-и-декларативную-GUI-проекцию/запрос.md)
- [исходный запрос о начальном безоконном прототипе](../../Журнал/2026-07-24_10-44-28_MSK_начать-безоконный-Swift-прототип-воспроизводимого-пополнения-памяти-FUM/запрос.md)
...
<!-- content-sha256: sha256:fdcb67e1a9e3e04aca6a82ce2c3804ba65c3150eef630c442284c52806261f02 -->
<!-- FUM-MD-RECENCY:END -->
