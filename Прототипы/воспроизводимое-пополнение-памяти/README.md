...
flowchart LR
    ...
    generation --> cas["Межпроцессный CAS ожидаемого родителя"]
    cas --> current["Атомарный указатель CURRENT"]
...
`MemoryGenerationStore` сначала проверяет кандидата, записывает неизменяемый файл `generations/<sha256>.json` и только затем входит в межпроцессную CAS-секцию. Постоянный sidecar `CURRENT.lock` получает эксклюзивную POSIX record lock; под ней хранилище заново читает и полностью проверяет `CURRENT`, сравнивает его хэш с закреплённым `previous_generation_sha256` кандидата и атомарно заменяет указатель только при совпадении. Точный повтор уже подтверждённого хэша возвращает идемпотентный успех, а кандидат от устаревшего родителя получает типизированный конфликт. Подготовленный проигравшим файл остаётся безопасным адресуемым сиротой: он не меняет `CURRENT` и не удаляет поколение победителя.

Блокировка является advisory и обеспечивает CAS только для сотрудничающих процессов, использующих этот протокол на локальной файловой системе с работающими POSIX record locks и атомарной заменой файла. `CURRENT.lock` сохраняется на месте и не удаляется между операциями. Произвольные потоки одного процесса, старые писатели, обходящие протокол, и сетевые файловые системы этой проверкой не покрыты. Линеаризуемое решение также не доказывает синхронизацию данных и метаданных при аварии процесса или потере питания.
...
Автономные тесты дополнительно проверяют канонические тела событий и их связь с трассой, самодостаточный replay начального и продолженного поколений, накопленный журнал больше 1 МиБ, явный немутирующий отказ схемы `1` и две внутренне хэш-согласованные подделки `remember` и `compose`, которые отклоняются только после переисполнения. Отдельный сценарий запускает два реальных процесса с разными кандидатами от одного родителя: ровно один публикует поколение, второй получает типизированный конфликт, а его точный адресуемый объект остаётся неподтверждённым. Дополнительная проверка удерживает `CURRENT.lock` в родительском процессе и доказывает, что дочерний писатель не проходит CAS до освобождения блокировки. Побайтовая повторяемость в текущем Swift-runtime, ограничения входа и памяти, идемпотентный повтор, отказ устаревшему родителю, наследование, происхождение проекции и обратное событие намерения сохраняют прежнее покрытие.
...
Прототип доказывает самодостаточное переисполнение сохранённых принятых событий, межпроцессный CAS файлового указателя для сотрудничающих писателей и восстановление между штатно завершившимися процессами. За границей остаются журнал отклонённых кандидатов, process-crash- и power-loss-durability, языконейтральность канонических байтов, чтение или миграция схемы поколения `1`, сборка мусора, сетевые файловые системы, внутрипроцессная многопоточность, масштабирование, разграничение доступа, качество автоматически найденных структурирующих операторов, агентский цикл и способность FUM самостоятельно синтезировать следующий исполняемый пакет. Фикстура является версионированным инженерным сценарием, а не записью живой модели.

Статус: действующий безоконный Swift-прототип с поколениями схемы `2`, полным каноническим журналом принятых событий, самодостаточным replay снимка, трассы, происхождения и инертной декларативной модели представления и межпроцессным CAS `CURRENT`. Журнал отклонённых кандидатов, аварийная durability, языконейтральный байтовый профиль, GUI и renderer отсутствуют.
...
<!-- content-sha256: sha256:567bd3d3792064fa796c74672b4a93cc12bb22774e61e915aa998c863ee8193c -->
<!-- FUM-MD-RECENCY:END -->
